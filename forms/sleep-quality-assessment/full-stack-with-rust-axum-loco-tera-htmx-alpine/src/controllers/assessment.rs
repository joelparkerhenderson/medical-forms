...
use crate::engine::{sleep_quality_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "sleep_quality_psqi" => "sleepQualityPsqi",
        "daytime_sleepiness" => "daytimeSleepiness",
        "sleep_disturbances" => "sleepDisturbances",
        "sleep_apnoea_screening" => "sleepApnoeaScreening",
...
async fn show_report(
    ...
    // Run grading engine
    let (sleep_quality, psqi_score, ess_score, stop_bang_score, fired_rules) =
        sleep_quality_grader::calculate_sleep_quality(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        sleep_quality: sleep_quality.clone(),
        psqi_score,
        ess_score,
        stop_bang_score,
    ...
    let ess_category = crate::engine::utils::ess_category(ess_score);
    let stop_bang_category = crate::engine::utils::stop_bang_category(stop_bang_score);

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("sleep_quality", &sleep_quality);
    context.insert("sleep_quality_label", &crate::engine::utils::sleep_quality_label(&sleep_quality));
    context.insert("psqi_score", &psqi_score);
    context.insert("ess_score", &ess_score);
    context.insert("ess_category", &ess_category);
    context.insert("stop_bang_score", &stop_bang_score);
    context.insert("stop_bang_category", &stop_bang_category);
