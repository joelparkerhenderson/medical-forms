<script lang="ts">
	...
	import { psqiScoreLabel, psqiScoreColor, calculateAge, sleepEfficiencyCalc } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `sleep-quality-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">PSQI Score Breakdown</h2>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				...
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					...
					<div>
						<span class="font-medium text-gray-600">Usual Bedtime:</span>
						{data.sleepHabits.usualBedtime || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Usual Wake Time:</span>
						{data.sleepHabits.usualWakeTime || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Hours of Sleep:</span>
						{data.sleepDuration.actualSleepHours ?? 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Sleep Efficiency:</span>
					...
					<div>
						<span class="font-medium text-gray-600">Sleep Environment:</span>
						{data.sleepHabits.sleepEnvironment || 'N/A'}
					</div>
				</div>
			</div>
			...
			<!-- Lifestyle Factors -->
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				<h2 class="mb-4 text-lg font-bold text-gray-900">Lifestyle Factors</h2>
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					<div>
						<span class="font-medium text-gray-600">Caffeine:</span>
						{data.medicalLifestyle.caffeineIntake || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Alcohol:</span>
						{data.medicalLifestyle.alcoholUse || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Exercise:</span>
						{data.medicalLifestyle.exerciseFrequency || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Screen Time Before Bed:</span>
						{data.medicalLifestyle.screenTimeBeforeBed || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Shift Work:</span>
						{data.medicalLifestyle.shiftWork || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Driving Drowsiness:</span>
						{data.daytimeDysfunction.drivingDrowsiness || 'N/A'}
					</div>
				</div>
