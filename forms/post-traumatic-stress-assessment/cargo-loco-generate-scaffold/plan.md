...
Generate Loco (loco.rs) scaffolding for the `post-traumatic-stress-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name post-traumatic-stress-assessment --db postgres --bg async --assets serverside
   ```
