...
use crate::engine::{mobility_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "mobility_history" => "mobilityHistory",
        "balance_assessment" => "balanceAssessment",
        "gait_analysis" => "gaitAnalysis",
        "transfers_bed_mobility" => "transfersBedMobility",
        "stairs_obstacles" => "stairsObstacles",
        "upper_limb_function" => "upperLimbFunction",
        "assistive_devices" => "assistiveDevices",
        "falls_risk_assessment" => "fallsRiskAssessment",
...
async fn show_report(
    ...
    // Run grading engine
    let (mobility_level, mobility_score, fired_rules) =
        mobility_grader::calculate_mobility(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        mobility_level: mobility_level.clone(),
        mobility_score,
    ...
    let falls_risk = crate::engine::utils::falls_risk_category(
        &assessment_data.falls_risk_assessment.falls_in_past_year,
    );

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("mobility_level", &mobility_level);
    context.insert("mobility_label", &crate::engine::utils::mobility_level_label(&mobility_level));
    context.insert("mobility_score", &mobility_score);
    context.insert("falls_risk", &falls_risk);
