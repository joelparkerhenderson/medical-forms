...
Generate Loco (loco.rs) scaffolding for the `framingham-risk-score-for-hard-coronary-heart-disease` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name framingham-risk-score-for-hard-coronary-heart-disease --db postgres --bg async --assets serverside
   ```
