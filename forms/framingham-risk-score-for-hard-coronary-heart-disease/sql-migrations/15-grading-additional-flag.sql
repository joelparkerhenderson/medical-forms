...
-- Additional safety-critical flags detected during Framingham risk grading.
...
COMMENT ON TABLE grading_additional_flag IS
    'Additional safety-critical flags detected during Framingham risk grading (e.g. hypertensive crisis, model applicability warnings for diabetic or prior CHD patients).';
...
COMMENT ON COLUMN grading_additional_flag.flag_id IS
    'Identifier of the flag (e.g. FLAG-HYPERT-001, FLAG-DIABETIC-001, FLAG-PRIORCHD-001).';
COMMENT ON COLUMN grading_additional_flag.category IS
    'Category of the flag (e.g. Blood Pressure, Model Applicability, Combined Risk).';
