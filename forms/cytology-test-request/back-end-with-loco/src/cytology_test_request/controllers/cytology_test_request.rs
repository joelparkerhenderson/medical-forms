...
use crate::models::_entities::cytology_test_requests::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.hpv_test_requested = Set(self.hpv_test_requested);
      item.previous_abnormal_cytology = Set(self.previous_abnormal_cytology.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/cytology_test_requests/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
