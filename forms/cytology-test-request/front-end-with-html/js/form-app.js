...
// Cytology Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'cytology-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 6;
...
function lilyInputClass(type) {
  switch (type) {
    ...
    case 'datetime-local': return 'datetime-local-input';
...
const SPECIMEN_TYPE_OPTIONS = [
  { value: 'cervical-smear', label: 'Cervical smear' },
  { value: 'urine-cytology', label: 'Urine cytology' },
  { value: 'sputum-cytology', label: 'Sputum cytology' },
  { value: 'fluid-pleural-ascitic', label: 'Fluid — pleural / ascitic' },
  { value: 'fine-needle-aspiration-thyroid', label: 'FNA — thyroid' },
  { value: 'fine-needle-aspiration-breast', label: 'FNA — breast' },
  { value: 'csf-cytology', label: 'CSF cytology' },
  { value: 'other', label: 'Other' }
];

const INDICATION_OPTIONS = [
  { value: 'cervical-screening', label: 'Cervical screening' },
  { value: 'suspected-malignancy', label: 'Suspected malignancy' },
  { value: 'haematuria', label: 'Haematuria' },
  { value: 'effusion-investigation', label: 'Effusion investigation' },
...
const PREVIOUS_CYTOLOGY_OPTIONS = [
...
const SPECIMEN_COLLECTED_OPTIONS = [
  { value: 'yes', label: 'Yes — already collected' },
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Specimen type, site, indication, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Requested specimen type', section: 'request', field: 'specimenType',
    options: SPECIMEN_TYPE_OPTIONS, required: true
  }));
  card.appendChild(textInput({ label: 'Specimen site / source', section: 'request', field: 'specimenSite', placeholder: 'e.g. cervix, right pleural cavity, left thyroid lobe' }));
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Exclude malignant cells in pleural effusion.'
  }));
  card.appendChild(textArea({ label: 'Relevant clinical details', section: 'request', field: 'clinicalDetails', rows: 3, placeholder: 'History, prior imaging, prior results.' }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Cytology context',
    description: 'HPV testing, previous abnormal cytology, and cervical-smear timing.'
  });
  card.appendChild(boolField({ label: 'High-risk HPV test requested', section: 'context', field: 'hpvTestRequested' }));
  card.appendChild(grid('two-col', [
    selectInput({
      label: 'Previous abnormal cytology', section: 'context', field: 'previousAbnormalCytology',
      options: PREVIOUS_CYTOLOGY_OPTIONS
    }),
    textInput({ label: 'Last menstrual period (LMP)', section: 'context', field: 'lastMenstrualPeriodDate', type: 'date' })
  ]));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'Pre-analytical timing — drives the specimen-adequacy axis.'
  });
  card.appendChild(selectInput({
    label: 'Specimen collected?', section: 'collection', field: 'specimenCollected',
    options: SPECIMEN_COLLECTED_OPTIONS
  }));
  card.appendChild(textInput({ label: 'Collection date & time', section: 'collection', field: 'collectionDatetime', type: 'datetime-local' }));
  return card;
}
...
const TRACKED_FIELDS = [
  ...
  // 3 Request
  ['request', 'specimenType'], ['request', 'specimenSite'],
  ['request', 'primaryIndication'], ['request', 'clinicalQuestion'],
  ['request', 'clinicalDetails'],
  // 4 Context
  ['context', 'previousAbnormalCytology'],
  // 5 Collection
  ['collection', 'specimenCollected'],
  // 6 Triage
...
const STEP_DEFINITIONS = [
  ...
  { step: 5, section: 'collection', title: 'Collection' },
...
function renderReport() {
  ...
  const preanalyticalValue = preanalyticalBand
    ? `<span class="band-badge preanalytical-${esc(preanalyticalBand)}">${esc(titleCase(preanalyticalBand))}</span>`
  ...
  out.innerHTML = `
    <h2>Cytology Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Specimen: ${esc(specimenTypeLabel(state.request.specimenType) || '—')}${state.request.primaryIndication ? ` · ${esc(indicationLabel(state.request.primaryIndication))}` : ''}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Pre-analytical adequacy</span>
        <span class="axis-value">${preanalyticalValue}</span>
      </div>
      <div class="axis-card">
