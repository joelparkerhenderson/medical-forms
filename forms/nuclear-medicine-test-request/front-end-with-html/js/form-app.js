...
import { emptyRequest, indicationLabel, scanTypeLabel } from './types.js';

// Nuclear Medicine Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'nuclear-medicine-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'Patient demographics and weight (used for administered-activity calculation).'
  });
  card.appendChild(grid('two-col', [
...
function renderStep3() {
  ...
  card.appendChild(selectInput({
    label: 'Requested scan type', section: 'request', field: 'scanType',
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Identify osseous metastatic disease in a patient with prostate cancer and bone pain.'
  }));
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Pregnancy, breastfeeding, renal function, and recent radionuclide exposure drive the preparation & radiation-safety band.'
  });
  card.appendChild(selectInput({
    label: 'Pregnancy status', section: 'safety', field: 'pregnancyStatus',
    options: PREGNANCY_OPTIONS, required: true
  }));
  card.appendChild(grid('two-col', [
    textInput({ label: 'eGFR', section: 'safety', field: 'egfr', type: 'number', min: 0, max: 200, step: 0.1, unit: 'mL/min/1.73m²' }),
    document.createElement('div')
  ]));
  card.appendChild(subHead('Radiation-safety checks'));
  card.appendChild(boolField({ label: 'Currently breastfeeding', section: 'safety', field: 'breastfeeding' }));
  card.appendChild(boolField({ label: 'Recent other radionuclide study', section: 'safety', field: 'recentOtherNuclearScan' }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'IR(ME)R justification',
    description: 'Referrer justification statement for the exposure and the responsible supervising consultant.'
  });
  card.appendChild(textArea({
    label: 'IR(ME)R justification statement', section: 'justification', field: 'irMeRJustification',
    rows: 3, required: true,
    placeholder: 'Clinical rationale that justifies the radiation exposure for this patient.'
  }));
  card.appendChild(textInput({ label: 'Supervising / responsible consultant', section: 'justification', field: 'supervisingConsultant' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Triage',
    description: 'Requested urgency, requested-by date, setting, and site. High-acuity indications auto-escalate the triage tier.'
  });
  card.appendChild(grid('two-col', [
...
function renderStep7() {
  const card = sectionCard({
    stepNumber: 7,
    title: 'Review & submit',
    description: 'Add any notes, then submit to compute the four-axis grade, flags, and recommendation.'
  });
...
const TRACKED_FIELDS = [
  ...
  // 5 Justification
  ['justification', 'irMeRJustification'],
  // 6 Triage
  ['triage', 'urgency'], ['triage', 'setting']
];

const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'request', 4: 'safety',
  5: 'justification', 6: 'triage', 7: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 4, section: 'safety',        title: 'Radiation safety' },
  { step: 5, section: 'justification', title: 'Justification' },
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>Nuclear Medicine Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Scan: ${esc(scanTypeLabel(state.request.scanType) || '—')}${state.request.primaryIndication ? ` · ${esc(indicationLabel(state.request.primaryIndication))}` : ''}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Preparation &amp; radiation safety</span>
        <span class="axis-value">${prepValue} ${doseValue}</span>
      </div>
      <div class="axis-card">
