...
use crate::models::_entities::nuclear_medicine_test_requests::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.recent_other_nuclear_scan = Set(self.recent_other_nuclear_scan);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/nuclear_medicine_test_requests/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
