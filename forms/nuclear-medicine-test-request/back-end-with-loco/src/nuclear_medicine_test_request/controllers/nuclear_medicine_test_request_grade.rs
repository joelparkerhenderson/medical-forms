...
use crate::models::_entities::nuclear_medicine_test_request_grades::{ActiveModel, Entity, Model};

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Params {
    pub deleted_at: Option<DateTimeWithTimeZone>,
    pub nuclear_medicine_test_request_id: i64,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.nuclear_medicine_test_request_id = Set(self.nuclear_medicine_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.prep_safety_band = Set(self.prep_safety_band.clone());
      item.radiation_dose_band = Set(self.radiation_dose_band.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/nuclear_medicine_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
