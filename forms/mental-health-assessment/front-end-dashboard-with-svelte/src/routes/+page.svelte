<script lang="ts">
	...
	function applyFilters() {
		...
		const filter = (row: PatientRow) => {
			...
			// PHQ-9 severity filter
			if (phq9Filter && phq9Severity(row.phq9Score) !== phq9Filter) {
				return false;
			}
	...
	let hasActiveFilters = $derived(
		searchTerm !== '' || riskFilter !== '' || phq9Filter !== '' || allergyFilter !== ''
	);
</script>
...
<div class="min-h-screen bg-gray-50">
	...
	<header class="bg-nhs-blue text-white shadow">
		<div class="mx-auto max-w-7xl px-4 py-4 sm:px-6">
			<h1 class="text-2xl font-bold">Mental Health Assessment — Clinician Dashboard</h1>
			<p class="mt-1 text-sm text-blue-100">Patient list with mental health assessment status</p>
		</div>
	</header>
	...
	<main class="mx-auto max-w-7xl px-4 py-6 sm:px-6">
		...
		<div class="mb-4 rounded-lg bg-white p-4 shadow-sm">
			<div class="flex flex-wrap items-end gap-4">
				...
				<!-- PHQ-9 Severity -->
				<div>
					<label for="phq9-filter" class="mb-1 block text-sm font-medium text-gray-700">
						PHQ-9 Severity
					</label>
					<select
						id="phq9-filter"
						bind:value={phq9Filter}
						onchange={applyFilters}
						class="rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none"
					>
						{#each phq9Options as opt (opt.value)}
							<option value={opt.value}>{opt.label}</option>
						{/each}
					</select>
				</div>
