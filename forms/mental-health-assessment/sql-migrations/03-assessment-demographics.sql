...
-- Demographics section of the mental health assessment.
...
CREATE TABLE assessment_demographics (
    ...
    living_situation VARCHAR(20) NOT NULL DEFAULT ''
        CHECK (living_situation IN ('alone', 'with-family', 'with-partner', 'shared', 'supported', 'homeless', '')),
...
COMMENT ON TABLE assessment_demographics IS
    'Demographics section: occupation, marital status, living situation, and contact information. One-to-one child of assessment.';
...
COMMENT ON COLUMN assessment_demographics.marital_status IS
    'Marital status: single, married, civil-partnership, divorced, widowed, separated, or empty string.';
COMMENT ON COLUMN assessment_demographics.living_situation IS
    'Living situation: alone, with-family, with-partner, shared, supported, homeless, or empty string.';
