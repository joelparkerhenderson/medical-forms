...
// Eye Vision Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'eye-vision-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
const TEST_TYPE_OPTIONS = [
  ...
  { value: 'optical-coherence-tomography', label: 'Optical coherence tomography (OCT)' },
...
const INDICATION_OPTIONS = [
  { value: 'reduced-vision', label: 'Reduced vision' },
  { value: 'suspected-glaucoma', label: 'Suspected glaucoma' },
  { value: 'diabetic-retinopathy-screening', label: 'Diabetic retinopathy screening' },
  { value: 'sudden-visual-loss', label: 'Sudden visual loss' },
  { value: 'flashes-floaters', label: 'Flashes / floaters' },
  { value: 'red-eye', label: 'Red eye' },
  { value: 'childhood-squint', label: 'Childhood squint' },
  { value: 'visual-field-defect', label: 'Visual-field defect' },
  { value: 'cataract-assessment', label: 'Cataract assessment' },
  { value: 'headache-visual-symptoms', label: 'Headache with visual symptoms' },
  { value: 'other', label: 'Other' }
];
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Test type, eye(s), indication, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(grid('two-col', [
    selectInput({
    ...
    selectInput({
      label: 'Laterality (eye)', section: 'request', field: 'laterality',
      options: LATERALITY_OPTIONS, required: true
    })
  ]));
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Confirm or exclude glaucomatous visual-field loss.'
  }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'History',
    description: 'Relevant ocular, medical, and family history.'
  });
  card.appendChild(textArea({
    label: 'Relevant history', section: 'request', field: 'relevantHistory', rows: 3,
    placeholder: 'e.g. Family history of glaucoma; previous retinal laser; current eye drops.'
  }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'Red flags auto-escalate the triage tier to emergency regardless of the other axes.'
  });
  card.appendChild(boolField({ label: 'Reduced vision', section: 'symptoms', field: 'reducedVision' }));
  card.appendChild(subHead('Red flags'));
  card.appendChild(boolField({ label: 'Sudden visual loss', section: 'symptoms', field: 'suddenLoss' }));
  card.appendChild(boolField({ label: 'Flashes and/or floaters', section: 'symptoms', field: 'flashesFloaters' }));
  card.appendChild(boolField({ label: 'Eye pain', section: 'symptoms', field: 'eyePain' }));
  card.appendChild(boolField({ label: 'Red eye', section: 'symptoms', field: 'redEye' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Risk factors',
    description: 'Risk factors relevant to clinical priority.'
  });
  card.appendChild(boolField({ label: 'Diabetes (retinopathy risk)', section: 'riskFactors', field: 'diabetes' }));
  card.appendChild(boolField({ label: 'Known / treated glaucoma', section: 'riskFactors', field: 'knownGlaucoma' }));
  return card;
}
...
const SECTION_BY_STEP = {
  ...
  5: 'symptoms', 6: 'riskFactors', 7: 'triage'
};
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>Eye Vision Test Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Test: ${esc(testTypeLabel(state.request.testType) || '—')}${state.request.primaryIndication ? ` · ${esc(indicationLabel(state.request.primaryIndication))}` : ''}</p>
