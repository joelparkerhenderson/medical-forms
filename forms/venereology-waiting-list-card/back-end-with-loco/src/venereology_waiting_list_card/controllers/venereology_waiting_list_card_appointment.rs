...
use crate::models::_entities::venereology_waiting_list_card_appointments::{ActiveModel, Entity, Model};

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Params {
    pub deleted_at: Option<DateTimeWithTimeZone>,
    pub venereology_waiting_list_card_id: i64,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.venereology_waiting_list_card_id = Set(self.venereology_waiting_list_card_id);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/venereology_waiting_list_card_appointments/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
