# First Aid Training Checklist

First Aid at Work (FAW) competency assessment aligned with UK HSE guidance and the St John Ambulance syllabus, covering scene assessment, primary survey, life-threatening emergencies, and injury management with a pass / fail / needs-development outcome.

## Scoring system

- **Instrument**: First Aid at Work Competency Assessment
- **Range**: Pass / Fail / Needs Development
- **Categories**:
  - Pass: All required skills demonstrated to competent standard
  - Needs Development: Minor deficiencies; targeted retraining required
  - Fail: Critical deficiency in life-saving skills; full reassessment required

## Steps

| #   | Step                                         |
| --- | -------------------------------------------- |
| 1   | Trainee Details                              |
| 2   | Scene Assessment & Safety                    |
| 3   | Primary Survey (DRABC)                       |
| 4   | CPR & AED                                    |
| 5   | Choking Management                           |
| 6   | Bleeding & Wound Care                        |
| 7   | Burns & Scalds                               |
| 8   | Fractures, Sprains & Spinal Injury           |
| 9   | Medical Emergencies (stroke, seizure, anaphylaxis) |
| 10  | Recording, Reporting & Handover              |

## Directory structure

```
first-aid-training-checklist/
  front-end-form-with-svelte/
  front-end-dashboard-with-svelte/
```

## Technology

See [root index.md](../index.md) for technology stacks.
