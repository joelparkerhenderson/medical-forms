...
// inline structured echocardiography report. State is persisted to
...
const STORAGE_KEY = 'echocardiogram-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').EchocardiogramResult} */
...
/** @param {import('./types.js').EchocardiogramResult} state */
...
/** @type {import('./types.js').EchocardiogramResult} */
let state = loadState();
...
function renderStep1() {
  const card = sectionCard({
    ...
    description: 'Who authored the report, the study type and quality, and key dates.'
  });
  ...
  card.appendChild(selectInput({
    label: 'Study quality',
    field: 'studyQuality',
    hint: 'Acoustic-window / image quality. A poor-quality study classifies as inconclusive.',
    options: [
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The clinical question the study was performed to answer.'
  });

  card.appendChild(textArea({
    label: 'Clinical history',
    field: 'clinicalHistory', rows: 4,
    placeholder: 'Clinical history and the question the echocardiogram was performed to answer…'
  }));
...
function renderStep3() {
  const card = sectionCard({
    stepNumber: 3,
    title: 'LV function and dimensions',
    description: 'Left-ventricular ejection fraction, qualitative function, and dimensions.'
  });

  card.appendChild(numberInput({
    label: 'Ejection fraction — LVEF (%)',
    field: 'lvEjectionFractionPercent',
    min: 0, max: 100, step: 0.1,
    hint: 'Left-ventricular ejection fraction as a percentage (0–100). An EF below 30% grades as severe LV impairment.'
  }));
  card.appendChild(selectInput({
    label: 'Qualitative LV function',
    field: 'lvFunction',
    options: [
      { value: 'normal', label: 'Normal' },
      { value: 'mildly-impaired', label: 'Mildly impaired' },
      { value: 'moderately-impaired', label: 'Moderately impaired' },
      { value: 'severely-impaired', label: 'Severely impaired' }
    ]
  }));
  card.appendChild(numberInput({
    label: 'LV internal diameter — LVIDd (mm)',
    field: 'lvInternalDiameterDiastoleMm',
    min: 0, max: 200, step: 0.1,
    hint: 'Left-ventricular internal diameter at end-diastole.'
  }));
  card.appendChild(checkboxGroup({
    label: 'LV structure',
    items: [
      { field: 'lvHypertrophy', label: 'Left-ventricular hypertrophy' },
      { field: 'regionalWallMotionAbnormality', label: 'Regional wall-motion abnormality' }
    ]
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Valves and pressures',
    description: 'Valvular stenosis and regurgitation grades, and estimated pulmonary pressure.'
  });

  card.appendChild(selectInput({
    label: 'Aortic stenosis',
    field: 'aorticStenosis',
    options: VALVE_GRADE_OPTIONS
  }));
  card.appendChild(selectInput({
    label: 'Aortic regurgitation',
    field: 'aorticRegurgitation',
    options: VALVE_GRADE_OPTIONS
  }));
  card.appendChild(selectInput({
    label: 'Mitral stenosis',
    field: 'mitralStenosis',
    options: VALVE_GRADE_OPTIONS
  }));
  card.appendChild(selectInput({
    label: 'Mitral regurgitation',
    field: 'mitralRegurgitation',
    options: VALVE_GRADE_OPTIONS
  }));
  card.appendChild(numberInput({
    label: 'Pulmonary artery systolic pressure — PASP (mmHg)',
    field: 'pulmonaryArterySystolicPressureMmhg',
    min: 0, max: 200, step: 0.1,
    hint: 'Estimated pulmonary artery systolic pressure.'
  }));

  card.appendChild(conditionalAlert({
    id: 'severe-valve-alert',
    type: 'error',
    heading: 'Severe valve disease selected',
    message: 'Severe valvular disease auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'Structured finding flags that drive classification, severity, and alerts.'
  });

  card.appendChild(checkboxGroup({
    label: 'Structured findings',
    items: [
      { field: 'pericardialEffusion', label: 'Pericardial effusion' },
      { field: 'vegetation', label: 'Vegetation (suspected endocarditis)' },
      { field: 'intracardiacThrombus', label: 'Intracardiac thrombus' },
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'A pericardial effusion, vegetation, or intracardiac thrombus auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep6() {
  const card = sectionCard({
    ...
    description: 'The narrative findings, comparison, summary impression, and follow-up.'
  });
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'severe-valve-alert': hasSevereValveDisease(state),
    'critical-finding-alert':
      state.pericardialEffusion || state.vegetation || state.intracardiacThrombus,
...
// counts only when ticked; a number counts when non-null). This lets the
// checkbox-only slots count as answered once any structured finding is
// recorded — including the explicit "Normal study" tick on step 5.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['echoType'],
      ['reportStatus'], ['studyQuality'], ['performedDate'], ['reportedDate']],
  2: [['clinicalHistory']],
  3: [['lvEjectionFractionPercent'], ['lvFunction'], ['lvInternalDiameterDiastoleMm'],
      ['lvHypertrophy', 'regionalWallMotionAbnormality']],
  4: [['aorticStenosis'], ['aorticRegurgitation'], ['mitralStenosis'],
      ['mitralRegurgitation'], ['pulmonaryArterySystolicPressureMmhg']],
  5: [['pericardialEffusion', 'vegetation', 'intracardiacThrombus', 'normalStudy']],
  6: [['findingsNarrative'], ['comparisonWithPrevious'], ['impression'], ['recommendedFollowUp']],
...
/** Format a nullable measurement with its unit for the report tables. */
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Echo type', echoTypeLabel(state.echoType)],
    ['Report status', reportStatusLabel(state.reportStatus)],
    ['Study quality', studyQualityLabel(state.studyQuality)],
    ['Performed date', state.performedDate || 'Not recorded'],
    ['Reported date', state.reportedDate || 'Not recorded'],
    ['Ejection fraction (LVEF)', measurement(state.lvEjectionFractionPercent, '%')],
    ['Qualitative LV function', lvFunctionLabel(state.lvFunction)],
    ['LV internal diameter (LVIDd)', measurement(state.lvInternalDiameterDiastoleMm, 'mm')],
    ['Pulmonary pressure (PASP)', measurement(state.pulmonaryArterySystolicPressureMmhg, 'mmHg')],
