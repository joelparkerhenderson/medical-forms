...
`F-UNEXPECTED-FINDING-001` checked only severe valve disease, vegetation, and intracardiac thrombus — not a pericardial effusion or severe LV impairment, both otherwise-critical structured findings. Fixed by widening the flag's condition to add both, in both `js/flags.js` and `src/lib/engine/flagged-issues.ts`. Fixed 2026-09-06; previously verified and documented (not silently patched) in `examples/personas.json`.
