...
Generate Loco (loco.rs) scaffolding for the `medical-records-release-permission` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name medical-records-release-permission --db postgres --bg async --assets serverside
   ```
