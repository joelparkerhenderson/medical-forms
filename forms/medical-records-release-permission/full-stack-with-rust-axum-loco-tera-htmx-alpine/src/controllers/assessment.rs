...
use crate::engine::{release_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "records_specification" => "recordsSpecification",
        "purpose_of_release" => "purposeOfRelease",
        "authorization_scope" => "authorizationScope",
        "sensitive_information" => "sensitiveInformation",
        "duration_expiry" => "durationExpiry",
        "verification_identity" => "verificationIdentity",
        "signatures_consent" => "signaturesConsent",
...
/// Parse a form value string: empty strings become null, otherwise keep as string.
...
/// All fields in this form are string fields (no Likert numeric conversion needed).
fn parse_field_value(_json_field: &str, value: &serde_json::Value) -> serde_json::Value {
    if let Some(s) = value.as_str() {
...
async fn show_report(
    ...
    // Run grading engine
    let (completion_level, completion_score, fired_rules) =
        release_grader::calculate_completion(&assessment_data);
    ...
    let review_decision = crate::engine::utils::review_decision_label(
        &assessment_data.clinical_review.review_decision,
    );

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("completion_level", &completion_level);
    context.insert("completion_label", &crate::engine::utils::completion_level_label(&completion_level));
    context.insert("completion_score", &completion_score);
    context.insert("review_decision", &review_decision);
