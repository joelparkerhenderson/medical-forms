...
function getFilteredPatients() {
  ...
  const completenessFilter = document.getElementById('completeness-filter').value;
  ...
  return patients.filter(p => {
    ...
    if (completenessFilter && p.completenessLabel !== completenessFilter) return false;
    return true;
  });
}
...
function renderTable() {
  ...
  if (filtered.length === 0) {
    ...
    tbody.innerHTML = filtered.map(p => {
      ...
      const completenessClass = p.completenessLabel === 'complete' ? 'completeness-complete'
        : p.completenessLabel === 'nearly-complete' ? 'completeness-nearly'
      ...
      return '<tr>' +
        ...
        '<td>' + esc(p.recipientName) + '<br><small style="color:#6b7280">' + esc(p.recipientOrganization) + '</small></td>' +
        '<td><span class="status-badge ' + statusClass + '">' + capitalize(p.status) + '</span></td>' +
        '<td><span class="completeness-badge ' + completenessClass + '">' + p.completenessScore + '%</span> ' + flagBadge + '</td>' +
...
function bindEvents() {
  ...
  document.getElementById('completeness-filter').addEventListener('change', renderTable);
  document.getElementById('clear-filters').addEventListener('click', () => {
    document.getElementById('search-input').value = '';
    document.getElementById('status-filter').value = '';
    document.getElementById('completeness-filter').value = '';
    renderTable();
  });
