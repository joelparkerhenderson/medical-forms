...
// inline structured neurophysiology report. State is persisted to
...
const STORAGE_KEY = 'electroencephalogram-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').ElectroencephalogramResult} */
...
function mergeIntoDefaults(parsed) {
  ...
  // Nullable numeric fields (defaulting to null) accept a saved number or
  // null; every other field must match the type of its fresh default.
...
/** @param {import('./types.js').ElectroencephalogramResult} state */
...
/** @type {import('./types.js').ElectroencephalogramResult} */
let state = loadState();
...
function textInput(opts) {
  ...
  input.addEventListener('input', () => {
    if (type === 'number') {
      // Unanswered numeric fields are null, never '' (repo convention).
      const raw = input.value.trim();
      const num = raw === '' ? null : Number(raw);
      setField(opts.field, num === null || Number.isNaN(num) ? null : num);
    } else {
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The performed recording type, duration, and technical quality.'
  });
  ...
  card.appendChild(selectInput({
    ...
    field: 'eegType',
    options: [
      { value: 'routine-awake', label: 'Routine awake' },
      { value: 'sleep-deprived', label: 'Sleep-deprived' },
      { value: 'ambulatory-24h', label: 'Ambulatory 24-hour' },
      { value: 'video-telemetry', label: 'Video telemetry' },
      { value: 'other', label: 'Other' }
    ]
  }));
  card.appendChild(textInput({
    label: 'Recording duration (minutes)',
    field: 'recordingDurationMinutes', type: 'number',
    min: 0, max: 100000, step: 0.1,
    hint: 'Total EEG recording duration in minutes.'
  }));
  card.appendChild(selectInput({
  ...
  card.appendChild(conditionalAlert({
    id: 'limited-quality-warning',
    type: 'warning',
    heading: 'Limited recording quality',
    message: 'A limited / non-interpretable recording reduces diagnostic confidence; a limited recording with no impression classifies the result as inconclusive.'
  }));
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'The clinical question and any comparison with previous EEG.'
  });

  card.appendChild(textArea({
    label: 'Clinical history',
    field: 'clinicalHistory', rows: 3,
    placeholder: 'Clinical history and the question the EEG was performed to answer…'
  }));
  card.appendChild(textArea({
    label: 'Comparison with previous EEG',
    field: 'comparisonWithPrevious', rows: 3,
    placeholder: 'Relevant previous EEG studies and changes since…'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Findings',
    description: 'The background rhythm, structured neurophysiological findings, and narrative.'
  });

  card.appendChild(selectInput({
    label: 'Background rhythm',
    field: 'backgroundRhythm',
    options: [
      { value: 'normal', label: 'Normal' },
      { value: 'excess-slow', label: 'Excess slow' },
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'epileptiformDischarges', label: 'Epileptiform discharges' },
      { field: 'focalSlowing', label: 'Focal slowing' },
      { field: 'generalisedSlowing', label: 'Generalised slowing' },
      { field: 'seizureRecorded', label: 'Seizure recorded' },
      { field: 'statusEpilepticus', label: 'Status epilepticus' },
      { field: 'photoparoxysmalResponse', label: 'Photoparoxysmal response' },
      { field: 'normalEeg', label: 'Normal EEG' }
    ]
  }));
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'Status epilepticus, a recorded seizure, or epileptiform discharges auto-escalate the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'Correlate the findings with the clinical context and the structured-reporting category.'
  });

  card.appendChild(textArea({
    label: 'Clinical correlation',
    field: 'clinicalCorrelation', rows: 4,
    placeholder: 'Correlation of the EEG findings with the clinical context and question…'
  }));
  card.appendChild(textInput({
    label: 'Reporting category',
    field: 'reportingCategory',
    hint: 'A structured-reporting category label (e.g. a SCORE category or epileptiform-classification label).',
    placeholder: 'e.g. SCORE: focal epileptiform'
  }));
...
function renderStep6() {
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'Recommended follow-up recording, referral, or management…'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'limited-quality-warning': state.recordingQuality === 'limited',
    'critical-finding-alert':
      state.statusEpilepticus || state.seizureRecorded || state.epileptiformDischarges,
...
const STEP_SLOTS = {
  ...
  2: [['eegType'], ['recordingDurationMinutes'], ['recordingQuality']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['backgroundRhythm'], ['findingsNarrative'],
      ['epileptiformDischarges', 'focalSlowing', 'generalisedSlowing',
       'seizureRecorded', 'statusEpilepticus', 'photoparoxysmalResponse',
       'normalEeg']],
  5: [['clinicalCorrelation'], ['reportingCategory']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['EEG type', eegTypeLabel(state.eegType)],
    ['Recording duration',
      state.recordingDurationMinutes === null
        ? 'Not recorded'
        : `${state.recordingDurationMinutes} min`],
    ['Recording quality', recordingQualityLabel(state.recordingQuality)],
    ['Background rhythm', backgroundRhythmLabel(state.backgroundRhythm)],
  ...
  out.innerHTML = `
    <div class="report-card">
      <header class="report-header">
        <h2>Electroencephalogram Report</h2>
      ...
      ${state.clinicalCorrelation ? `<p class="muted">${esc(state.clinicalCorrelation)}</p>` : ''}
