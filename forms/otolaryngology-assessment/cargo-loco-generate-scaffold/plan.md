...
Generate Loco (loco.rs) scaffolding for the `otolaryngology-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name otolaryngology-assessment --db postgres --bg async --assets serverside
   ```
