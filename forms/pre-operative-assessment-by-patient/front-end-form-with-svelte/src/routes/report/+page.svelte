<script lang="ts">
	...
	import { asaGradeLabel, asaGradeColor, bmiCategory, calculateAge } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `preop-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
...
{#if result}
	<div class="min-h-screen bg-gray-50">
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			<!-- ASA Grade Banner -->
			<div class="mb-6 rounded-xl border-2 p-6 text-center {asaGradeColor(result.asaGrade)}">
				<div class="text-3xl font-bold">ASA {result.asaGrade}</div>
				<div class="mt-1 text-lg">{asaGradeLabel(result.asaGrade)}</div>
			...
			{#if result.additionalFlags.length > 0}
				<div class="mb-6 rounded-xl border border-red-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-red-800">Flagged Issues for Anaesthetist</h2>
					<div class="space-y-2">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">ASA Grade Justification</h2>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				...
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					...
					<div class="sm:col-span-2">
						<span class="font-medium text-gray-600">Planned Procedure:</span>
						{data.demographics.plannedProcedure}
						({data.demographics.procedureUrgency})
					</div>
				</div>
			</div>
