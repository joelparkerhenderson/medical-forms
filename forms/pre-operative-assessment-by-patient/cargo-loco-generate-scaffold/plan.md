...
Generate Loco (loco.rs) scaffolding for the `pre-operative-assessment-by-patient` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name pre-operative-assessment-by-patient --db postgres --bg async --assets serverside
   ```
