...
// and a live four-axis interpretation preview (classification, severity,
// completeness, follow-up urgency) updates as the report is edited.
// Submission runs the pure grading engine and renders an inline structured
// lung-function report with the fired-rule audit trail and safety flags.
...
const STORAGE_KEY = 'pulmonary-function-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').PulmonaryFunctionResult} */
...
function mergeIntoDefaults(parsed) {
  ...
  // Numeric fields default to null, so a plain typeof comparison would
  // reject saved numbers — accept a saved value when it has the same type
  // as the default, or when either side is null (numeric '' -> null).
  ...
  if (parsed && typeof parsed === 'object') {
    for (const key of Object.keys(fresh)) {
      ...
      if (fresh[key] === null) {
        // Numeric slot: accept a finite number or null.
        if (value === null || typeof value === 'number') fresh[key] = value;
      } else if (value === null && typeof fresh[key] === 'number') {
        fresh[key] = null;
...
/** @param {import('./types.js').PulmonaryFunctionResult} state */
...
/** @type {import('./types.js').PulmonaryFunctionResult} */
let state = loadState();
...
function textInput(opts) {
  ...
  input.addEventListener('input', () => {
    let v = input.value;
    if (type === 'number') {
      // Unanswered numeric fields are null (never '').
      v = v === '' ? null : Number(v);
    }
...
/** Numeric input: '' -> null; otherwise Number. */
...
function renderStep1() {
  ...
  card.appendChild(textInput({
    ...
    placeholder: 'e.g. Dr A Respiratory'
  }));
  card.appendChild(textInput({
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'spirometry-with-reversibility', label: 'Spirometry with reversibility' },
      { value: 'full-lung-function', label: 'Full lung function' },
      { value: 'gas-transfer-dlco', label: 'Gas transfer (DLCO)' },
      { value: 'peak-flow', label: 'Peak flow' },
...
function renderStep2() {
  const card = sectionCard({
    stepNumber: 2,
    title: 'Test quality',
    description: 'The ATS/ERS acceptability and repeatability grade, plus the clinical history.'
  });

  card.appendChild(selectInput({
    label: 'Test quality (ATS/ERS)',
    field: 'testQuality',
    hint: 'Acceptability and repeatability of the manoeuvres.',
    options: [
      { value: 'acceptable', label: 'Acceptable' },
      { value: 'sub-optimal', label: 'Sub-optimal' },
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Spirometry and gas-transfer values (litres and percent predicted).'
  });

  card.appendChild(numberInput({
    label: 'FEV1 (litres)',
    field: 'fev1Litres',
    hint: 'Forced expiratory volume in one second.',
    min: 0, max: 12, step: 0.01
  }));
  card.appendChild(numberInput({
    label: 'FEV1 (% predicted)',
    field: 'fev1PercentPredicted',
    min: 0, max: 250, step: 0.1
  }));
  card.appendChild(numberInput({
    label: 'FVC (litres)',
    field: 'fvcLitres',
    hint: 'Forced vital capacity.',
    min: 0, max: 12, step: 0.01
  }));
  card.appendChild(numberInput({
    label: 'FVC (% predicted)',
    field: 'fvcPercentPredicted',
    min: 0, max: 250, step: 0.1
  }));
  card.appendChild(numberInput({
    label: 'FEV1/FVC ratio',
    field: 'fev1FvcRatio',
    hint: 'Post-bronchodilator ratio < 0.70 confirms airflow obstruction (NICE NG115 / GOLD).',
    min: 0, max: 1, step: 0.01
  }));
  card.appendChild(numberInput({
    label: 'Peak expiratory flow (L/min)',
    field: 'peakExpiratoryFlow',
    min: 0, max: 1200, step: 0.1
  }));
  card.appendChild(numberInput({
    label: 'DLCO (% predicted)',
    field: 'dlcoPercentPredicted',
    hint: 'Diffusing capacity of the lung for carbon monoxide (transfer factor).',
    min: 0, max: 250, step: 0.1
  }));
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'The distilled ventilatory pattern, severity, reversibility, and structured findings.'
  });

  card.appendChild(selectInput({
    label: 'Ventilatory pattern',
    field: 'ventilatoryPattern',
    options: [
  ...
  card.appendChild(selectInput({
    label: 'Severity',
    field: 'severity',
    hint: 'ATS/ERS z-score or GOLD percent-predicted severity band.',
    options: [
  ...
  card.appendChild(selectInput({
    ...
    field: 'bronchodilatorReversibility',
    options: [
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'airflowObstruction', label: 'Airflow obstruction' },
      { field: 'restriction', label: 'Restriction' },
      { field: 'reducedGasTransfer', label: 'Reduced gas transfer' },
      { field: 'significantReversibility', label: 'Significant reversibility' },
      { field: 'normalSpirometry', label: 'Normal spirometry' }
    ]
  }));
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'Severe or very-severe airflow obstruction or restriction auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Findings',
    description: 'The narrative findings and any comparison with previous studies.'
  });

  card.appendChild(textArea({
    label: 'Findings narrative',
    field: 'findingsNarrative', rows: 5,
    placeholder: 'Narrative description of the lung-function findings (the body of the report)…'
  }));
  card.appendChild(textArea({
    label: 'Comparison with previous',
    field: 'comparisonWithPrevious', rows: 3,
    placeholder: 'Relevant previous lung-function studies and changes since (e.g. decline in FEV1)…'
  }));
...
function renderStep6() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Structured-reporting label, e.g. a GOLD 1-4 or ATS/ERS severity band.',
    placeholder: 'e.g. GOLD 2 (moderate)'
  }));
  card.appendChild(textArea({
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'critical-finding-alert': hasCriticalFinding(state),
...
// checkbox-only structured-findings slot count as answered once any finding
// is recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['testType'],
      ['reportStatus'], ['performedDate'], ['reportedDate']],
  2: [['testQuality'], ['clinicalHistory']],
  3: [['fev1Litres', 'fev1PercentPredicted'], ['fvcLitres', 'fvcPercentPredicted'],
      ['fev1FvcRatio'], ['peakExpiratoryFlow', 'dlcoPercentPredicted']],
  4: [['ventilatoryPattern'], ['severity'], ['bronchodilatorReversibility'],
      ['airflowObstruction', 'restriction', 'reducedGasTransfer',
       'significantReversibility', 'normalSpirometry']],
  5: [['findingsNarrative'], ['comparisonWithPrevious']],
...
/** Format a nullable numeric measured value with a unit. */
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Ventilatory pattern', ventilatoryPatternLabel(state.ventilatoryPattern)],
    ['Severity', severityLabel(state.severity)],
    ['Bronchodilator reversibility', bronchodilatorReversibilityLabel(state.bronchodilatorReversibility)],
    ['FEV1', measured(state.fev1Litres, 'L')],
    ['FEV1 % predicted', measured(state.fev1PercentPredicted, '%')],
    ['FVC', measured(state.fvcLitres, 'L')],
    ['FVC % predicted', measured(state.fvcPercentPredicted, '%')],
    ['FEV1/FVC ratio', measured(state.fev1FvcRatio, '')],
    ['Peak expiratory flow', measured(state.peakExpiratoryFlow, 'L/min')],
    ['DLCO % predicted', measured(state.dlcoPercentPredicted, '%')],
  ...
  out.innerHTML = `
    <div class="report-card">
      <header class="report-header">
        <h2>Pulmonary Function Test Report</h2>
