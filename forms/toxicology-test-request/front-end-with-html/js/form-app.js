...
const STORAGE_KEY = 'toxicology-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 6;
...
function setAssay(field, checked) {
  state.assays[field] = !!checked;
  saveState(state);
  updateProgress();
  updateAssayCount();
}
...
/**
 * Render the requested-assays checkbox group. Each assay is a Lily
 * `.checkbox-input` inside a `.checkbox-group` (role="group"). A live count of
 * selected assays sits above the group.
 */
function assayCheckboxGroup() {
  const wrapper = document.createElement('fieldset');
  wrapper.className = 'field';
  wrapper.id = 'assays-fieldset';

  const legend = document.createElement('legend');
  legend.className = 'label';
  legend.innerHTML = 'Requested assays <span class="req" aria-hidden="true">*</span>';
  wrapper.appendChild(legend);

  const hint = document.createElement('span');
  hint.className = 'hint';
  hint.id = 'assays-count';
  wrapper.appendChild(hint);

  const group = document.createElement('div');
  group.className = 'checkbox-group assay-grid';
  group.setAttribute('role', 'group');
  group.setAttribute('aria-labelledby', wrapper.id);

  for (const a of ASSAYS) {
    const cbId = `assays-${a.field}`;
    const checked = state.assays[a.field] === true ? ' checked' : '';
    const label = document.createElement('label');
    label.htmlFor = cbId;
    const tag = a.critical
      ? ' <span class="assay-tag assay-tag-critical">timing-critical</span>'
      : '';
    const note = a.note
      ? ` <span class="assay-note">${esc(a.note)}</span>`
      : '';
    label.innerHTML = `
      <input class="checkbox-input" type="checkbox" id="${cbId}" name="assays" value="${esc(a.field)}"${checked}>
      <span>${esc(a.label)}${tag}${note}</span>
    `;
    const input = label.querySelector('input');
    input.addEventListener('change', () => {
      setAssay(a.field, input.checked);
      clearFieldError('assays');
    });
  ...
  errSpan.id = 'assays-error';
  wrapper.appendChild(errSpan);
  return wrapper;
}

function updateAssayCount() {
  const el = document.getElementById('assays-count');
  if (!el) return;
  const n = countSelectedAssays(state.assays);
  el.textContent = n === 0
    ? 'No assays selected yet — select at least one.'
    : `${n} assay${n === 1 ? '' : 's'} selected.`;
}
...
const INDICATION_OPTIONS = [
  ...
  { value: 'deliberate-self-harm', label: 'Deliberate self-harm' },
  { value: 'therapeutic-drug-monitoring', label: 'Therapeutic drug monitoring' },
  { value: 'suspected-poisoning', label: 'Suspected poisoning' },
  { value: 'substance-misuse-screen', label: 'Substance-misuse screen' },
  { value: 'occupational-screen', label: 'Occupational screen' },
  { value: 'forensic', label: 'Forensic' },
  { value: 'other', label: 'Other' }
];

const SPECIMEN_OPTIONS = [
  { value: 'yes', label: 'Yes — collected' },
  { value: 'no', label: 'No — not yet collected' }
];
...
function renderStep1() {
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'toxicologist', label: 'Clinical toxicologist' },
      { value: 'gp', label: 'GP' },
...
function renderStep3() {
  const card = sectionCard({
    stepNumber: 3,
    title: 'Requested assays',
    description: 'Tick every assay to order. At least one is required. Timing-critical assays (paracetamol) depend on accurate ingestion timing.'
  });
  card.appendChild(assayCheckboxGroup());
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Indication, clinical details, and ingestion timing — the highest-value fields. A deliberate overdose or a symptomatic patient escalates triage to stat.'
  });
  card.appendChild(selectInput({
  ...
  card.appendChild(grid('two-col', [
    textInput({ label: 'Suspected agent / named drug', section: 'clinical', field: 'suspectedAgent', placeholder: 'e.g. paracetamol, lithium' }),
    textInput({ label: 'Time since ingestion', section: 'clinical', field: 'timeSinceIngestionHours', type: 'number', min: 0, max: 720, step: 0.5, unit: 'hours' })
  ]));
  card.appendChild(subHead('Escalation flags'));
  card.appendChild(boolField({ label: 'Deliberate overdose', section: 'clinical', field: 'deliberateOverdose' }));
  card.appendChild(boolField({ label: 'Patient currently symptomatic', section: 'clinical', field: 'symptomatic' }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Specimen',
    description: 'Specimen collection status and time.'
  });
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Specimen collected', section: 'specimen', field: 'specimenCollected', options: SPECIMEN_OPTIONS }),
...
const TRACKED_FIELDS = [
  ...
  ['clinical', 'suspectedAgent'], ['clinical', 'timeSinceIngestionHours'],
  // 5 Specimen
...
function updateProgress() {
  ...
  // The requested assays (step 3) count as one tracked unit.
  sectionTotal['assays'] = 1;
  if (countSelectedAssays(state.assays) > 0) {
    answered++;
    sectionAnswered['assays'] = 1;
  }
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, section: 'assays',    title: 'Assays' },
...
function validateForm() {
  ...
  // At least one assay must be selected.
  if (countSelectedAssays(state.assays) === 0) {
    errors.push({ id: 'assays-fieldset', message: 'Select at least one assay' });
    const el = document.getElementById('assays-error');
    if (el) el.textContent = 'Select at least one assay.';
  } else {
    const el = document.getElementById('assays-error');
    if (el) el.textContent = '';
  }
...
function selectedAssayLabels() {
  return ASSAYS.filter((a) => state.assays[a.field] === true).map((a) => a.label);
}
...
function renderReport() {
  ...
  const assays = selectedAssayLabels();
  const assaysText = assays.length ? assays.join(', ') : '—';

  const timingValue = timingBand
    ? `<span class="band-badge timing-${esc(timingBand)}">${esc(titleCase(timingBand))}</span>`
  ...
  out.innerHTML = `
    <h2>Toxicology Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · ${esc(assays.length)} assay${assays.length === 1 ? '' : 's'} · Indication: ${esc(indicationLabel(state.clinical.primaryIndication) || '—')}</p>
    <p class="muted">Assays: ${esc(assaysText)}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Ingestion timing</span>
        <span class="axis-value">${timingValue}</span>
      </div>
      <div class="axis-card">
