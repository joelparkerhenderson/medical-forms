...
use crate::models::_entities::geriatrics_waiting_list_card_appointments::{ActiveModel, Entity, Model};

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Params {
    pub deleted_at: Option<DateTimeWithTimeZone>,
    pub geriatrics_waiting_list_card_id: i64,
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/geriatrics_waiting_list_card_appointments/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
