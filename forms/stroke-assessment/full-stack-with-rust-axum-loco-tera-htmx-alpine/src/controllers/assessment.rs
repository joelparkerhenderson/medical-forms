...
use crate::engine::{stroke_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "nihss_assessment" => "nihssAssessment",
        "stroke_classification" => "strokeClassification",
        "risk_factors" => "riskFactors",
        "investigations" => "investigations",
        "acute_treatment" => "acuteTreatment",
...
async fn show_report(
    ...
    // Run grading engine
    let (severity_level, nihss_total, fired_rules) =
        stroke_grader::calculate_severity(&assessment_data);
    ...
    context.insert("nihss_total", &nihss_total);
