...
const STORAGE_KEY =
  'electroencephalogram-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
const INDICATION_OPTIONS = [
  { value: 'suspected-epilepsy', label: 'Suspected epilepsy' },
  { value: 'seizure-classification', label: 'Seizure classification' },
  { value: 'status-epilepticus', label: 'Status epilepticus' },
  { value: 'encephalopathy', label: 'Encephalopathy' },
  { value: 'first-seizure', label: 'First seizure' },
  { value: 'funny-turns', label: 'Funny turns' },
  { value: 'dementia', label: 'Dementia' },
  { value: 'pre-surgical-evaluation', label: 'Pre-surgical evaluation' },
...
function renderStep1() {
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'clinical-physiologist', label: 'Clinical physiologist' },
      { value: 'other', label: 'Other' }
    ]
  }));
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'EEG type, indication, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Requested EEG type', section: 'request', field: 'eegType',
    options: EEG_TYPE_OPTIONS, required: true
  }));
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Support diagnosis and classify seizure type after a clinically suspected seizure.'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Seizure / epilepsy context',
    description: 'Seizure frequency, epilepsy status, and current antiepileptic therapy.'
  });
  card.appendChild(textInput({ label: 'Seizure frequency', section: 'context', field: 'seizureFrequency', placeholder: 'e.g. 2 per week' }));
  card.appendChild(textArea({ label: 'Current antiepileptic drugs', section: 'context', field: 'currentAntiepileptics', rows: 2, placeholder: 'e.g. Levetiracetam 500mg BD' }));
  card.appendChild(subHead('Epilepsy status'));
  card.appendChild(boolField({ label: 'First unprovoked seizure', section: 'context', field: 'firstSeizure' }));
  card.appendChild(boolField({ label: 'Known / established epilepsy', section: 'context', field: 'knownEpilepsy' }));
  return card;
}
...
function renderStep5() {
  ...
  card.appendChild(boolField({ label: 'Recent seizure', section: 'redFlags', field: 'recentSeizure' }));
  card.appendChild(boolField({ label: 'Suspected status epilepticus', section: 'redFlags', field: 'suspectedStatusEpilepticus' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Triage',
    description: 'Requested urgency, requested-by date, and setting.'
  });
  card.appendChild(grid('two-col', [
...
const TRACKED_FIELDS = [
  ...
  // 3 Request
  ['request', 'eegType'], ['request', 'primaryIndication'],
  ['request', 'clinicalQuestion'],
  // 4 Context
  ['context', 'seizureFrequency'], ['context', 'currentAntiepileptics'],
  // 6 Triage
...
function updateStepListStatuses(sectionAnswered, sectionTotal) {
  ...
  for (const def of STEP_DEFINITIONS) {
    ...
    // Steps with no tracked text fields (red flags / review) are informational.
    if (t > 0 && a === t) {
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>EEG Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · EEG: ${esc(eegTypeLabel(state.request.eegType) || '—')}</p>
