...
Generate Loco (loco.rs) scaffolding for the `legal-requirements-privacy-notice` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name legal-requirements-privacy-notice --db postgres --bg async --assets serverside
   ```
