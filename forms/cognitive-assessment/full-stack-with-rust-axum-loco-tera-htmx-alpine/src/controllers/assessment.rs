...
use crate::engine::{cognitive_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "registration_attention" => "registrationAttention",
        "recall" => "recall",
        "language" => "language",
...
async fn show_report(
    ...
    // Run grading engine
    let (impairment_level, mmse_score, moca_score, fired_rules) =
        cognitive_grader::calculate_cognitive_status(&assessment_data);
    ...
    let decline_rate = crate::engine::utils::decline_rate_label(
        &assessment_data.cognitive_history.rate_of_decline,
    );
    ...
    context.insert("mmse_score", &mmse_score);
    context.insert("moca_score", &moca_score);
    context.insert("decline_rate", &decline_rate);
