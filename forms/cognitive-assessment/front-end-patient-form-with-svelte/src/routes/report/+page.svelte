<script lang="ts">
	...
	import { mmseScoreLabel, mmseScoreColor, calculateAge, educationLabel, adlLabel } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `cognitive-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">MMSE Score Breakdown</h2>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				...
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					...
					<div>
						...
						{data.demographics.educationLevel ? educationLabel(data.demographics.educationLevel) : 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Referral Source:</span>
						{data.referralInformation.referralSource || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Referral Reason:</span>
						{data.referralInformation.referralReason || 'N/A'}
					</div>
				</div>
			</div>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				<h2 class="mb-4 text-lg font-bold text-gray-900">Functional History</h2>
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					<div>
						<span class="font-medium text-gray-600">Living Arrangement:</span>
						{data.functionalHistory.livingArrangement || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Carers Available:</span>
						{data.functionalHistory.carersAvailable || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Bathing:</span>
						{adlLabel(data.functionalHistory.adlBathing) || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Dressing:</span>
						{adlLabel(data.functionalHistory.adlDressing) || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Meals:</span>
						{adlLabel(data.functionalHistory.adlMeals) || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Medications:</span>
						{adlLabel(data.functionalHistory.adlMedications) || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Finances:</span>
						{adlLabel(data.functionalHistory.adlFinances) || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Transport:</span>
						{adlLabel(data.functionalHistory.adlTransport) || 'N/A'}
					</div>
				</div>
				{#if data.functionalHistory.safetyConerns}
					<div class="mt-4 rounded-lg border border-red-200 bg-red-50 p-3">
						<span class="font-medium text-red-800">Safety Concerns:</span>
						<p class="text-sm text-red-700">{data.functionalHistory.safetyConerns}</p>
					</div>
				{/if}
			</div>
		</main>
