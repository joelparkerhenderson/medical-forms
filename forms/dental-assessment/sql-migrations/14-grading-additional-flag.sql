...
-- Additional safety-critical flags detected during dental grading.
...
COMMENT ON TABLE grading_additional_flag IS
    'Additional safety-critical flags detected during dental assessment grading (e.g. bisphosphonate use, bleeding disorder, oral pathology suspicion).';
...
COMMENT ON COLUMN grading_additional_flag.flag_id IS
    'Identifier of the flag (e.g. FLAG-BISPHOS-001, FLAG-BLEED-001).';
COMMENT ON COLUMN grading_additional_flag.category IS
    'Category of the flag (e.g. Medication Risk, Infection Control, Oral Pathology).';
