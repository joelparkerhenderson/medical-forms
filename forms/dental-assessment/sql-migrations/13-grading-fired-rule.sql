...
-- Individual DMFT and periodontal rules that fired during grading.
...
COMMENT ON TABLE grading_fired_rule IS
    'Individual dental assessment rules that evaluated to true during DMFT grading.';
...
COMMENT ON COLUMN grading_fired_rule.rule_id IS
    'Identifier of the rule that fired (e.g. DMFT-001, PERIO-002).';
COMMENT ON COLUMN grading_fired_rule.category IS
    'Category of the rule (e.g. DMFT, Periodontal, Caries, Oral Pathology).';
...
COMMENT ON COLUMN grading_fired_rule.severity_level IS
    'Severity level contributed by this rule.';
