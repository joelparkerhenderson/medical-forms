...
use crate::engine::{dental_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "caries_assessment" => "cariesAssessment",
        "occlusion_tmj" => "occlusionTmj",
...
async fn show_report(
    ...
    // Run grading engine
    let (oral_health_status, dmft_score, fired_rules) =
        dental_grader::calculate_oral_health(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        oral_health_status: oral_health_status.clone(),
        dmft_score,
    ...
    context.insert("oral_health_status", &oral_health_status);
    context.insert("oral_health_label", &crate::engine::utils::oral_health_status_label(&oral_health_status));
    context.insert("dmft_score", &dmft_score);
