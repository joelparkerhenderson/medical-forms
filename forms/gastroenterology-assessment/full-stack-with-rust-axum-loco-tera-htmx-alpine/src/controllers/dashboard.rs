...
struct DashboardParams {
    ...
    endoscopy_needed: Option<String>,
}
...
async fn dashboard(
    ...
    // Endoscopy needed filter
    if let Some(ref endo) = params.endoscopy_needed {
        if !endo.is_empty() {
            items.retain(|row| row.endoscopy_needed == *endo);
        }
    }
    ...
    context.insert("endoscopy_needed", &params.endoscopy_needed.unwrap_or_default());
