...
use crate::engine::{gi_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "upper_gi_symptoms" => "upperGiSymptoms",
        "lower_gi_symptoms" => "lowerGiSymptoms",
        "alarm_features" => "alarmFeatures",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "patientName"
            ...
            | "primaryDiagnosis"
            | "visitType"
            | "previousGiConditions"
            | "familyCancerHistory"
            | "familyCancerDetails"
            | "familyIbdHistory"
            | "previousEndoscopy"
            | "previousEndoscopyDate"
            | "previousEndoscopyFindings"
            | "surgicalHistory"
            | "odynophagia"
            | "bowelHabitChange"
            | "stoolFrequency"
...
async fn show_report(
    ...
    let (severity_level, severity_score, fired_rules) =
        gi_grader::calculate_severity(&assessment_data);
    ...
    let alarm_count = crate::engine::utils::count_alarm_features(&assessment_data);
    ...
    context.insert("alarm_count", &alarm_count);
