...
import { severityLabel, bmiCategory, calculateAge } from '$lib/engine/utils';
...
export function buildPdfDocument(
	...
	return {
		...
		header: {
			text: 'GASTROENTEROLOGY ASSESSMENT REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		content: [
			...
			{
				text: `GI Severity Score: ${result.severityScore}`,
				fontSize: 24,
				bold: true,
				alignment: 'center',
				margin: [0, 0, 0, 4]
			...
			sectionHeader('Patient Details'),
			{
				table: {
					...
					body: [
						...
						[
							field('Primary Symptom', data.chiefComplaint.primarySymptom || 'N/A'),
							field('Severity (0-10)', data.chiefComplaint.severityScore !== null ? String(data.chiefComplaint.severityScore) : 'N/A')
						]
					]
				},
			...
			...(result.additionalFlags.length > 0
				? [
						sectionHeader('Flagged Issues for Gastroenterologist'),
						{
			...
			...(result.firedRules.length > 0
				? [
						sectionHeader('Severity Score Breakdown'),
						{
							table: {
								headerRows: 1,
								widths: [60, 80, '*', 40],
								body: [
									...
									...result.firedRules.map((r) => [
										...
										{ text: `+${r.points}`, fontSize: 9, bold: true }
									])
								]
							},
							layout: 'lightHorizontalLines',
			...
			// Drug Allergies
			...(data.allergiesDiet.drugAllergies.length > 0
				? [
						sectionHeader('Drug Allergies'),
						{
							ul: data.allergiesDiet.drugAllergies.map(
								(a) =>
