<script lang="ts">
	...
	import { severityLabel, severityColor, bmiCategory, calculateAge, bristolStoolDescription } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `gi-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
	...
	<div class="min-h-screen bg-gray-50">
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			...
			<div class="mb-6 rounded-xl border-2 p-6 text-center {severityColor(result.severityLevel)}">
				<div class="text-3xl font-bold">Score: {result.severityScore}</div>
				<div class="mt-1 text-lg">{severityLabel(result.severityLevel)}</div>
			...
			{#if result.additionalFlags.length > 0}
				<div class="mb-6 rounded-xl border border-red-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-red-800">Flagged Issues for Gastroenterologist</h2>
					<div class="space-y-2">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">Severity Score Breakdown</h2>
						...
						<tbody>
							{#each result.firedRules as rule}
								<tr class="border-b border-gray-100">
									...
									<td class="py-2 font-bold">+{rule.points}</td>
								</tr>
							{/each}
						</tbody>
					</table>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				...
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					...
					{#if data.chiefComplaint.primarySymptom}
						<div class="sm:col-span-2">
							<span class="font-medium text-gray-600">Primary Symptom:</span>
							{data.chiefComplaint.primarySymptom}
						</div>
					{/if}
				</div>
			</div>
			...
			{#if data.allergiesDiet.drugAllergies.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					...
					<ul class="list-disc space-y-1 pl-5 text-sm">
						{#each data.allergiesDiet.drugAllergies as allergy}
							<li>
								<strong>{allergy.allergen}</strong> - {allergy.reaction}
								{#if allergy.severity}
									<span class="ml-1 rounded px-1.5 py-0.5 text-xs {allergy.severity === 'severe' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}">
										{allergy.severity}
									</span>
								{/if}
							</li>
			...
			{#if data.currentMedications.otherMedications.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">Other Medications</h2>
					<ul class="list-disc space-y-1 pl-5 text-sm">
