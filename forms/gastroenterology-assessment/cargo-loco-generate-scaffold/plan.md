...
Generate Loco (loco.rs) scaffolding for the `gastroenterology-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name gastroenterology-assessment --db postgres --bg async --assets serverside
   ```
