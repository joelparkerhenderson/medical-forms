...
-- Individual GI severity and red flag rules that fired during grading.
...
COMMENT ON TABLE grading_fired_rule IS
    'Individual gastroenterology assessment rules that evaluated to true during GI severity grading.';
...
COMMENT ON COLUMN grading_fired_rule.rule_id IS
    'Identifier of the rule that fired (e.g. GI-001, UGI-002, LGI-003).';
COMMENT ON COLUMN grading_fired_rule.category IS
    'Category of the rule (e.g. Upper GI, Lower GI, Hepatobiliary, Red Flag, Abdominal Pain).';
COMMENT ON COLUMN grading_fired_rule.description IS
    'Human-readable description of the condition or risk detected.';
