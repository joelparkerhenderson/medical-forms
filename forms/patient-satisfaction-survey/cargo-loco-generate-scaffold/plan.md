...
Generate Loco (loco.rs) scaffolding for the `patient-satisfaction-survey` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name patient-satisfaction-survey --db postgres --bg async --assets serverside
   ```
