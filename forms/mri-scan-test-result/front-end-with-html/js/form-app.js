...
// MRI Scan Test Result — reporting-clinician wizard
...
// inline structured MRI report. State is persisted to localStorage so a
...
const STORAGE_KEY = 'mri-scan-test-result.front-end-with-html.v1';
 ...
 *
 * @returns {import('./types.js').MriScanResult}
 */
...
/** @param {import('./types.js').MriScanResult} state */
...
/** @type {import('./types.js').MriScanResult} */
let state = loadState();
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The performed examination, contrast, pulse sequences, and adequacy.'
  });
  ...
  card.appendChild(textArea({
    ...
    field: 'sequencesPerformed', rows: 3,
    placeholder: 'Pulse sequences and protocol performed (e.g. T1, T2, FLAIR, DWI, DCE), with any deviations…'
  }));
...
function renderStep4() {
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      ...
      { field: 'haemorrhage', label: 'Haemorrhage' },
      { field: 'infarct', label: 'Infarct' },
      { field: 'demyelination', label: 'Demyelination' },
      { field: 'discHerniation', label: 'Disc herniation' },
      { field: 'cordCompression', label: 'Cord / cauda-equina compression' },
      { field: 'infectionInflammation', label: 'Infection / inflammation' },
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'Cord / cauda-equina compression, a haemorrhage, or an infarct auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Measurements and category',
    description: 'Key measurement and the structured-reporting category.'
  });
  ...
  card.appendChild(numberInput({
    ...
    hint: 'Largest lesion long-axis size, for surveillance and categorisation. 30 mm or larger grades the severity major.'
  }));
  card.appendChild(textInput({
    label: 'Structured-reporting category',
    field: 'reportingCategory',
    placeholder: 'e.g. PI-RADS 4, BI-RADS 4, or Likert 3',
    hint: 'PI-RADS 1–5 for prostate MRI, BI-RADS 0–6 for breast MRI, or a Likert score.'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    ...
    'critical-finding-alert':
      state.cordCompression || state.haemorrhage || state.infarct,
...
const STEP_SLOTS = {
  ...
  2: [['bodyRegion'], ['contrastUsed'], ['examinationAdequacy'], ['sequencesPerformed']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['findingsNarrative'],
      ['massOrLesion', 'haemorrhage', 'infarct', 'demyelination', 'discHerniation',
       'cordCompression', 'infectionInflammation', 'incidentalFinding']],
  5: [['largestLesionSizeMm'], ['reportingCategory']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Structured-reporting category', state.reportingCategory || 'Not recorded'],
