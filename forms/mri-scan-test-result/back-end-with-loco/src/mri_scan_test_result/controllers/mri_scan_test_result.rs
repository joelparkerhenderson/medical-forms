...
use crate::models::_entities::mri_scan_test_results::{ActiveModel, Entity, Model};
...
pub struct Params {
    ...
    pub sequences_performed: String,
    ...
    pub infarct: bool,
    pub demyelination: bool,
    pub disc_herniation: bool,
    pub cord_compression: bool,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.sequences_performed = Set(self.sequences_performed.clone());
      ...
      item.mass_or_lesion = Set(self.mass_or_lesion);
      item.haemorrhage = Set(self.haemorrhage);
      item.infarct = Set(self.infarct);
      item.demyelination = Set(self.demyelination);
      item.disc_herniation = Set(self.disc_herniation);
      item.cord_compression = Set(self.cord_compression);
      item.infection_inflammation = Set(self.infection_inflammation);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/mri_scan_test_results/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
