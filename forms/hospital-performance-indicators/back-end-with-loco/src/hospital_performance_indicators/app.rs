...
impl Hooks for App {
    ...
    fn routes(_ctx: &AppContext) -> AppRoutes {
        AppRoutes::with_default_routes() // controller routes below
            ...
            .add_route(controllers::hospital_performance_indicator_value::routes())
            .add_route(controllers::hospital_performance_indicators::routes())
    ...
    async fn seed(ctx: &AppContext, _base: &Path) -> Result<()> {
        db::seed::<users::ActiveModel>(
            &ctx.db,
            &format!(
                "{}/src/hospital_performance_indicators/fixtures/users.yaml",
