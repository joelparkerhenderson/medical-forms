...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "patientName"
            ...
            | "carryingDistance"
            | "pushingPulling"
            | "teamLiftingAvailable"
            | "manualHandlingTraining"
            | "mechanicalAidsAvailable"
            | "screenFlickerFree"
            | "screenBrightnessAdjustable"
            | "screenGlareFree"
            | "keyboardSeparate"
            | "keyboardTiltable"
            | "mouseComfortable"
...
async fn show_report(
    ...
    let symptom_count = crate::engine::utils::count_pain_sites(&assessment_data);
    let dse_compliance = crate::engine::utils::dse_compliance_percentage(&assessment_data);
    ...
    context.insert("symptom_count", &symptom_count);
    context.insert("dse_compliance", &dse_compliance);
