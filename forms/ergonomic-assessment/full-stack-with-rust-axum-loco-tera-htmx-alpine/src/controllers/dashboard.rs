...
async fn dashboard(
    ...
    // Adjustment needed filter
    if let Some(ref adj) = params.adjustment_needed {
        if !adj.is_empty() {
            items.retain(|row| row.adjustment_needed == *adj);
        }
    }
    ...
    context.insert("adjustment_needed", &params.adjustment_needed.unwrap_or_default());
