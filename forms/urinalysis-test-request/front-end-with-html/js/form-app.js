...
const STORAGE_KEY = 'urinalysis-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
/**
 * Render the requested-tests checkbox group. Each test is a Lily
 * `.checkbox-input` inside a `.checkbox-group` (role="group"). A live count of
 * selected tests sits above the group.
 */
function testCheckboxGroup() {
  ...
  legend.innerHTML = 'Requested tests <span class="req" aria-hidden="true">*</span>';
...
const INDICATION_OPTIONS = [
  { value: 'suspected-uti', label: 'Suspected UTI' },
  ...
  { value: 'pregnancy-screen', label: 'Pregnancy screen' },
  { value: 'pre-operative', label: 'Pre-operative' },
  { value: 'catheter-related', label: 'Catheter-related' },
  { value: 'suspected-malignancy', label: 'Suspected malignancy' },
  { value: 'drug-monitoring', label: 'Drug monitoring' },
...
function renderStep1() {
  const card = sectionCard({
    ...
    description: 'Identify the clinician making this request.'
  });
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Tick every urine test to order. At least one is required.'
  });
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Indication and clinical details — the highest-value fields that drive every axis.'
  });
  card.appendChild(selectInput({
    label: 'Primary indication', section: 'context', field: 'primaryIndication',
    options: INDICATION_OPTIONS, required: true
  }));
  card.appendChild(textArea({
    label: 'Clinical details', section: 'context', field: 'clinicalDetails',
    rows: 3, required: true,
    placeholder: 'e.g. 72F, recurrent lower-UTI symptoms, two prior treatment failures.'
  }));
  card.appendChild(subHead('Modifiers'));
  card.appendChild(boolField({ label: 'Pregnant', section: 'context', field: 'pregnant' }));
  card.appendChild(boolField({ label: 'Catheterised', section: 'context', field: 'catheterised' }));
  card.appendChild(boolField({ label: 'Currently on antibiotics', section: 'context', field: 'currentAntibiotics' }));
  return card;
}
...
function renderStep5() {
  ...
  card.appendChild(boolField({ label: 'Frequency / urgency', section: 'symptoms', field: 'symptomFrequency' }));
  card.appendChild(boolField({ label: 'Visible (macroscopic) haematuria', section: 'symptoms', field: 'symptomVisibleHaematuria' }));
  card.appendChild(boolField({ label: 'Loin / flank pain', section: 'symptoms', field: 'symptomLoinPain' }));
  card.appendChild(boolField({ label: 'Fever / systemic symptoms', section: 'symptoms', field: 'symptomFever' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Specimen',
    description: 'Specimen type and collection — drives the preanalytical axis.'
  });
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Specimen type', section: 'specimen', field: 'specimenType', options: SPECIMEN_TYPE_OPTIONS }),
    selectInput({ label: 'Specimen collected', section: 'specimen', field: 'specimenCollected', options: COLLECTED_OPTIONS })
  ]));
  card.appendChild(textInput({ label: 'Collection date & time', section: 'specimen', field: 'collectionDatetime', type: 'datetime-local' }));
  return card;
}
...
const TRACKED_FIELDS = [
  ...
  // 4 Context
  ['context', 'primaryIndication'], ['context', 'clinicalDetails'],
  // 6 Specimen
  ['specimen', 'specimenType'], ['specimen', 'specimenCollected'],
  // 7 Triage
  ['triage', 'urgency'], ['triage', 'setting']
];

const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'tests', 4: 'context',
  5: 'symptoms', 6: 'specimen', 7: 'triage'
};
...
function updateProgress() {
  ...
  // Tests step: count as one tracked unit, answered when >=1 selected.
...
const STEP_DEFINITIONS = [
  ...
  { step: 4, section: 'context',   title: 'Context' },
  { step: 5, section: 'symptoms',  title: 'Symptoms' },
  { step: 6, section: 'specimen',  title: 'Specimen' },
...
function validateForm() {
  ...
  if (countSelectedTests(state.tests) === 0) {
    errors.push({ id: 'tests-fieldset', message: 'Select at least one test' });
    const el = document.getElementById('tests-error');
    if (el) el.textContent = 'Select at least one test.';
  } else {
    const el = document.getElementById('tests-error');
    if (el) el.textContent = '';
  }
...
function selectedTestsLabel() {
  const fields = Object.keys(state.tests).filter((k) => state.tests[k] === true);
  if (fields.length === 0) return '—';
  return fields.map((f) => testLabel(f)).join(', ');
}
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>Urinalysis Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Tests: ${esc(selectedTestsLabel())}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Preanalytical specimen</span>
        <span class="axis-value"><span class="band-badge preanalytical-${esc(preanalyticalBand)}">${esc(titleCase(preanalyticalBand))}</span></span>
        ${fastingOrSpecimen ? `<p class="muted">${esc(fastingOrSpecimen)}</p>` : ''}
      </div>
      <div class="axis-card">
