...
use crate::models::_entities::urinalysis_test_request_grades::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.urinalysis_test_request_id = Set(self.urinalysis_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.preanalytical_band = Set(self.preanalytical_band.clone());
      item.fasting_or_specimen = Set(self.fasting_or_specimen.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/urinalysis_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
