...
-- Additional safety-critical flags detected during first responder assessment grading.
...
COMMENT ON TABLE grading_additional_flag IS
    'Additional safety-critical flags detected during first responder assessment grading (e.g. expired registration, failed PTSD screen, unfit for driving).';
...
COMMENT ON COLUMN grading_additional_flag.flag_id IS
    'Identifier of the flag (e.g. FLAG-REG-001, FLAG-PTSD-001).';
COMMENT ON COLUMN grading_additional_flag.category IS
    'Category of the flag (e.g. Registration, Psychological, Physical, Training).';
