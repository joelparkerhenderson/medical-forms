# First Responder Assessment

First responder fitness and competency evaluation for paramedics, EMTs, and first aiders. Covers physical fitness, clinical skills, equipment competency, and psychological readiness.

## Scoring system

- **Instrument**: First Responder Competency Framework
- **Competency Levels**: Not Competent (1), Developing (2), Competent (3), Expert (4)
- **Overall Fitness Decisions**:
  - Fit for duty: No restrictions, all competencies met
  - Fit with restrictions: Some limitations, remedial actions required
  - Temporarily unfit: Significant gaps, re-assessment required within defined period
  - Permanently unfit: Unable to meet minimum competency requirements

## Steps

| #   | Step                           |
| --- | ------------------------------ |
| 1   | Demographics                   |
| 2   | Role & Qualifications          |
| 3   | Physical Fitness Assessment    |
| 4   | Clinical Skills Competency     |
| 5   | Equipment & Vehicle Competency |
| 6   | Communication Skills           |
| 7   | Psychological Readiness        |
| 8   | Occupational Health            |
| 9   | CPD & Training Record          |
| 10  | Overall Fitness Decision       |

## Directory structure

```
first-responder-assessment/
  front-end-patient-form-with-svelte/
  front-end-clinician-dashboard-with-svelte/
```

## Technology

See [root index.md](../index.md) for technology stacks.
