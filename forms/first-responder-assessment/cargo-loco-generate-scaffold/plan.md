...
Generate Loco (loco.rs) scaffolding for the `first-responder-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name first-responder-assessment --db postgres --bg async --assets serverside
   ```
