<script lang="ts">
	...
	import { gafScoreLabel, gafScoreColor, calculateAge, riskLevelLabel, riskLevelColor } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `psychiatry-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			...
			{#if data.riskAssessment.violenceRisk && data.riskAssessment.violenceRisk !== 'none'}
				<div class="mb-6 rounded-xl border-2 p-4 {riskLevelColor(data.riskAssessment.violenceRisk)}">
					<div class="text-center font-bold">Violence Risk: {riskLevelLabel(data.riskAssessment.violenceRisk)}</div>
				</div>
			{/if}
			...
			{#if result.additionalFlags.length > 0}
				<div class="mb-6 rounded-xl border border-red-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-red-800">Flagged Issues for Psychiatrist</h2>
					<div class="space-y-2">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">GAF Score Justification</h2>
						...
						<tbody>
							{#each result.firedRules as rule}
								<tr class="border-b border-gray-100">
									...
									<td class="py-2 font-bold text-red-600">-{rule.scoreImpact}</td>
								</tr>
							{/each}
						</tbody>
					</table>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				...
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					...
					<div>
						<span class="font-medium text-gray-600">Legal Status:</span>
						{data.demographics.legalStatus}
					</div>
