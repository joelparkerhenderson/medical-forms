...
use crate::engine::{hematology_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "blood_count_analysis" => "bloodCountAnalysis",
        "coagulation_studies" => "coagulationStudies",
        "peripheral_blood_film" => "peripheralBloodFilm",
        "iron_studies" => "ironStudies",
        "hemoglobinopathy_screening" => "hemoglobinopathyScreening",
        "bone_marrow_assessment" => "boneMarrowAssessment",
        "transfusion_history" => "transfusionHistory",
        "treatment_medications" => "treatmentMedications",
...
async fn show_report(
    ...
    // Run grading engine
    let (abnormality_level, abnormality_score, fired_rules) =
        hematology_grader::calculate_abnormality(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        abnormality_level: abnormality_level.clone(),
        abnormality_score,
    ...
    context.insert("abnormality_level", &abnormality_level);
    context.insert("abnormality_label", &crate::engine::utils::abnormality_level_label(&abnormality_level));
    context.insert("abnormality_score", &abnormality_score);
