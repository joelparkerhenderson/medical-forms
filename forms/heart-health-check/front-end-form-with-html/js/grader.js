...
import { calculateBMI, calculateTcHdlRatio, smokingPoints, isLikelyDraft } from './utils.js';
import { evaluateRules } from './grading-rules.js';

/**
 * Estimate 10-year CVD risk from the assessment data.
 * Returns a percentage value clamped between 0.1 and 95.
 */
...
/**
 * Calculate heart age by finding the age at which a baseline-risk
 * individual matches the actual patient risk.
 */
export function calculateHeartAge(data, actualRisk) {
  ...
  for (let testAge = 25; testAge <= 100; testAge++) {
    const baseline = createDefaultAssessment();
...
/**
 * Calculate the full risk result including category, fired rules, etc.
 */
