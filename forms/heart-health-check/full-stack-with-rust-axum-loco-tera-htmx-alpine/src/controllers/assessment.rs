...
use crate::engine::{flagged_issues, risk_grader, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "demographics_ethnicity" => "demographicsEthnicity",
        "blood_pressure" => "bloodPressure",
        "cholesterol" => "cholesterol",
        "medical_conditions" => "medicalConditions",
        "family_history" => "familyHistory",
        "smoking_alcohol" => "smokingAlcohol",
        "physical_activity_diet" => "physicalActivityDiet",
        "body_measurements" => "bodyMeasurements",
        "review_calculate" => "reviewCalculate",
        other => other,
    }
}
...
/// POST /assessment/{id} -- save all fields, redirect to report
#[debug_handler]
...
async fn show_report(
    ...
    // Run grading engine
    let (risk_category, ten_year_risk, heart_age, fired_rules) =
    ...
    let grading_result = crate::engine::types::GradingResult {
        ...
        ten_year_risk_percent: ten_year_risk,
        heart_age,
    ...
    context.insert("ten_year_risk", &ten_year_risk);
    context.insert("heart_age", &heart_age);
    context.insert("actual_age", &assessment_data.demographics_ethnicity.age);
...
pub fn routes(tera: Arc<Tera>) -> Routes {
    Routes::new()
        ...
        .add("assessment/{id}", post(save_assessment))
