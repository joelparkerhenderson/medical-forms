...
use crate::models::_entities::bronchoscopy_test_results::{ActiveModel, Entity, Model};
...
pub struct Params {
    ...
    pub sedation_used: String,
    pub extent_examined: String,
    pub clinical_history: String,
    pub endobronchial_lesion: bool,
    pub mucosal_abnormality: bool,
    pub extrinsic_compression: bool,
    pub bleeding: bool,
    pub foreign_body: bool,
    pub secretions_purulent: bool,
    pub normal_examination: bool,
    pub lesion_location: String,
    pub samples_taken: String,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.sedation_used = Set(self.sedation_used.clone());
      item.extent_examined = Set(self.extent_examined.clone());
      item.clinical_history = Set(self.clinical_history.clone());
      item.endobronchial_lesion = Set(self.endobronchial_lesion);
      item.mucosal_abnormality = Set(self.mucosal_abnormality);
      item.extrinsic_compression = Set(self.extrinsic_compression);
      item.bleeding = Set(self.bleeding);
      item.foreign_body = Set(self.foreign_body);
      item.secretions_purulent = Set(self.secretions_purulent);
      item.normal_examination = Set(self.normal_examination);
      item.lesion_location = Set(self.lesion_location.clone());
      item.samples_taken = Set(self.samples_taken.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/bronchoscopy_test_results/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
