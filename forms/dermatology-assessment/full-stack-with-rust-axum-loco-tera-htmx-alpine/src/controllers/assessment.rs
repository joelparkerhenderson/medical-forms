...
use crate::engine::{dermatology_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "triggers_comorbidities" => "triggersComorbidities",
...
async fn show_report(
    ...
    // Run grading engine
    let (severity_level, dlqi_score, fired_rules) =
        dermatology_grader::calculate_severity(&assessment_data);
    ...
    let bsa_percent = crate::engine::utils::calculate_bsa_percent(&assessment_data);
    ...
    context.insert("dlqi_score", &dlqi_score);
    context.insert("bsa_percent", &bsa_percent);
