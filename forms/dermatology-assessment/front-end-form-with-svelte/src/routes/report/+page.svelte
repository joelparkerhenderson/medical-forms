<script lang="ts">
	...
	import { dlqiScoreLabel, dlqiScoreColor, calculateAge, fitzpatrickLabel } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `dermatology-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
...
{#if result}
	<div class="min-h-screen bg-gray-50">
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			<!-- DLQI Score Banner -->
			<div class="mb-6 rounded-xl border-2 p-6 text-center {dlqiScoreColor(result.dlqiScore)}">
				<div class="text-3xl font-bold">DLQI {result.dlqiScore}/30</div>
				<div class="mt-1 text-lg">{result.dlqiCategory}</div>
			...
			{#if result.additionalFlags.length > 0}
				<div class="mb-6 rounded-xl border border-red-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-red-800">Flagged Issues for Dermatologist</h2>
					<div class="space-y-2">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">DLQI Score Breakdown</h2>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				...
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					...
					<div>
						<span class="font-medium text-gray-600">Skin Type:</span>
						{data.demographics.skinType ? fitzpatrickLabel(data.demographics.skinType) : 'N/A'}
					</div>
					...
					<div>
						...
						{data.chiefComplaint.location}
					</div>
				</div>
			</div>

			<!-- Medications -->
			{#if data.currentMedications.topicals.length > 0 || data.currentMedications.systemics.length > 0 || data.currentMedications.biologics.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">Current Medications</h2>
					{#if data.currentMedications.topicals.length > 0}
						<h3 class="text-sm font-semibold text-gray-600">Topicals</h3>
						<ul class="list-disc space-y-1 pl-5 text-sm mb-3">
							{#each data.currentMedications.topicals as med}
								<li>{med.name} {med.dose} {med.frequency}</li>
							{/each}
						</ul>
					{/if}
					{#if data.currentMedications.systemics.length > 0}
						<h3 class="text-sm font-semibold text-gray-600">Systemics</h3>
						<ul class="list-disc space-y-1 pl-5 text-sm mb-3">
							{#each data.currentMedications.systemics as med}
								<li>{med.name} {med.dose} {med.frequency}</li>
							{/each}
						</ul>
					{/if}
					{#if data.currentMedications.biologics.length > 0}
						<h3 class="text-sm font-semibold text-gray-600">Biologics</h3>
