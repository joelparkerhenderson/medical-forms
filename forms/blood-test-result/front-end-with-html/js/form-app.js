...
// inline structured pathology report. State is persisted to localStorage so
...
const STORAGE_KEY = 'blood-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').BloodTestResult} */
...
function mergeIntoDefaults(parsed) {
  ...
  // Analyte fields default to null (not measured) and accept a stored
...
/** @param {import('./types.js').BloodTestResult} state */
...
/** @type {import('./types.js').BloodTestResult} */
let state = loadState();
...
/**
 * Numeric analyte input bound to a nullable-number field: an empty box maps
 * to `null` (the analyte was not measured), anything else to a number.
 */
...
/** A small in-card sub-heading dividing a step into analyte panels. */
...
function renderStep1() {
  ...
  card.appendChild(textInput({
    ...
    placeholder: 'e.g. Dr A Pathologist'
  }));
  card.appendChild(textInput({
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The specimen analysed and its condition / quality.'
  });
  ...
  card.appendChild(conditionalAlert({
    id: 'specimen-warning',
    type: 'warning',
    heading: 'Specimen quality may affect results',
    message: 'The specimen condition may reduce result reliability and will raise an inadequate-specimen safety flag. Consider repeating the test on a fresh specimen.'
  }));
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'The clinical question and any comparison with previous results.'
  });
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'Relevant previous results and changes since (trend / delta)…'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Result values',
    description: 'The quantitative analyte values the laboratory measured, grouped by panel. Leave an analyte blank when it was not measured.'
  });

  // Full blood count (FBC)
  card.appendChild(panelHeading('Full blood count (FBC)'));
  card.appendChild(numberInput({ label: 'Haemoglobin (g/L)', field: 'haemoglobinGL', min: 0, max: 300, step: 0.1 }));
  card.appendChild(numberInput({ label: 'White cell count (×10⁹/L)', field: 'whiteCellCount', min: 0, max: 500, step: 0.01 }));
  card.appendChild(numberInput({ label: 'Platelets (×10⁹/L)', field: 'platelets', min: 0, max: 5000, step: 1 }));
  card.appendChild(numberInput({ label: 'Neutrophils (×10⁹/L)', field: 'neutrophils', min: 0, max: 500, step: 0.01 }));

  // Urea & electrolytes / renal (U&E)
  card.appendChild(panelHeading('Urea & electrolytes / renal (U&E)'));
  card.appendChild(numberInput({ label: 'Sodium (mmol/L)', field: 'sodiumMmolL', min: 0, max: 250, step: 0.1 }));
  card.appendChild(numberInput({ label: 'Potassium (mmol/L)', field: 'potassiumMmolL', min: 0, max: 20, step: 0.1 }));
  card.appendChild(numberInput({ label: 'Urea (mmol/L)', field: 'ureaMmolL', min: 0, max: 200, step: 0.1 }));
  card.appendChild(numberInput({ label: 'Creatinine (µmol/L)', field: 'creatinineUmolL', min: 0, max: 5000, step: 1 }));
  card.appendChild(numberInput({
    label: 'eGFR (mL/min/1.73m²)', field: 'egfr', min: 0, max: 200, step: 1,
    hint: 'An eGFR derives a structured CKD staging band (NICE G1–G5); below 30 raises an urgent-referral flag.'
  }));

  // Liver function (LFT)
  card.appendChild(panelHeading('Liver function (LFT)'));
  card.appendChild(numberInput({ label: 'ALT (U/L)', field: 'altUL', min: 0, max: 20000, step: 1 }));
  card.appendChild(numberInput({ label: 'Alkaline phosphatase (U/L)', field: 'alkalinePhosphatase', min: 0, max: 20000, step: 1 }));
  card.appendChild(numberInput({ label: 'Bilirubin (µmol/L)', field: 'bilirubinUmolL', min: 0, max: 2000, step: 1 }));
  card.appendChild(numberInput({ label: 'Albumin (g/L)', field: 'albuminGL', min: 0, max: 100, step: 1 }));

  // Inflammation, glycaemic, endocrine, haematinics, coagulation
  card.appendChild(panelHeading('Inflammation, glycaemic, endocrine, haematinics, coagulation'));
  card.appendChild(numberInput({ label: 'C-reactive protein (mg/L)', field: 'cReactiveProtein', min: 0, max: 1000, step: 0.1 }));
  card.appendChild(numberInput({
    label: 'HbA1c (mmol/mol)', field: 'hba1cMmolMol', min: 0, max: 250, step: 1,
    hint: 'An HbA1c derives a structured glycaemic band (WHO/IFCC: normal < 42, prediabetes 42–47, diabetes ≥ 48).'
  }));
  card.appendChild(numberInput({ label: 'Glucose (mmol/L)', field: 'glucoseMmolL', min: 0, max: 100, step: 0.1 }));
  card.appendChild(numberInput({ label: 'TSH (mU/L)', field: 'tsh', min: 0, max: 1000, step: 0.01 }));
  card.appendChild(numberInput({ label: 'Ferritin (µg/L)', field: 'ferritin', min: 0, max: 100000, step: 1 }));
  card.appendChild(numberInput({ label: 'INR (ratio)', field: 'inr', min: 0, max: 30, step: 0.1 }));
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'The overall result status with abnormal- and critical-value flags, and the findings narrative.'
  });
  ...
  card.appendChild(checkboxGroup({
    label: 'Result flags',
    items: [
      { field: 'abnormalResultsPresent', label: 'Abnormal results present' },
      { field: 'criticalValuePresent', label: 'Critical (panic) value present' }
    ]
  }));
  ...
  card.appendChild(conditionalAlert({
    ...
    type: 'error',
    heading: 'Critical (panic) value selected',
    message: 'A critical (panic) value auto-escalates the result classification to critical and the follow-up urgency to a critical alert. Record the detail below and ensure the result is communicated to the requester on sign-off.'
  }));
...
function renderStep6() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Structured-reporting category label, e.g. an eGFR CKD stage or a glycaemic band.',
    placeholder: 'e.g. eGFR CKD G3a, HbA1c diabetes range'
  }));
  card.appendChild(textArea({
...
function renderStep7() {
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'This report contains a critical (panic) value. Communicate the result directly to the requester and record it below before signing.'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    ...
    'critical-value-alert':
      state.criticalValuePresent || state.overallResultStatus === 'critical',
...
// counts only when ticked; a nullable analyte counts when non-null). This
// lets the analyte panels count as answered once any value in the panel is
// recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['specimenType'], ['specimenCondition']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['haemoglobinGL', 'whiteCellCount', 'platelets', 'neutrophils'],
      ['sodiumMmolL', 'potassiumMmolL', 'ureaMmolL', 'creatinineUmolL', 'egfr'],
      ['altUL', 'alkalinePhosphatase', 'bilirubinUmolL', 'albuminGL'],
      ['cReactiveProtein', 'hba1cMmolMol', 'glucoseMmolL', 'tsh', 'ferritin', 'inr']],
  5: [['overallResultStatus'],
      ['abnormalResultsPresent', 'criticalValuePresent', 'criticalValueDetail'],
      ['findingsNarrative']],
...
function renderReport() {
  ...
  const measured = analyteValues(state).filter((a) => a.value !== null);
  const analyteTable = measured.length === 0
    ? `<p class="muted">No analyte result values were recorded.</p>`
    : `
        ...
        <tbody>
          ${measured.map((a) => `
            <tr>
              ...
              <td>${esc(a.panel)}</td>
              <td>${esc(String(a.value))}&nbsp;${esc(a.units)}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
  ...
  const flagsList = g.flags.length === 0
    ? `<p class="empty-message">No safety flags raised.</p>`
    : `
      <ul class="flags">
        ${g.flags.map((f) => `
  ...
  out.innerHTML = `
    <div class="report-card">
      ...
      <h3>Measured result values (${measured.length})</h3>
      ${analyteTable}
