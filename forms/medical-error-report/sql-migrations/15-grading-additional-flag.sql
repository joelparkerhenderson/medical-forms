...
-- Additional safety-critical flags detected during medical error report grading.
...
COMMENT ON TABLE grading_additional_flag IS
    'Additional safety-critical flags detected during medical error report grading (e.g. patient death, duty of candour, regulatory reporting required).';
...
COMMENT ON COLUMN grading_additional_flag.flag_id IS
    'Identifier of the flag (e.g. FLAG-DEATH-001, FLAG-DOC-001).';
COMMENT ON COLUMN grading_additional_flag.category IS
    'Category of the flag (e.g. Patient Safety, Regulatory, Duty of Candour).';
