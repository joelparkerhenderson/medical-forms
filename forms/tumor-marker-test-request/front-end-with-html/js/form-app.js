...
const STORAGE_KEY =
  'tumor-marker-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 5;
...
function setMarker(field, checked) {
  state.markers[field] = !!checked;
  saveState(state);
  updateProgress();
  updateMarkerCount();
}
...
/**
 * Render the requested-markers checkbox group. Each marker is a Lily
 * `.checkbox-input` inside a `.checkbox-group` (role="group"). A live count of
 * selected markers sits above the group. Mirrors the BOOLEAN columns in SQL
 * migration 04; all markers default to false.
 */
function markerCheckboxGroup() {
  const wrapper = document.createElement('fieldset');
  wrapper.className = 'field';
  wrapper.id = 'markers-fieldset';

  const legend = document.createElement('legend');
  legend.className = 'label';
  legend.innerHTML = 'Requested serum tumour markers <span class="req" aria-hidden="true">*</span>';
  wrapper.appendChild(legend);

  const hint = document.createElement('span');
  hint.className = 'hint';
  hint.id = 'markers-count';
  wrapper.appendChild(hint);

  const group = document.createElement('div');
  group.className = 'checkbox-group marker-grid';
  group.setAttribute('role', 'group');
  group.setAttribute('aria-labelledby', wrapper.id);

  for (const m of MARKERS) {
    const cbId = `markers-${m.field}`;
    const checked = state.markers[m.field] === true ? ' checked' : '';
    const label = document.createElement('label');
    label.htmlFor = cbId;
    label.innerHTML = `
      <input class="checkbox-input" type="checkbox" id="${cbId}" name="markers" value="${esc(m.field)}"${checked}>
      <span class="marker-text"><span class="marker-name">${esc(m.label)}</span><span class="marker-use">${esc(m.use)}</span></span>
    `;
    const input = label.querySelector('input');
    input.addEventListener('change', () => {
      setMarker(m.field, input.checked);
      clearFieldError('markers');
    });
  ...
  errSpan.id = 'markers-error';
  wrapper.appendChild(errSpan);
  return wrapper;
}

function updateMarkerCount() {
  const el = document.getElementById('markers-count');
  if (!el) return;
  const n = countSelectedMarkers(state.markers);
  el.textContent = n === 0
    ? 'No markers selected yet — select at least one.'
    : `${n} marker${n === 1 ? '' : 's'} selected.`;
}
...
const INDICATION_OPTIONS = [
  ...
  { value: 'cancer-monitoring', label: 'Cancer monitoring' },
  { value: 'treatment-response', label: 'Treatment response' },
  { value: 'recurrence-surveillance', label: 'Recurrence surveillance' },
  { value: 'screening-high-risk', label: 'Screening (high-risk)' },
  { value: 'characterise-mass', label: 'Characterise mass' },
...
function renderStep3() {
  const card = sectionCard({
    stepNumber: 3,
    title: 'Requested markers',
    description: 'Select one or more serum tumour markers. Marker-to-indication fit drives the appropriateness axis.'
  });
  card.appendChild(markerCheckboxGroup());
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Indication, clinical details, known cancer site, and prior marker value — the highest-value fields.'
  });
  card.appendChild(selectInput({
  ...
  card.appendChild(textInput({ label: 'Known / suspected cancer site', section: 'context', field: 'knownCancerSite' }));
  card.appendChild(boolField({ label: 'Patient is currently on cancer treatment', section: 'context', field: 'onTreatment' }));
  card.appendChild(grid('two-col', [
    textInput({ label: 'Previous marker value', section: 'context', field: 'previousMarkerValue', type: 'number', min: 0, step: 0.01 }),
    textInput({ label: 'Previous marker date', section: 'context', field: 'previousMarkerDate', type: 'date' })
  ]));
  return card;
}
...
function updateProgress() {
  ...
  // Markers count as one tracked item for the "markers" section.
  const markersAnswered = countSelectedMarkers(state.markers) > 0 ? 1 : 0;
  sectionTotal.markers = 1;
  sectionAnswered.markers = markersAnswered;

  const total = TRACKED_FIELDS.length + 1;
  answered += markersAnswered;
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, section: 'markers',   title: 'Markers' },
  { step: 4, section: 'context',   title: 'Clinical context' },
...
function validateForm() {
  ...
  // At least one marker must be selected.
  if (countSelectedMarkers(state.markers) === 0) {
    errors.push({ id: 'markers-fieldset', message: 'Select at least one tumour marker' });
    const err = document.getElementById('markers-error');
    if (err) err.textContent = 'Select at least one tumour marker';
  } else {
    clearFieldError('markers');
  }
...
function renderReport() {
  ...
  const markerNames = selectedMarkerFields(state.markers).map(markerLabel).join(', ') || '—';
  const indication = indicationLabel(state.context.primaryIndication) || '—';

  const interpValue = interpretationBand
    ? `<span class="band-badge interp-${esc(interpretationBand)}">${esc(titleCase(interpretationBand))}</span>`
  ...
  out.innerHTML = `
    <h2>Tumour-Marker Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Markers: ${esc(markerNames)} · Indication: ${esc(indication)}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Interpretation safety</span>
        <span class="axis-value">${interpValue}</span>
      </div>
      <div class="axis-card">
