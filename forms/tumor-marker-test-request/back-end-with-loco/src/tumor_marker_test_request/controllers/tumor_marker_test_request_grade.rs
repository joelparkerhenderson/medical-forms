...
use crate::models::_entities::tumor_marker_test_request_grades::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.tumor_marker_test_request_id = Set(self.tumor_marker_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.interpretation_band = Set(self.interpretation_band.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/tumor_marker_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
