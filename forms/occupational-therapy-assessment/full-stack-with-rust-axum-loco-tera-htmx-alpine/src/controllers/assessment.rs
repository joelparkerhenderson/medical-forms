...
use crate::engine::{ot_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "occupational_profile" => "occupationalProfile",
        "daily_living_activities" => "dailyLivingActivities",
        "instrumental_activities" => "instrumentalActivities",
        "cognitive_perceptual" => "cognitivePerceptual",
        "motor_sensory" => "motorSensory",
        "home_environment" => "homeEnvironment",
        "work_leisure" => "workLeisure",
...
async fn show_report(
    ...
    // Run grading engine
    let (function_level, function_score, fired_rules) =
        ot_grader::calculate_function_level(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        function_level: function_level.clone(),
        function_score,
    ...
    context.insert("function_level", &function_level);
    context.insert("function_label", &crate::engine::utils::function_level_label(&function_level));
    context.insert("function_score", &function_score);
