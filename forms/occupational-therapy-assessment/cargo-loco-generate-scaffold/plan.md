...
Generate Loco (loco.rs) scaffolding for the `occupational-therapy-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name occupational-therapy-assessment --db postgres --bg async --assets serverside
   ```
