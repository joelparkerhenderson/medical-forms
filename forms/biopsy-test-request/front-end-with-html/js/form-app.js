...
import { biopsySiteLabel, emptyRequest, indicationLabel } from './types.js';

// Biopsy Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY = 'biopsy-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 8;
...
const INDICATION_OPTIONS = [
  ...
  { value: 'suspected-infection', label: 'Suspected infection' },
  { value: 'inflammatory-disease', label: 'Inflammatory disease' },
  { value: 'transplant-monitoring', label: 'Transplant monitoring' },
  { value: 'lymphadenopathy', label: 'Lymphadenopathy' },
  { value: 'characterise-lesion', label: 'Characterise lesion' },
  { value: 'other', label: 'Other' }
];
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Biopsy site, method, and laterality — drives the appropriateness axis.'
  });
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Biopsy site', section: 'procedure', field: 'biopsySite', options: BIOPSY_SITE_OPTIONS, required: true }),
    selectInput({ label: 'Biopsy method', section: 'procedure', field: 'biopsyMethod', options: BIOPSY_METHOD_OPTIONS, required: true })
  ]));
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Laterality', section: 'procedure', field: 'laterality', options: LATERALITY_OPTIONS }),
    selectInput({ label: 'Care setting', section: 'procedure', field: 'setting', options: SETTING_OPTIONS })
  ]));
  card.appendChild(boolField({ label: 'Image guidance required (ultrasound / CT / MRI)', section: 'procedure', field: 'imagingGuidanceRequired' }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Primary indication and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    label: 'Specific clinical question', section: 'indication', field: 'clinicalQuestion',
    rows: 2, required: true,
    placeholder: 'e.g. Confirm or exclude malignancy and provide tissue for molecular typing.'
  }));
  card.appendChild(textArea({ label: 'Relevant history', section: 'indication', field: 'relevantHistory', rows: 2 }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'Target lesion size, location, and any imaging correlate or previous finding.'
  });
  card.appendChild(textArea({
    label: 'Lesion description', section: 'lesion', field: 'lesionDescription',
    rows: 2,
    placeholder: 'e.g. 18 mm hypoechoic nodule, upper outer quadrant left breast.'
  }));
  card.appendChild(grid('two-col', [
    textInput({ label: 'Lesion size', section: 'lesion', field: 'lesionSize', type: 'number', min: 0, max: 500, step: 0.1, unit: 'mm' }),
    textInput({ label: 'Lesion location', section: 'lesion', field: 'lesionLocation' })
  ]));
  card.appendChild(grid('two-col', [
    selectInput({
      label: 'Imaging correlate', section: 'lesion', field: 'imagingCorrelate',
      options: [
    ...
    textInput({ label: 'Previous finding', section: 'lesion', field: 'previousFinding' })
  ]));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Bleeding & coagulation',
    description: 'Anticoagulant / antiplatelet use, INR, platelets, bleeding disorder, and immunosuppression — drive the bleeding-risk axis.'
  });
  card.appendChild(boolField({ label: 'Taking an anticoagulant (e.g. warfarin, DOAC)', section: 'bleeding', field: 'takingAnticoagulant' }));
  card.appendChild(textInput({ label: 'Anticoagulant agent', section: 'bleeding', field: 'anticoagulantAgent', placeholder: 'e.g. apixaban' }));
  card.appendChild(boolField({ label: 'Taking an antiplatelet (e.g. clopidogrel)', section: 'bleeding', field: 'takingAntiplatelet' }));
  card.appendChild(textInput({ label: 'Antiplatelet agent', section: 'bleeding', field: 'antiplateletAgent', placeholder: 'e.g. clopidogrel' }));
  card.appendChild(grid('two-col', [
    textInput({ label: 'INR', section: 'bleeding', field: 'inr', type: 'number', min: 0.5, max: 20, step: 0.1 }),
    textInput({ label: 'Platelet count', section: 'bleeding', field: 'plateletCount', type: 'number', min: 0, max: 2000, step: 1, unit: '×10⁹/L' })
  ]));
  card.appendChild(subHead('Coagulation flags'));
  card.appendChild(boolField({ label: 'Known bleeding disorder / coagulopathy', section: 'bleeding', field: 'bleedingDisorder' }));
  card.appendChild(boolField({ label: 'Immunosuppressed', section: 'bleeding', field: 'immunosuppressed' }));
  return card;
}
...
function renderStep7() {
  const card = sectionCard({
    stepNumber: 7,
    title: 'Risk & safety review',
    description: 'Live preview of the computed bleeding-risk band and safety flags as you complete the request.'
  });
  const preview = document.createElement('div');
  preview.id = 'risk-preview';
...
// ----------------------------------------------------------------------
// Conditional sections (live risk preview on step 7)
// ----------------------------------------------------------------------
...
function updateConditionalSections() {
  ...
  const flagsList = result.flags.length === 0
    ? `<p class="muted">No safety flags raised yet.</p>`
    : `<ul class="flags">${result.flags.map((f) => `
  ...
  preview.innerHTML = `
    <div class="recommendation-banner">
      <span class="band-badge bleed-${esc(result.bleedingRiskBand)}">Bleeding risk: ${esc(titleCase(result.bleedingRiskBand))}</span>
      ${result.twoWeekWaitEligible ? `<span class="band-badge tier-two-week-wait">Two-week-wait eligible</span>` : ''}
    </div>
    <p class="muted">${esc(result.anticoagulantAction)}</p>
    ${flagsList}
  `;
}
...
const TRACKED_FIELDS = [
  ...
  // 3 Procedure
  ['procedure', 'biopsySite'], ['procedure', 'biopsyMethod'],
  ['procedure', 'laterality'],
  // 4 Indication
  ['indication', 'primaryIndication'], ['indication', 'clinicalQuestion'],
  // 5 Lesion
  ['lesion', 'lesionDescription'],
  // 6 Bleeding
  ['bleeding', 'inr'], ['bleeding', 'plateletCount'],
  // 8 Triage
  ['triage', 'urgency']
];

const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'procedure', 4: 'indication',
  5: 'lesion', 6: 'bleeding', 7: 'bleeding', 8: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, section: 'procedure',  title: 'Procedure' },
  { step: 4, section: 'indication', title: 'Indication' },
  { step: 5, section: 'lesion',     title: 'Lesion' },
  { step: 6, section: 'bleeding',   title: 'Bleeding' },
  { step: 7, section: 'bleeding',   title: 'Risk review' },
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>Biopsy Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · ${esc(biopsySiteLabel(state.procedure.biopsySite) || '—')}${state.indication.primaryIndication ? ` · ${esc(indicationLabel(state.indication.primaryIndication))}` : ''}</p>
    ...
    <div class="recommendation-banner">
      ...
      ${twoWeekWaitEligible ? `<span class="band-badge tier-two-week-wait">Two-week-wait eligible</span>` : ''}
    </div>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Bleeding risk</span>
        <span class="axis-value"><span class="band-badge bleed-${esc(bleedingRiskBand)}">${esc(titleCase(bleedingRiskBand))}</span></span>
      ...
      <div class="axis-card">
        <span class="axis-name">D · Urgency / cancer pathway</span>
