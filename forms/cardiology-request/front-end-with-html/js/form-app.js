...
import { emptyRequest, reasonLabel, serviceLabel } from './types.js';

// Cardiology Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'cardiology-request.front-end-with-html.v1';
const TOTAL_STEPS = 8;
...
const SERVICE_OPTIONS = [
  ...
  { value: 'rapid-access-chest-pain', label: 'Rapid-access chest-pain clinic' },
  { value: 'heart-failure', label: 'Heart-failure clinic' },
  { value: 'arrhythmia-ep', label: 'Arrhythmia / EP clinic' },
  { value: 'valve-clinic', label: 'Valve clinic' },
  { value: 'inherited-cardiac-conditions', label: 'Inherited cardiac conditions' },
  { value: 'pre-operative-cardiac', label: 'Pre-operative cardiac' },
...
const REASON_OPTIONS = [
  ...
  { value: 'heart-failure-symptoms', label: 'Heart-failure symptoms' },
  { value: 'murmur-or-valve', label: 'Murmur / valve' },
  { value: 'abnormal-ecg', label: 'Abnormal ECG' },
...
const CHEST_PAIN_CHARACTER_OPTIONS = [
...
function renderStep1() {
  const card = sectionCard({
    ...
    description: 'Identify the clinician making this referral into cardiology.'
  });
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'physiologist', label: 'Cardiac physiologist' },
  ...
  card.appendChild(grid('two-col', [
    textInput({ label: 'Referring site / practice', section: 'clinician', field: 'siteName' }),
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Requested service, primary reason, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Requested cardiology service', section: 'request', field: 'requestedService',
    options: SERVICE_OPTIONS, required: true
  }));
  card.appendChild(selectInput({
    label: 'Primary referral reason', section: 'request', field: 'referralReason',
    options: REASON_OPTIONS, required: true
  }));
  card.appendChild(textArea({
    label: 'Specific clinical question', section: 'request', field: 'clinicalQuestion',
    rows: 2, required: true,
    placeholder: 'e.g. Does this patient have angina requiring revascularisation assessment?'
  }));
  card.appendChild(textArea({ label: 'Relevant history', section: 'request', field: 'relevantHistory', rows: 2 }));
  card.appendChild(selectInput({
    label: 'Care setting', section: 'request', field: 'setting', options: SETTING_OPTIONS
  }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Symptoms',
    description: 'Presenting symptoms and functional class.'
  });
  card.appendChild(boolField({ label: 'Chest pain', section: 'symptoms', field: 'symptomChestPain' }));
  card.appendChild(selectInput({
    label: 'Chest pain character', section: 'symptoms', field: 'chestPainCharacter',
    options: CHEST_PAIN_CHARACTER_OPTIONS
  }));
  card.appendChild(boolField({ label: 'Breathlessness (dyspnoea)', section: 'symptoms', field: 'symptomBreathlessness' }));
  card.appendChild(selectInput({
    label: 'NYHA functional class', section: 'symptoms', field: 'nyhaClass',
    options: NYHA_OPTIONS
  }));
  card.appendChild(boolField({ label: 'Palpitations', section: 'symptoms', field: 'symptomPalpitations' }));
  card.appendChild(boolField({ label: 'Syncope / blackout', section: 'symptoms', field: 'symptomSyncope' }));
  card.appendChild(boolField({ label: 'Peripheral oedema', section: 'symptoms', field: 'symptomOedema' }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Red flags & investigations',
    description: 'Acute red flags drive the safety axis and auto-escalate triage. Record any investigations already performed.'
  });
  card.appendChild(subHead('Acute red flags'));
  card.appendChild(boolField({ label: 'Suspected acute coronary syndrome', section: 'redFlags', field: 'suspectedAcs' }));
  card.appendChild(boolField({ label: 'Exertional syncope', section: 'redFlags', field: 'exertionalSyncope' }));
  card.appendChild(boolField({ label: 'New-onset heart failure', section: 'redFlags', field: 'newOnsetHeartFailure' }));
  card.appendChild(subHead('Investigations performed'));
  card.appendChild(boolField({ label: 'Resting ECG done', section: 'redFlags', field: 'ecgDone' }));
  card.appendChild(textArea({
    label: 'ECG findings', section: 'redFlags', field: 'ecgFindings', rows: 2,
    placeholder: 'e.g. ST changes, Q waves, AF, LBBB.'
  }));
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Troponin status', section: 'redFlags', field: 'troponinStatus', options: TEST_STATUS_OPTIONS }),
    selectInput({ label: 'BNP / NT-proBNP status', section: 'redFlags', field: 'bnpStatus', options: TEST_STATUS_OPTIONS })
  ]));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    ...
    description: 'Known cardiac history and risk factors.'
  });
  card.appendChild(boolField({ label: 'Known coronary artery disease', section: 'history', field: 'knownCoronaryArteryDisease' }));
  card.appendChild(boolField({ label: 'Previous myocardial infarction', section: 'history', field: 'previousMi' }));
  card.appendChild(boolField({ label: 'Established heart failure', section: 'history', field: 'heartFailure' }));
  card.appendChild(boolField({ label: 'Valve disease', section: 'history', field: 'valveDisease' }));
  card.appendChild(boolField({ label: 'Arrhythmia', section: 'history', field: 'arrhythmia' }));
  card.appendChild(boolField({ label: 'Hypertension', section: 'history', field: 'hypertension' }));
  card.appendChild(boolField({ label: 'Diabetes', section: 'history', field: 'diabetes' }));
  card.appendChild(textArea({
    label: 'Current medications', section: 'history', field: 'currentMedications', rows: 2,
...
function renderStep7() {
  const card = sectionCard({
    stepNumber: 7,
    title: 'Triage',
    description: 'Requested urgency. Red flags auto-escalate the triage tier.'
  });
  card.appendChild(grid('two-col', [
...
const TRACKED_FIELDS = [
  ...
  // 3 Request
  ['request', 'requestedService'], ['request', 'referralReason'],
  ['request', 'clinicalQuestion'],
  // 4 Symptoms
  ['symptoms', 'chestPainCharacter'],
  // 5 Red flags & investigations
  ['redFlags', 'troponinStatus'],
  // 7 Triage
  ['triage', 'urgency']
];

const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'request', 4: 'symptoms',
  5: 'redFlags', 6: 'history', 7: 'triage', 8: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, section: 'request',   title: 'Reason' },
  { step: 4, section: 'symptoms',  title: 'Symptoms' },
  { step: 5, section: 'redFlags',  title: 'Red flags' },
  { step: 6, section: 'history',   title: 'History' },
  { step: 7, section: 'triage',    title: 'Triage' },
  { step: 8, section: 'triage',    title: 'Review' }
];
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>Cardiology Referral Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Reason: ${esc(reasonLabel(state.request.referralReason) || '—')} · Service: ${esc(serviceLabel(state.request.requestedService) || '—')}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Safety / red-flag</span>
        <span class="axis-value"><span class="band-badge safety-${esc(safetyBand)}">${esc(titleCase(safetyBand))}</span></span>
      </div>
      <div class="axis-card">
