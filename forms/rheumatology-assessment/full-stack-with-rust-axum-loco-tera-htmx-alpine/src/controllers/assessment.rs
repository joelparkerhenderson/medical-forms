...
use crate::engine::{rheumatology_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "laboratory_markers" => "laboratoryMarkers",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "patientName"
            ...
            | "assessmentDate"
            | "rheumatologistName"
            | "jointDeformityPresent"
            | "jointErosionPresent"
            | "affectedJointRegions"
            | "stiffnessFrequency"
            | "stiffnessImprovementWithActivity"
            | "flareFrequency"
            | "rheumatoidFactorPositive"
            | "antiCcpPositive"
            | "anaPositive"
            | "xrayErosionsPresent"
            | "xrayJointSpaceNarrowing"
            | "ultrasoundSynovitisPresent"
            | "mriBoneEdemaPresent"
            | "imagingProgressionSinceLast"
            | "overallRadiographicStage"
            | "workDisability"
            | "assistiveDevicesNeeded"
            | "currentDmardTherapy"
            | "biologicTherapy"
            | "corticosteroidUse"
            | "nsaidUse"
...
async fn show_report(
    ...
    // Run grading engine
    let (activity_level, activity_score, fired_rules) =
        rheumatology_grader::calculate_activity(&assessment_data);
    ...
    let das28_cat = crate::engine::utils::das28_category(
        assessment_data.disease_activity.das28_score,
    );

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("activity_level", &activity_level);
    context.insert("activity_label", &crate::engine::utils::activity_level_label(&activity_level));
    context.insert("activity_score", &activity_score);
    context.insert("das28_category", &das28_cat);
    context.insert("das28_score", &assessment_data.disease_activity.das28_score);
