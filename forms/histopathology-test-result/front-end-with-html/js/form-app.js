...
// inline structured histopathology report. State is persisted to localStorage
...
const STORAGE_KEY = 'histopathology-test-result.front-end-with-html.v1';

/**
 * Load saved state, merging over a fresh empty result so newly-added fields
 * default correctly and stale / mistyped values are dropped (null-safe
 * rehydration: a saved value is only kept when its type matches the fresh
 * default's type — this form has no numeric fields, but a numeric field
 * defaulting to `null` would be accepted as either number or null here).
 * @returns {import('./types.js').HistopathologyResult}
 */
...
function mergeIntoDefaults(parsed) {
  ...
  for (const key of Object.keys(fresh)) {
    if (!parsed) continue;
    if (fresh[key] === null) {
      // Numeric fields default to null: accept a finite number or null.
      if (parsed[key] === null || typeof parsed[key] === 'number') {
        fresh[key] = parsed[key];
      }
    } else if (typeof parsed[key] === typeof fresh[key]) {
      fresh[key] = parsed[key];
    }
  }
  return fresh;
...
/** @param {import('./types.js').HistopathologyResult} state */
...
/** @type {import('./types.js').HistopathologyResult} */
let state = loadState();
...
function renderStep1() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Link to the originating histopathology request. Malignancy with no linked request is treated as an unexpected finding.',
...
function renderStep2() {
  const card = sectionCard({
    stepNumber: 2,
    title: 'Specimen',
    description: 'The examined specimen, its anatomical site, and its diagnostic adequacy.'
  });

  card.appendChild(textInput({
    label: 'Specimen type',
    field: 'specimenType', required: true,
    placeholder: 'e.g. core biopsy, excision, resection, endoscopic biopsy'
  }));
  card.appendChild(textInput({
    label: 'Specimen site',
    field: 'specimenSite',
    placeholder: 'e.g. right breast, sigmoid colon, left forearm skin'
  }));
  card.appendChild(selectInput({
    label: 'Specimen adequacy',
    field: 'specimenAdequacy',
    hint: 'An inadequate specimen classifies the result as inconclusive.',
    options: [
  ...
  card.appendChild(conditionalAlert({
    id: 'adequacy-warning',
    type: 'warning',
    heading: 'Specimen adequacy reduced',
    message: 'An inadequate specimen classifies the result as inconclusive; a suboptimal specimen with no recorded diagnosis also classifies as inconclusive. Consider requesting a repeat or further specimen.'
  }));
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'The clinical question and any comparison with previous histology.'
  });
  ...
  card.appendChild(textArea({
    label: 'Comparison with previous histology',
    field: 'comparisonWithPrevious', rows: 3,
    placeholder: 'Relevant previous histology and changes since…'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Examination',
    description: 'The macroscopic and microscopic descriptions and the histopathological diagnosis.'
  });

  card.appendChild(textArea({
    label: 'Macroscopic description',
    field: 'macroscopicDescription', rows: 4,
    placeholder: 'Gross description: dimensions, appearance, blocks taken…'
  }));
  card.appendChild(textArea({
    label: 'Microscopic description',
    field: 'microscopicDescription', rows: 5,
    placeholder: 'Histological description of the tissue under the microscope…'
  }));
  card.appendChild(textArea({
    label: 'Diagnosis',
    field: 'diagnosis', rows: 3, required: true,
    placeholder: 'Histopathological diagnosis (the body of the report conclusion)…'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Malignancy & staging',
    description: 'Malignancy and tumour characterisation: tumour type, histological grade, pathological TNM stage, resection margins, lymphovascular invasion, and ancillary coding.'
  });

  card.appendChild(checkboxGroup({
    label: 'Malignancy',
    items: [
      { field: 'malignancyPresent', label: 'Malignancy present' },
      { field: 'lymphovascularInvasion', label: 'Lymphovascular invasion' }
    ]
  }));
  ...
  card.appendChild(textInput({
    label: 'Pathological pT',
    field: 'tnmPt',
    hint: 'UICC/AJCC TNM 8th edition, e.g. pT1c.',
    placeholder: 'e.g. pT1c'
  }));
  card.appendChild(textInput({
    label: 'Pathological pN',
    field: 'tnmPn',
    hint: 'e.g. pN0.',
    placeholder: 'e.g. pN0'
  }));
  card.appendChild(textInput({
    label: 'Pathological pM',
    field: 'tnmPm',
    hint: 'e.g. pM0 / pM1.',
    placeholder: 'e.g. pM0'
  }));
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'An unexpected malignancy (no linked originating request) or an involved resection margin auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the requester on sign-off.'
  }));
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Impression',
    description: 'The summary impression, structured grade/stage summary, and recommended follow-up.'
  });
  ...
  card.appendChild(textInput({
    label: 'Reporting category (grade/stage summary)',
    field: 'reportingCategory',
    hint: "RCPath cancer-dataset grade/stage summary line, e.g. 'pT1c pN0, moderately differentiated'.",
    placeholder: 'e.g. pT1c pN0, moderately differentiated'
  }));
  card.appendChild(textArea({
    label: 'Recommended follow-up',
    field: 'recommendedFollowUp', rows: 3,
    placeholder: 'Recommended follow-up, referral, MDT discussion, or management…'
  }));
...
function renderStep7() {
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'This report contains a critical finding. Communicate the result directly to the requester and record it below before signing.'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'adequacy-warning':
      state.specimenAdequacy === 'inadequate' || state.specimenAdequacy === 'suboptimal',
    'critical-finding-alert': hasUnexpectedMalignancy(state) || hasInvolvedMargin(state),
...
const STEP_SLOTS = {
  ...
  2: [['specimenType'], ['specimenSite'], ['specimenAdequacy']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['macroscopicDescription'], ['microscopicDescription'], ['diagnosis']],
  5: [['malignancyPresent', 'lymphovascularInvasion', 'tumourType'],
      ['histologicalGrade'], ['tnmPt', 'tnmPn', 'tnmPm'], ['resectionMargins'],
      ['immunohistochemistry', 'snomedCode']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Specimen type', state.specimenType || 'Not recorded'],
    ['Specimen site', state.specimenSite || 'Not recorded'],
    ['Specimen adequacy', specimenAdequacyLabel(state.specimenAdequacy)],
    ['Malignancy present', state.malignancyPresent ? 'Yes' : 'No'],
    ['Tumour type', state.tumourType || 'Not recorded'],
    ['Histological grade', histologicalGradeLabel(state.histologicalGrade)],
    ['Pathological TNM', [state.tnmPt, state.tnmPn, state.tnmPm].filter((t) => t.trim() !== '').join(' ') || 'Not recorded'],
    ['Resection margins', resectionMarginsLabel(state.resectionMargins)],
    ['Lymphovascular invasion', state.lymphovascularInvasion ? 'Yes' : 'No'],
    ['SNOMED CT code', state.snomedCode || 'Not recorded'],
  ...
  out.innerHTML = `
    <div class="report-card">
      ...
      <h3>Diagnosis</h3>
      <p>${esc(state.diagnosis || 'Not recorded')}</p>
