...
use crate::engine::{mcas_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "skin_manifestations" => "skinManifestations",
        "gastrointestinal_symptoms" => "gastrointestinalSymptoms",
        "cardiovascular_neurological" => "cardiovascularNeurological",
        "respiratory_symptoms" => "respiratorySymptoms",
        "laboratory_studies" => "laboratoryStudies",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "patientName"
            ...
            | "symptomProgression"
            | "episodeDuration"
            | "emergencyVisitsPastYear"
            | "flushingFrequency"
            | "dermatographismPresent"
            | "skinLesionsPresent"
            | "foodIntolerancesCount"
            | "malabsorptionSigns"
            | "stridorPresent"
            | "previousAnaphylaxis"
            | "serumTryptaseElevated"
            | "serumTryptaseLevel"
            | "urineProstaglandinD2Elevated"
            | "urineNMethylhistamineElevated"
            | "plasmaHistamineElevated"
            | "serumChromograninAElevated"
            | "otherMediatorsElevated"
            | "boneMarrowBiopsyDone"
            | "triggerPredictability"
            | "epinephrineAutoInjector"
            | "corticosteroidUse"
            | "otherMedications"
            | "consensusCriteriaMet"
            | "organSystemsInvolvedCount"
            | "differentialDiagnosesExcluded"
            | "comorbidConditions"
            | "additionalNotes"
    )
}

/// Parse a form value, keeping string fields as strings and converting severity fields to numbers.
...
async fn show_report(
    ...
    let (severity_level, severity_score, fired_rules) =
        mcas_grader::calculate_severity(&assessment_data);
