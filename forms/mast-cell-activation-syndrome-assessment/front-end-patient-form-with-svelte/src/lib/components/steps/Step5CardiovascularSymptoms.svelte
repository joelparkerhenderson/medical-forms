...
<SectionCard title="Cardiovascular Symptoms" description="Rate the severity and frequency of your heart and blood pressure symptoms">
	{#each symptoms as symptom}
		<div class="mb-6 border-b border-gray-100 pb-4 last:border-0 last:pb-0">
					...
					{#each severityOptions as opt}
						<label
							class="flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-1.5 text-sm transition-colors
								{c[symptom.key].severity === opt.value ? 'border-primary bg-blue-50 font-medium' : 'border-gray-300 bg-white hover:bg-gray-50'}"
						>
							<input
								type="radio"
			...
			<SelectInput
				...
				bind:value={c[symptom.key].frequency}
			/>
		</div>
	{/each}
</SectionCard>
