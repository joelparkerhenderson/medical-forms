...
Generate Loco (loco.rs) scaffolding for the `mast-cell-activation-syndrome-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name mast-cell-activation-syndrome-assessment --db postgres --bg async --assets serverside
   ```
