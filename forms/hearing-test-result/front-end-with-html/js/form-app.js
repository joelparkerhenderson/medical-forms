...
// inline structured audiology report. State is persisted to localStorage so a
...
const STORAGE_KEY = 'hearing-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').HearingResult} */
...
function mergeIntoDefaults(parsed) {
  ...
  // Numeric fields default to null (typeof null === 'object'), so accept a
  // saved finite number or null there; every other field must type-match.
...
/** @param {import('./types.js').HearingResult} state */
...
/** @type {import('./types.js').HearingResult} */
let state = loadState();
...
function renderStep1() {
  ...
  card.appendChild(textInput({
    ...
    placeholder: 'e.g. Ms A Audiologist'
  }));
  card.appendChild(textInput({
...
function renderStep2() {
  const card = sectionCard({
    stepNumber: 2,
    title: 'Test details',
    description: 'The performed audiology test and its reliability.'
  });
  ...
  card.appendChild(selectInput({
    ...
    options: [
      { value: 'pure-tone-audiometry', label: 'Pure-tone audiometry' },
      { value: 'tympanometry', label: 'Tympanometry' },
      { value: 'speech-audiometry', label: 'Speech audiometry' },
      { value: 'otoacoustic-emissions', label: 'Otoacoustic emissions' },
      { value: 'auditory-brainstem-response', label: 'Auditory brainstem response' },
      { value: 'newborn-hearing-screen', label: 'Newborn hearing screen' },
      { value: 'other', label: 'Other' }
    ]
  }));
  card.appendChild(selectInput({
    label: 'Test reliability',
    field: 'testReliability',
    hint: 'Reliability / validity of the test result (e.g. patient cooperation, masking, conditions).',
    options: [
  ...
  card.appendChild(conditionalAlert({
    ...
    type: 'warning',
    heading: 'Reduced test reliability',
    message: 'A fair or poor reliability reduces diagnostic confidence and raises an unreliable-test flag. A poor test with no impression classifies the result as inconclusive.'
  }));
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'The clinical question the test was performed to answer.'
  });
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Audiometry and interpretation',
    description: 'Per-ear pure-tone averages (dB HL), hearing-loss type and severity (BSA descriptors), and tympanogram type (Jerger).'
  });

  card.appendChild(textInput({
    label: 'Pure-tone average — right ear (dB HL)',
    field: 'pureToneAverageRightDb',
    type: 'number', min: -10, max: 130, step: 0.1, unit: 'dB HL',
    hint: 'BSA mean threshold; no-response frequencies counted at 130 dB HL.'
  }));
  card.appendChild(textInput({
    label: 'Pure-tone average — left ear (dB HL)',
    field: 'pureToneAverageLeftDb',
    type: 'number', min: -10, max: 130, step: 0.1, unit: 'dB HL',
    hint: 'BSA mean threshold; no-response frequencies counted at 130 dB HL.'
  }));
  card.appendChild(selectInput({
    label: 'Hearing-loss type — right ear',
    field: 'hearingLossTypeRight',
    options: [
  ...
  card.appendChild(selectInput({
    label: 'Hearing-loss type — left ear',
    field: 'hearingLossTypeLeft',
    options: [
      { value: 'none', label: 'None' },
      { value: 'conductive', label: 'Conductive' },
      { value: 'sensorineural', label: 'Sensorineural' },
  ...
  card.appendChild(selectInput({
    label: 'Hearing-loss severity — right ear',
    field: 'hearingLossSeverityRight',
    options: [
  ...
  card.appendChild(selectInput({
    label: 'Hearing-loss severity — left ear',
    field: 'hearingLossSeverityLeft',
    options: [
      { value: 'normal', label: 'Normal' },
      { value: 'mild', label: 'Mild' },
      { value: 'moderate', label: 'Moderate' },
  ...
  card.appendChild(selectInput({
    label: 'Tympanogram — right ear (Jerger)',
    field: 'tympanometryTypeRight',
    options: [
      { value: 'A', label: 'A' },
      { value: 'As', label: 'As' },
      { value: 'Ad', label: 'Ad' },
  ...
  card.appendChild(selectInput({
    label: 'Tympanogram — left ear (Jerger)',
    field: 'tympanometryTypeLeft',
    options: [
      { value: 'A', label: 'A' },
      { value: 'As', label: 'As' },
      { value: 'Ad', label: 'Ad' },
...
function renderStep5() {
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'hearingLossPresent', label: 'Hearing loss present' },
      { field: 'asymmetricLoss', label: 'Marked asymmetry between ears' },
      { field: 'suddenSensorineuralLoss', label: 'Sudden sensorineural hearing loss' },
      { field: 'conductiveComponent', label: 'Conductive component' },
      { field: 'normalHearing', label: 'Normal hearing bilaterally' }
    ]
  }));
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'Sudden sensorineural hearing loss (an otological emergency) or a marked asymmetry (red flag for retrocochlear pathology) auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep6() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Structured-reporting category label (e.g. a BSA audiometric descriptor or configuration).',
    placeholder: 'e.g. bilateral symmetrical high-frequency loss'
  }));
  card.appendChild(textArea({
    label: 'Recommended follow-up',
    field: 'recommendedFollowUp', rows: 3,
    placeholder: 'Recommended follow-up testing, referral, or management (e.g. hearing-aid fitting, ENT referral, MRI IAM)…'
  }));
...
function renderStep7() {
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'This report contains a critical finding (sudden sensorineural hearing loss or marked asymmetry). Communicate the result directly to the referrer and record it below before signing.'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'reliability-warning': state.testReliability === 'poor' || state.testReliability === 'fair',
    'critical-finding-alert': state.suddenSensorineuralLoss || state.asymmetricLoss,
...
// answered once any structured finding is recorded, and the per-ear pairs
// count once either ear is interpreted.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['testType'], ['testReliability']],
  3: [['clinicalHistory']],
  4: [['pureToneAverageRightDb'], ['pureToneAverageLeftDb'],
      ['hearingLossTypeRight', 'hearingLossTypeLeft'],
      ['hearingLossSeverityRight', 'hearingLossSeverityLeft'],
      ['tympanometryTypeRight', 'tympanometryTypeLeft']],
  5: [['findingsNarrative'],
      ['hearingLossPresent', 'asymmetricLoss', 'suddenSensorineuralLoss',
       'conductiveComponent', 'normalHearing']],
...
/** Format a nullable dB HL measurement for the report. */
function formatDb(value) {
  return value === null || value === undefined ? 'Not recorded' : `${value} dB HL`;
}
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Test reliability', testReliabilityLabel(state.testReliability)],
    ['PTA — right ear', formatDb(state.pureToneAverageRightDb)],
    ['PTA — left ear', formatDb(state.pureToneAverageLeftDb)],
    ['Hearing-loss type (R / L)',
      `${hearingLossTypeLabel(state.hearingLossTypeRight)} / ${hearingLossTypeLabel(state.hearingLossTypeLeft)}`],
    ['Hearing-loss severity (R / L)',
      `${hearingLossSeverityLabel(state.hearingLossSeverityRight)} / ${hearingLossSeverityLabel(state.hearingLossSeverityLeft)}`],
    ['Tympanogram (R / L)',
      `${state.tympanometryTypeRight || 'Unspecified'} / ${state.tympanometryTypeLeft || 'Unspecified'}`],
