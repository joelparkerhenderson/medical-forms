# Hearing Test Result — specification
...
`hasAnyAbnormalFinding` (Axis A's classification predicate) did not check the worst pure-tone average, so a study with a PTA >= 21 dB HL and no other structured finding classified `normal` on Axis A while Axis B graded it independently. Fixed by adding that condition to `hasAnyAbnormalFinding` in both `js/rules.js` and `src/lib/engine/utils.ts`. Fixed 2026-09-06; previously verified and documented (not silently patched) in `examples/personas.json`.
...
- `bin/test-form hearing-test-result` exits cleanly.
...
- `bin/lily-html-refactor --check hearing-test-result` reports no drift.
- LocalStorage keys preserve draft state across reloads:
  - `hearing-test-result.front-end-with-html.v1` (HTML)
  - `hearing-test-result.front-end-with-svelte.v1` (SvelteKit)
