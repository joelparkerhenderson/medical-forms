...
use crate::models::_entities::ultrasound_test_results::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.cyst = Set(self.cyst);
      item.gallstones = Set(self.gallstones);
      item.hydronephrosis = Set(self.hydronephrosis);
      item.free_fluid = Set(self.free_fluid);
      item.dvt_present = Set(self.dvt_present);
      item.aneurysm = Set(self.aneurysm);
      item.organ_enlargement = Set(self.organ_enlargement);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/ultrasound_test_results/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
