...
// Ultrasound Test Result — reporting-clinician wizard
...
// inline structured ultrasound report. State is persisted to localStorage so
...
/** @param {import('./types.js').UltrasoundResult} state */
...
/** @type {import('./types.js').UltrasoundResult} */
let state = loadState();
...
/**
 * Numeric input bound to a `number | null` field. An empty control maps to
 * `null` (the repo convention for unanswered numeric fields); a filled
 * control maps to `Number(value)`.
 */
...
function renderStep1() {
  ...
  card.appendChild(textInput({
    ...
    placeholder: 'e.g. Dr A Sonographer'
  }));
  card.appendChild(textInput({
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The examined body region, laterality, and diagnostic adequacy of the examination.'
  });
  ...
  card.appendChild(selectInput({
    ...
    hint: 'Diagnostic adequacy of the examination. A non-diagnostic study classifies as inconclusive.',
    options: [
      { value: 'adequate', label: 'Adequate' },
      { value: 'limited-body-habitus', label: 'Limited (body habitus)' },
  ...
  card.appendChild(conditionalAlert({
    id: 'adequacy-warning',
    type: 'warning',
    heading: 'Limited or non-diagnostic examination',
    message: 'Diagnostic confidence may be reduced. A non-diagnostic study classifies as inconclusive; a limited study without an impression also classifies as inconclusive. Consider repeating or supplementing the examination.'
  }));
...
function renderStep4() {
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'massOrLesion', label: 'Mass or lesion' },
      { field: 'cyst', label: 'Cyst' },
      { field: 'gallstones', label: 'Gallstones' },
      { field: 'hydronephrosis', label: 'Hydronephrosis' },
      { field: 'freeFluid', label: 'Free fluid' },
      { field: 'dvtPresent', label: 'DVT present' },
      { field: 'aneurysm', label: 'Aneurysm' },
      { field: 'organEnlargement', label: 'Organ enlargement' },
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'A DVT or aneurysm auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep5() {
  ...
  card.appendChild(numberInput({
    ...
    hint: 'Largest lesion long-axis size, for surveillance and categorisation. 10–29 mm grades as a measurable lesion; 30 mm or larger grades as major.',
    placeholder: 'e.g. 18'
  }));

  card.appendChild(conditionalAlert({
    id: 'missing-measurement-warning',
    type: 'warning',
    heading: 'Mass or lesion without a measurement',
    message: 'A mass / lesion is recorded on the findings step but no largest-lesion size has been measured. Record the long-axis size in millimetres for surveillance.'
  }));
...
function renderStep6() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Optional structured-reporting category label (e.g. an ACR TI-RADS level or breast U-classification).',
    placeholder: 'e.g. TR4 or U3'
  }));
  card.appendChild(textArea({
...
function renderStep7() {
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'This report contains a critical finding (DVT present or aneurysm). Communicate the result directly to the referrer and record it below before signing.'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'adequacy-warning':
      ...
      state.examinationAdequacy === 'limited-body-habitus',
    'critical-finding-alert': state.dvtPresent || state.aneurysm,
    'missing-measurement-warning':
      state.massOrLesion && state.largestLesionSizeMm === null,
...
// counts only when ticked; a nullable number counts when non-null). This lets
// the checkbox-only findings step count as answered once any structured
// finding is recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['bodyRegion'], ['laterality'], ['examinationAdequacy']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['findingsNarrative'],
      ['massOrLesion', 'cyst', 'gallstones', 'hydronephrosis', 'freeFluid',
       'dvtPresent', 'aneurysm', 'organEnlargement', 'incidentalFinding']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Body region', bodyRegionLabel(state.bodyRegion)],
