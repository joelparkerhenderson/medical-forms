...
Generate Loco (loco.rs) scaffolding for the `learning-disability-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name learning-disability-assessment --db postgres --bg async --assets serverside
   ```
