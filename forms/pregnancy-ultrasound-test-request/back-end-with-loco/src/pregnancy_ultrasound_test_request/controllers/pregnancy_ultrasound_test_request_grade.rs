...
use crate::models::_entities::pregnancy_ultrasound_test_request_grades::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.pregnancy_ultrasound_test_request_id = Set(self.pregnancy_ultrasound_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.gestational_age_window_fit = Set(self.gestational_age_window_fit.clone());
      item.recommended_scan_type = Set(self.recommended_scan_type.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/pregnancy_ultrasound_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
