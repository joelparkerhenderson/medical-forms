...
import { emptyRequest, indicationLabel, monitorTypeLabel } from './types.js';

// Holter Monitor Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'holter-monitor-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 6;
...
const MONITOR_TYPE_OPTIONS = [
  { value: '24-hour', label: '24-hour Holter' },
  { value: '48-hour', label: '48-hour Holter' },
  { value: '7-day', label: '7-day monitor' },
  { value: '14-day', label: '14-day monitor' },
...
const INDICATION_OPTIONS = [
  ...
  { value: 'suspected-arrhythmia', label: 'Suspected arrhythmia' },
  { value: 'syncope', label: 'Syncope' },
  { value: 'atrial-fibrillation-detection', label: 'Atrial-fibrillation detection' },
  { value: 'post-stroke-af-screen', label: 'Post-stroke AF screen' },
  { value: 'rate-control-assessment', label: 'Rate-control assessment' },
  { value: 'qt-monitoring', label: 'QT monitoring' },
  { value: 'pacemaker-check', label: 'Pacemaker check' },
  { value: 'other', label: 'Other' }
];

const SYMPTOM_FREQUENCY_OPTIONS = [
  { value: 'daily', label: 'Daily / near-daily' },
  { value: 'weekly', label: 'Weekly' },
  { value: 'monthly', label: 'Monthly' },
  { value: 'rare', label: 'Rare (< monthly)' }
];

const KNOWN_ARRHYTHMIA_OPTIONS = [
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Monitor type, indication, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Requested monitor type', section: 'request', field: 'monitorType',
    options: MONITOR_TYPE_OPTIONS, required: true
  }));
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Capture rhythm during palpitations and quantify ectopic burden.'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Symptoms',
    description: 'Symptoms and how often they occur — symptom frequency drives monitor-duration matching.'
  });
  card.appendChild(subHead('Reported symptoms'));
  card.appendChild(boolField({ label: 'Palpitations', section: 'symptoms', field: 'palpitations' }));
  card.appendChild(boolField({ label: 'Syncope (red flag)', section: 'symptoms', field: 'syncope' }));
  card.appendChild(boolField({ label: 'Presyncope / near-syncope', section: 'symptoms', field: 'presyncope' }));
  card.appendChild(boolField({ label: 'Breathlessness', section: 'symptoms', field: 'breathlessness' }));
  card.appendChild(selectInput({
    label: 'Symptom frequency', section: 'symptoms', field: 'symptomFrequency',
    options: SYMPTOM_FREQUENCY_OPTIONS
  }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Cardiac context & red flags',
    description: 'Known arrhythmia, recent stroke / TIA, and relevant medications. Red flags auto-escalate the triage tier.'
  });
  card.appendChild(selectInput({
    label: 'Known arrhythmia', section: 'cardiac', field: 'knownArrhythmia',
    options: KNOWN_ARRHYTHMIA_OPTIONS
  }));
  card.appendChild(boolField({ label: 'Recent stroke / TIA', section: 'cardiac', field: 'recentStrokeTia' }));
  card.appendChild(textArea({
    label: 'Relevant medications', section: 'cardiac', field: 'relevantMedications', rows: 2,
    placeholder: 'e.g. rate-control, antiarrhythmics, QT-prolonging drugs.'
  }));
  return card;
}
...
const TRACKED_FIELDS = [
  ...
  // 3 Request
  ['request', 'monitorType'], ['request', 'primaryIndication'],
  ['request', 'clinicalQuestion'],
  // 4 Symptoms
  ['symptoms', 'symptomFrequency'],
  // 5 Cardiac context
  ['cardiac', 'knownArrhythmia'],
  // 6 Triage
...
const STEP_DEFINITIONS = [
  ...
  { step: 5, section: 'cardiac',   title: 'Cardiac context' },
...
function renderReport() {
  ...
  const matchValue = matchFit
    ? `<span class="band-badge match-${esc(matchFit)}">${esc(titleCase(matchFit))}</span>${
        matchFit === 'mismatched' && recommendedMonitor
          ? ` <span class="muted">→ ${esc(recommendedMonitor)}</span>`
          : ''
      }`
  ...
  out.innerHTML = `
    <h2>Holter Monitor Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Monitor: ${esc(monitorTypeLabel(state.request.monitorType) || '—')} · Indication: ${esc(indicationLabel(state.request.primaryIndication) || '—')}</p>
    ...
    <div class="axis-grid">
      <div class="axis-card">
        ...
        <div class="completeness-bar" style="margin-top:0.5rem"><span style="width:${(appropriatenessScore / 9) * 100}%"></span></div>
        <span class="muted" style="display:block;margin-top:0.375rem">Frequency / monitor match: ${matchValue}</span>
      </div>
      <div class="axis-card">
        <span class="axis-name">B · Urgency / triage</span>
