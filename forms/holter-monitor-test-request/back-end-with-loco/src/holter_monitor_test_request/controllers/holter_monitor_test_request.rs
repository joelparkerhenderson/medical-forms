...
use crate::models::_entities::holter_monitor_test_requests::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.symptom_syncope = Set(self.symptom_syncope);
      item.symptom_presyncope = Set(self.symptom_presyncope);
      item.symptom_breathlessness = Set(self.symptom_breathlessness);
      item.symptom_frequency = Set(self.symptom_frequency.clone());
      item.known_arrhythmia = Set(self.known_arrhythmia.clone());
      item.recent_stroke_tia = Set(self.recent_stroke_tia);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/holter_monitor_test_requests/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
