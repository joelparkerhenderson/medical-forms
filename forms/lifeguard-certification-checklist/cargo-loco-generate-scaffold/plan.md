...
Generate Loco (loco.rs) scaffolding for the `lifeguard-certification-checklist` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name lifeguard-certification-checklist --db postgres --bg async --assets serverside
   ```
