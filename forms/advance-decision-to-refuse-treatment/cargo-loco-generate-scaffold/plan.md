...
Generate Loco (loco.rs) scaffolding for the `advance-decision-to-refuse-treatment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name advance-decision-to-refuse-treatment --db postgres --bg async --assets serverside
   ```
