...
// inline structured sleep-study report. State is persisted to localStorage so
...
const STORAGE_KEY = 'sleep-study-test-result.front-end-with-html.v1';

/**
 * Load the saved report from localStorage, merging over a fresh empty result
 * so newly-added fields default correctly. Numeric metric fields accept
 * `null` or a finite number (their unanswered value is `null`, so a plain
 * `typeof` comparison would wrongly discard saved numbers); every other field
 * is kept only when the saved value's type matches the fresh default's type.
 *
...
/** @param {import('./types.js').SleepStudyResult} state */
...
/** @type {import('./types.js').SleepStudyResult} */
let state = loadState();
...
/**
 * Numeric input bound to a `number | null` field: an empty (or unparsable)
 * value stores `null`, mirroring the SQL / Svelte convention for unanswered
 * numeric fields.
 */
...
function renderStep1() {
  ...
  card.appendChild(textInput({
    ...
    placeholder: 'e.g. Dr A Sleep-Physician'
  }));
  card.appendChild(textInput({
...
function renderStep2() {
  const card = sectionCard({
    stepNumber: 2,
    title: 'Study details',
    description: 'The performed study type and its diagnostic adequacy.'
  });

  card.appendChild(selectInput({
    label: 'Study type',
    field: 'studyType', required: true,
    options: [
      { value: 'home-sleep-apnoea-test', label: 'Home sleep apnoea test' },
      { value: 'polysomnography', label: 'Polysomnography' },
      { value: 'overnight-oximetry', label: 'Overnight oximetry' },
      { value: 'multiple-sleep-latency-test', label: 'Multiple sleep latency test' },
      { value: 'actigraphy', label: 'Actigraphy' },
      { value: 'other', label: 'Other' }
    ]
  }));
  card.appendChild(selectInput({
    label: 'Study adequacy',
    field: 'studyAdequacy',
    options: [
  ...
  card.appendChild(conditionalAlert({
    id: 'adequacy-warning',
    type: 'warning',
    heading: 'Study adequacy reduced',
    message: 'A failed study classifies the result as inconclusive; a limited study without an impression also classifies as inconclusive. Consider repeating or supplementing the study.'
  }));
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'The clinical question and any comparison with previous studies.'
  });

  card.appendChild(textArea({
    label: 'Clinical history',
    field: 'clinicalHistory', rows: 3,
    placeholder: 'Clinical history and the question the study was performed to answer…'
  }));
  card.appendChild(textArea({
    label: 'Comparison with previous studies',
    field: 'comparisonWithPrevious', rows: 3,
    placeholder: 'Relevant previous sleep studies and changes since…'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Metrics',
    description: 'Recording and sleep time, AHI, ODI, SpO2 metrics, and mean heart rate.'
  });

  card.appendChild(numberInput({
    label: 'Total recording time (hours)',
    field: 'totalRecordingTimeHours',
    min: 0, max: 24, step: 0.1,
    hint: 'Total recording time in hours.'
  }));
  card.appendChild(numberInput({
    label: 'Total sleep time (hours)',
    field: 'totalSleepTimeHours',
    min: 0, max: 24, step: 0.1,
    hint: 'Total sleep time in hours (for attended PSG with sleep staging).'
  }));
  card.appendChild(numberInput({
    label: 'Apnoea-Hypopnoea Index — AHI (events/hour)',
    field: 'apnoeaHypopnoeaIndex',
    min: 0, max: 200, step: 0.1,
    hint: 'AASM bands: normal <5, mild 5 to <15, moderate 15 to <30, severe >=30.'
  }));
  card.appendChild(numberInput({
    label: 'Oxygen Desaturation Index — ODI (events/hour)',
    field: 'oxygenDesaturationIndex',
    min: 0, max: 200, step: 0.1,
    hint: '>=3% (or >=4%) desaturations per hour.'
  }));
  card.appendChild(numberInput({
    label: 'Minimum SpO2 (%)',
    field: 'minimumSpo2Percent',
    min: 0, max: 100, step: 0.1,
    hint: 'Lowest (nadir) oxygen saturation recorded.'
  }));
  card.appendChild(numberInput({
    label: 'Time below 90% SpO2 — T90 (%)',
    field: 'timeBelow90PercentSpo2',
    min: 0, max: 100, step: 0.1,
    hint: 'Percentage of time with SpO2 below 90%.'
  }));
  card.appendChild(numberInput({
    label: 'Mean heart rate (bpm)',
    field: 'meanHeartRateBpm',
    min: 0, max: 300, step: 1,
    hint: 'Mean heart rate during the study.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Findings',
    description: 'OSA severity, structured finding flags, and the narrative findings.'
  });

  card.appendChild(selectInput({
    label: 'OSA severity',
    field: 'osaSeverity',
    hint: 'AASM band derived from AHI: none <5, mild 5 to <15, moderate 15 to <30, severe >=30.',
    options: [
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'obstructiveSleepApnoea', label: 'Obstructive sleep apnoea' },
      { field: 'centralSleepApnoea', label: 'Central sleep apnoea' },
      { field: 'periodicLimbMovements', label: 'Periodic limb movements' },
      { field: 'nocturnalHypoventilation', label: 'Nocturnal hypoventilation' },
      { field: 'significantDesaturation', label: 'Significant desaturation' },
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'Severe OSA with significant desaturation, or nocturnal hypoventilation, auto-escalates the follow-up urgency to a critical alert and triggers an urgent CPAP / ventilation review. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep6() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Structured-reporting label for the AHI severity band (e.g. Severe OSA (AHI >=30)).',
    placeholder: 'e.g. Moderate OSA (AHI 15 to <30)'
  }));
  card.appendChild(textArea({
    label: 'Recommended follow-up',
    field: 'recommendedFollowUp', rows: 3,
    placeholder: 'Recommended follow-up: CPAP titration, ventilation review, sleep-clinic referral…'
  }));
...
function renderStep7() {
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'This report contains a critical finding. Arrange an urgent CPAP / ventilation review, communicate the result directly to the referrer, and record it below before signing.'
  }));
...
function updateConditionalAlerts() {
  const criticalSelected =
    (isSevereAhi(state) && state.significantDesaturation) ||
    state.nocturnalHypoventilation;
  const preview = calculateGrade(state);
  const visibility = {
    'adequacy-warning': state.studyAdequacy === 'failed' || state.studyAdequacy === 'limited',
    'critical-finding-alert': criticalSelected,
...
// counts only when ticked; a numeric metric counts when non-null). This lets
...
const STEP_SLOTS = {
  ...
  2: [['studyType'], ['studyAdequacy']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['totalRecordingTimeHours'], ['totalSleepTimeHours'], ['apnoeaHypopnoeaIndex'],
      ['oxygenDesaturationIndex'], ['minimumSpo2Percent'], ['timeBelow90PercentSpo2'],
      ['meanHeartRateBpm']],
  5: [['osaSeverity'],
      ['obstructiveSleepApnoea', 'centralSleepApnoea', 'periodicLimbMovements',
       'nocturnalHypoventilation', 'significantDesaturation', 'normalStudy'],
      ['findingsNarrative']],
...
/** Format a nullable numeric metric for the report table. */
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Study type', studyTypeLabel(state.studyType)],
    ['Study adequacy', studyAdequacyLabel(state.studyAdequacy)],
    ['OSA severity (reported)', osaSeverityLabel(state.osaSeverity)],
  ...
  const metricRows = [
    ['Total recording time', metric(state.totalRecordingTimeHours, 'h')],
    ['Total sleep time', metric(state.totalSleepTimeHours, 'h')],
    ['Apnoea-Hypopnoea Index (AHI)', metric(state.apnoeaHypopnoeaIndex, 'events/h')],
    ['Oxygen Desaturation Index (ODI)', metric(state.oxygenDesaturationIndex, 'events/h')],
    ['Minimum SpO2', metric(state.minimumSpo2Percent, '%')],
    ['Time below 90% SpO2 (T90)', metric(state.timeBelow90PercentSpo2, '%')],
    ['Mean heart rate', metric(state.meanHeartRateBpm, 'bpm')]
  ].map(([name, value]) => `
    <tr>
      <th scope="row">${esc(name)}</th>
      <td>${esc(value)}</td>
