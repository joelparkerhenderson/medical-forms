...
use crate::engine::{kinesiology_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "postural_assessment" => "posturalAssessment",
        "range_of_motion" => "rangeOfMotion",
        "muscle_strength_testing" => "muscleStrengthTesting",
...
async fn show_report(
    ...
    let (impairment_level, impairment_score, fired_rules) =
        kinesiology_grader::calculate_impairment(&assessment_data);
    ...
    let pain_cat = crate::engine::utils::pain_category(
        assessment_data.pain_assessment.pain_severity,
    );
    ...
    context.insert("pain_category", &pain_cat);
    context.insert("pain_severity", &assessment_data.pain_assessment.pain_severity);
