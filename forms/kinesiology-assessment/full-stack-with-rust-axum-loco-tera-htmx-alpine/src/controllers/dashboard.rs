...
async fn dashboard(
    ...
    if let Some(ref level) = params.impairment_level {
        if !level.is_empty() {
            items.retain(|row| row.impairment_level == *level);
        }
    }

    // Pain category filter
    if let Some(ref pain) = params.pain_category {
        if !pain.is_empty() {
            items.retain(|row| row.pain_category == *pain);
        }
    }
    ...
    context.insert("pain_category", &params.pain_category.unwrap_or_default());
