...
-- Additional safety-critical flags detected during kinesiology assessment grading.
...
COMMENT ON TABLE grading_additional_flag IS
    'Additional safety-critical flags detected during kinesiology FMS assessment grading (e.g. pain during clearing tests, significant asymmetries).';
...
COMMENT ON COLUMN grading_additional_flag.flag_id IS
    'Identifier of the flag (e.g. FLAG-PAIN-001, FLAG-ASYM-001).';
COMMENT ON COLUMN grading_additional_flag.category IS
    'Category of the flag (e.g. Pain, Asymmetry, Movement Dysfunction).';
