...
use crate::models::_entities::histopathology_test_requests::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.number_of_specimens = Set(self.number_of_specimens);
      ...
      item.provisional_diagnosis = Set(self.provisional_diagnosis.clone());
      item.previous_histology = Set(self.previous_histology.clone());
      item.urgent_frozen_section = Set(self.urgent_frozen_section);
      item.two_week_wait = Set(self.two_week_wait);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/histopathology_test_requests/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
