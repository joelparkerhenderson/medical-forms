...
import { emptyRequest, indicationLabel, specimenTypeLabel } from './types.js';

// Histopathology Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'histopathology-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
const SPECIMEN_TYPE_OPTIONS = [
  ...
  { value: 'endoscopic-biopsy', label: 'Endoscopic biopsy' },
  { value: 'skin-lesion', label: 'Skin lesion' },
  { value: 'frozen-section', label: 'Frozen section' },
...
const INDICATION_OPTIONS = [
  ...
  { value: 'margin-assessment', label: 'Margin assessment' },
  { value: 'transplant-monitoring', label: 'Transplant monitoring' },
  { value: 'other', label: 'Other' }
];

const FIXATIVE_OPTIONS = [
  { value: 'formalin', label: 'Formalin' },
  { value: 'fresh', label: 'Fresh (unfixed)' },
  { value: 'other', label: 'Other' }
];
...
function renderStep1() {
  const card = sectionCard({
    ...
    description: 'Identify the clinician submitting this specimen.'
  });
...
function renderStep3() {
  const card = sectionCard({
    stepNumber: 3,
    title: 'Specimen',
    description: 'Specimen type, site, number of pots, and fixative — drives the specimen-quality axis.'
  });
  card.appendChild(selectInput({
  ...
  card.appendChild(textInput({
    label: 'Anatomical site', section: 'specimen', field: 'specimenSite', required: true,
    placeholder: 'e.g. Sigmoid colon, left breast, right forearm skin'
  }));
  card.appendChild(grid('two-col', [
    textInput({ label: 'Number of specimens / pots', section: 'specimen', field: 'numberOfSpecimens', type: 'number', min: 0, max: 100 }),
    selectInput({ label: 'Fixative', section: 'specimen', field: 'fixative', options: FIXATIVE_OPTIONS })
  ]));
  card.appendChild(boolField({ label: 'Specimen container labelled with patient identifiers', section: 'specimen', field: 'specimenLabelled' }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Indication & clinical context',
    description: 'Indication and specific clinical question are the highest-value fields.'
  });
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Confirm or exclude invasive malignancy; assess excision margins.'
  }));
  card.appendChild(textArea({
    label: 'Relevant clinical details', section: 'indication', field: 'clinicalDetails', rows: 2,
    placeholder: 'Relevant history, imaging, prior treatment, and findings.'
  }));
  card.appendChild(grid('two-col', [
    textInput({ label: 'Provisional diagnosis', section: 'indication', field: 'provisionalDiagnosis' }),
    textInput({ label: 'Previous histology / cytology', section: 'indication', field: 'previousHistology' })
  ]));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'A frozen section or two-week-wait pathway auto-escalates the triage tier.'
  });
  card.appendChild(selectInput({
    label: 'Requested urgency', section: 'urgency', field: 'urgency',
    options: URGENCY_OPTIONS, required: true
  }));
  card.appendChild(subHead('Red flags'));
  card.appendChild(boolField({ label: 'Urgent intra-operative frozen section (immediate)', section: 'urgency', field: 'urgentFrozenSection' }));
  card.appendChild(boolField({ label: 'Suspected-cancer two-week-wait (2WW) pathway', section: 'urgency', field: 'twoWeekWait' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Requester & site',
    description: 'Care setting, requested-by date, and any notes.'
  });
  card.appendChild(grid('two-col', [
    selectInput({
...
function renderStep7() {
  const card = sectionCard({
    stepNumber: 7,
    title: 'Review & submit',
    description: 'Submit to compute the four-axis grade (appropriateness, specimen quality, completeness, urgency) and safety flags.'
  });
  const note = document.createElement('p');
  note.className = 'muted';
  note.textContent =
    'When you submit, the engine grades the request and lists any safety flags below. Nothing is sent until you press Submit.';
...
const TRACKED_FIELDS = [
  ...
  // 3 Specimen
  ['specimen', 'specimenType'], ['specimen', 'specimenSite'],
  ['specimen', 'fixative'],
  // 4 Indication
...
const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'specimen', 4: 'indication',
  5: 'urgency', 6: 'triage', 7: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, section: 'specimen',   title: 'Specimen' },
  { step: 4, section: 'indication', title: 'Indication' },
  { step: 5, section: 'urgency',    title: 'Urgency' },
  { step: 6, section: 'triage',     title: 'Site' },
  { step: 7, section: null,         title: 'Review' }
];
...
function renderReport() {
  ...
  const specimenValue = specimenQualityBand
    ? `<span class="band-badge quality-${esc(specimenQualityBand)}">${esc(titleCase(specimenQualityBand))}</span>`
  ...
  out.innerHTML = `
    <h2>Histopathology Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Specimen: ${esc(specimenTypeLabel(state.specimen.specimenType) || '—')}${state.indication.primaryIndication ? ` · ${esc(indicationLabel(state.indication.primaryIndication))}` : ''}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Specimen quality</span>
        <span class="axis-value">${specimenValue}</span>
      </div>
      <div class="axis-card">
      ...
      <div class="axis-card">
        <span class="axis-name">D · Urgency triage</span>
