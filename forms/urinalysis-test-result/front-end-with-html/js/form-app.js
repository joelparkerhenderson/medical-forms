...
// Urinalysis Test Result — reporting-clinician wizard
...
// inline structured urinalysis report. State is persisted to localStorage so
...
const STORAGE_KEY = 'urinalysis-test-result.front-end-with-html.v1';

// Numeric fields are `number | null` (null when unanswered), so the
// rehydration merge below cannot rely on a plain typeof comparison
// (typeof null === 'object' but a saved answer is a number).
const NUMERIC_FIELDS = new Set(['ph', 'specificGravity']);

/** @returns {import('./types.js').UrinalysisResult} */
...
function mergeIntoDefaults(parsed) {
  ...
  for (const key of Object.keys(fresh)) {
    ...
    if (NUMERIC_FIELDS.has(key)) {
      // null-safe: accept a finite number or null; anything else resets.
      fresh[key] = (typeof v === 'number' && Number.isFinite(v)) ? v : null;
...
/** @param {import('./types.js').UrinalysisResult} state */
...
/** @type {import('./types.js').UrinalysisResult} */
let state = loadState();
 ...
 * `null` (the repo convention for unanswered numeric fields); a parseable
...
function numberInput(opts) {
  ...
  input.addEventListener('input', () => {
    ...
    if (raw === '') {
      ...
      const n = Number(raw);
      setField(opts.field, Number.isFinite(n) ? n : null);
    }
    clearFieldError(id);
  });
  return wrapper;
...
const DIPSTICK_GRADE_OPTIONS = [
  { value: 'negative', label: 'Negative' },
  { value: 'trace', label: 'Trace' },
  { value: 'plus-one', label: '1+' },
  { value: 'plus-two', label: '2+' },
  { value: 'plus-three', label: '3+' }
];
...
function renderStep2() {
  const card = sectionCard({
    stepNumber: 2,
    title: 'Specimen',
    description: 'The specimen, its condition, and the clinical context.'
  });
  ...
  card.appendChild(selectInput({
    ...
    options: [
      { value: 'midstream', label: 'Midstream (MSU)' },
      { value: 'catheter', label: 'Catheter (CSU)' },
      { value: 'clean-catch', label: 'Clean catch' },
      { value: '24h', label: '24-hour collection' },
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'Clinical history and the question the investigation was performed to answer…'
  }));
  card.appendChild(checkboxGroup({
    label: 'Pregnancy',
    items: [
      { field: 'pregnant', label: 'Patient is pregnant' }
    ]
  }));
...
function renderStep3() {
  const card = sectionCard({
    stepNumber: 3,
    title: 'Dipstick (reagent strip)',
    description: 'Semi-quantitative reagent-strip results.'
  });

  card.appendChild(selectInput({
    label: 'Leucocytes',
    field: 'leucocytes',
    options: DIPSTICK_GRADE_OPTIONS
  }));
  card.appendChild(selectInput({
    label: 'Nitrites',
    field: 'nitrites',
  ...
  card.appendChild(numberInput({
    ...
    hint: 'Urine pH (approximately 4.0–9.0).'
  }));
  card.appendChild(numberInput({
    label: 'Specific gravity',
    field: 'specificGravity',
    min: 1, max: 1.06, step: 0.001,
    hint: 'Urine specific gravity (approximately 1.000–1.060).'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Microscopy',
    description: 'Microscopy counts and observed elements (free-text as reported).'
  });

  card.appendChild(textInput({
    label: 'Red cell count',
    field: 'redCellCount',
    placeholder: 'e.g. <10 x10^6/L'
  }));
  card.appendChild(textInput({
    label: 'White cell count',
    field: 'whiteCellCount',
    placeholder: 'e.g. >100 x10^6/L (pyuria)'
  }));
  card.appendChild(textInput({
    label: 'Epithelial cells',
    field: 'epithelialCells',
    placeholder: 'e.g. few / numerous (raised suggests contamination)'
  }));
  card.appendChild(textInput({
    label: 'Casts',
    field: 'casts',
    placeholder: 'e.g. hyaline, granular, red-cell casts'
  }));
  card.appendChild(textInput({
    label: 'Crystals',
    field: 'crystals',
    placeholder: 'e.g. calcium oxalate, urate'
  }));
  card.appendChild(checkboxGroup({
    label: 'Organisms',
    items: [
      { field: 'organismsSeen', label: 'Organisms (bacteria) seen on microscopy' }
    ]
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Culture & sensitivities',
    description: 'Culture outcome, organism, colony count, antibiotic sensitivities, and structured critical-finding flags.'
  });
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'mixed-growth-likely-contaminant', label: 'Mixed growth (likely contaminant)' },
  ...
  card.appendChild(textInput({
    label: 'Colony count',
    field: 'colonyCountCfuMl',
    placeholder: 'e.g. >10^5 cfu/mL'
  }));
  card.appendChild(textArea({
    label: 'Antibiotic sensitivities',
    field: 'antibioticSensitivities', rows: 3,
    placeholder: 'e.g. Trimethoprim R, Nitrofurantoin S'
  }));
  card.appendChild(checkboxGroup({
    label: 'Critical-finding flags',
    items: [
      { field: 'criticalOrganism', label: 'Critical organism (requires prompt action)' },
      { field: 'visibleHaematuria', label: 'Visible (frank) haematuria' },
      { field: 'suspectedUrosepsis', label: 'Findings suggesting urosepsis' }
    ]
  }));
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'Significant growth in pregnancy, a critical organism, suspected urosepsis, or visible haematuria auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the requester on sign-off.'
  }));
...
function renderStep6() {
  const card = sectionCard({
    ...
    description: 'The overall result status, narrative, impression, and follow-up.'
  });
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'Narrative description of the urinalysis findings (the body of the report)…'
  }));
  card.appendChild(textArea({
    label: 'Impression',
  ...
  card.appendChild(textInput({
    ...
    hint: 'Optional structured-reporting category label.',
    placeholder: 'e.g. significant E. coli bacteriuria'
  }));
  card.appendChild(textArea({
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'critical-finding-alert':
      (state.cultureResult === 'significant-growth' && state.pregnant) ||
      state.criticalOrganism ||
      state.visibleHaematuria ||
      state.suspectedUrosepsis,
...
// the optional-findings groups count as answered once any member is recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['specimenType'], ['specimenCondition'], ['clinicalHistory']],
  3: [['leucocytes'], ['nitrites'], ['protein'], ['blood'], ['glucose'],
      ['ketones'], ['bilirubin'], ['ph', 'specificGravity']],
  4: [['redCellCount'], ['whiteCellCount'], ['epithelialCells'],
      ['casts', 'crystals', 'organismsSeen']],
  5: [['cultureResult'],
      ['organismIsolated', 'colonyCountCfuMl', 'antibioticSensitivities']],
  6: [['overallResultStatus'], ['findingsNarrative'], ['impression'],
      ['reportingCategory', 'recommendedFollowUp']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Colony count', state.colonyCountCfuMl || 'Not recorded'],
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, title: 'Dipstick' },
  { step: 4, title: 'Microscopy' },
  { step: 5, title: 'Culture' },
