...
const STORAGE_KEY =
  'fluoroscopy-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 5;
...
const INDICATION_OPTIONS = [
  ...
  { value: 'suspected-obstruction', label: 'Suspected obstruction' },
  { value: 'suspected-perforation', label: 'Suspected perforation' },
  { value: 'constipation', label: 'Constipation' },
  { value: 'infertility-tubal-patency', label: 'Infertility / tubal patency' },
  { value: 'vesicoureteric-reflux', label: 'Vesicoureteric reflux' },
  { value: 'joint-assessment', label: 'Joint assessment' },
  { value: 'other', label: 'Other' }
];
...
function renderStep3() {
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Is there a stricture causing the dysphagia?'
  }));
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Pregnancy and radiation-safety context drive the safety band; a contraindication forces query / redirect.'
  });
  card.appendChild(selectInput({
  ...
  card.appendChild(boolField({ label: 'Known contrast-media allergy', section: 'safety', field: 'contrastAllergy' }));
  card.appendChild(boolField({ label: 'Aspiration risk', section: 'safety', field: 'aspirationRisk' }));
  ...
  card.appendChild(textArea({
    ...
    rows: 2,
    placeholder: 'Clinical rationale for exposing the patient to ionising radiation.'
  }));
  return card;
}
...
const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'request', 4: 'safety', 5: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 4, section: 'safety',    title: 'Safety' },
...
function renderReport() {
  ...
  const safetyValue = safetyBand
    ? `<span class="band-badge safety-${esc(safetyBand)}">${esc(titleCase(safetyBand))}</span> ${doseValue}`
    : `<span class="muted">Not assessed</span> ${doseValue}`;
  ...
  out.innerHTML = `
    <h2>Fluoroscopy Request Vetting Report</h2>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Safety + radiation dose</span>
