...
use crate::models::_entities::fluoroscopy_test_requests::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.contrast_allergy = Set(self.contrast_allergy);
      item.aspiration_risk = Set(self.aspiration_risk);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/fluoroscopy_test_requests/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
