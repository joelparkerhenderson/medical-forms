...
import { ecgTypeLabel, emptyRequest, indicationLabel } from './types.js';

// Electrocardiogram (ECG) Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'electrocardiogram-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
const INDICATION_OPTIONS = [
  ...
  { value: 'suspected-mi-acs', label: 'Suspected MI / ACS' },
  { value: 'pre-operative', label: 'Pre-operative' },
  { value: 'medication-monitoring-qt', label: 'Medication monitoring (QT)' },
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'ECG type, indication, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Requested ECG type', section: 'request', field: 'ecgType',
    options: ECG_TYPE_OPTIONS, required: true
  }));
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Exclude ischaemic change in recent-onset chest pain.'
  }));
  card.appendChild(textArea({ label: 'Relevant cardiac history', section: 'request', field: 'relevantHistory', rows: 2 }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Red flags auto-escalate the triage tier and clinical priority. Suspected ACS or active chest pain implies an emergency same-hour 12-lead ECG.'
  });
  ...
  card.appendChild(boolField({ label: 'Syncope / collapse', section: 'symptoms', field: 'symptomSyncope' }));
  card.appendChild(boolField({ label: 'Breathlessness', section: 'symptoms', field: 'symptomBreathlessness' }));
  card.appendChild(boolField({ label: 'Dizziness / light-headedness', section: 'symptoms', field: 'symptomDizziness' }));
  card.appendChild(subHead('Red flags'));
  card.appendChild(boolField({ label: 'Currently symptomatic at time of request', section: 'symptoms', field: 'currentlySymptomatic' }));
  card.appendChild(boolField({ label: 'Suspected acute coronary syndrome (ACS)', section: 'symptoms', field: 'suspectedAcs' }));
  card.appendChild(selectInput({
    label: 'Known arrhythmia', section: 'symptoms', field: 'knownArrhythmia',
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Medications',
    description: 'Relevant current medications, especially QT-prolonging or rate-controlling agents.'
  });
  card.appendChild(textArea({
    label: 'Relevant medications', section: 'medications', field: 'relevantMedications',
    rows: 3,
    placeholder: 'e.g. sotalol, amiodarone, bisoprolol, citalopram.'
  }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Triage',
    description: 'Requested urgency, setting, and site.'
  });
  card.appendChild(grid('two-col', [
  ...
  card.appendChild(grid('two-col', [
    ...
    textInput({ label: 'Site / location', section: 'triage', field: 'siteName' })
  ]));
  return card;
}
...
function renderStep7() {
  const card = sectionCard({
    stepNumber: 7,
    title: 'Review & submit',
    description: 'Add any notes. Submit to compute the four-axis grade, flags, and recommendation.'
  });
...
const TRACKED_FIELDS = [
  ...
  // 3 Request
  ['request', 'ecgType'], ['request', 'primaryIndication'],
...
const SECTION_BY_STEP = {
  ...
  5: 'medications', 6: 'triage', 7: 'triage'
};
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>ECG Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · ECG: ${esc(ecgTypeLabel(state.request.ecgType) || '—')}${state.request.primaryIndication ? ` · ${esc(indicationLabel(state.request.primaryIndication))}` : ''}</p>
