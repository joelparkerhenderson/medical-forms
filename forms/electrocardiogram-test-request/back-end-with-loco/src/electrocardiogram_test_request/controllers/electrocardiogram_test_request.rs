...
use crate::models::_entities::electrocardiogram_test_requests::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.symptom_dizziness = Set(self.symptom_dizziness);
      item.currently_symptomatic = Set(self.currently_symptomatic);
      item.suspected_acs = Set(self.suspected_acs);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/electrocardiogram_test_requests/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
