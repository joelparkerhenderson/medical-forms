...
Generate Loco (loco.rs) scaffolding for the `provider-transfer-request` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name provider-transfer-request --db postgres --bg async --assets serverside
   ```
