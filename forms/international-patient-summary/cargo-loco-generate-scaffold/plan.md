...
Generate Loco (loco.rs) scaffolding for the `international-patient-summary` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name international-patient-summary --db postgres --bg async --assets serverside
   ```
