...
struct DashboardParams {
    ...
    vaccination_level: Option<String>,
...
async fn dashboard(
    ...
    // Vaccination level filter
    if let Some(ref level) = params.vaccination_level {
        if !level.is_empty() {
            items.retain(|row| row.vaccination_level == *level);
        }
    }

    // Completeness filter
    if let Some(ref comp) = params.completeness {
        if !comp.is_empty() {
            items.retain(|row| row.completeness == *comp);
        }
    }
    ...
    context.insert("vaccination_level", &params.vaccination_level.unwrap_or_default());
