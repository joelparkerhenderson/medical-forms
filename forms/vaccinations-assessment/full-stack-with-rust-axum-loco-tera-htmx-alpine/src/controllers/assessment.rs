...
use crate::engine::{vaccination_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "childhood_vaccinations" => "childhoodVaccinations",
        "adult_vaccinations" => "adultVaccinations",
        "travel_vaccinations" => "travelVaccinations",
        "occupational_vaccinations" => "occupationalVaccinations",
        "contraindications_allergies" => "contraindicationsAllergies",
        "consent_information" => "consentInformation",
        "administration_record" => "administrationRecord",
...
async fn show_report(
    ...
    // Run grading engine
    let (vaccination_level, vaccination_score, fired_rules) =
        vaccination_grader::calculate_vaccination_status(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        vaccination_level: vaccination_level.clone(),
        vaccination_score,
    ...
    let completeness = crate::engine::utils::completeness_category(vaccination_score);

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("vaccination_level", &vaccination_level);
    context.insert("vaccination_label", &crate::engine::utils::vaccination_level_label(&vaccination_level));
    context.insert("vaccination_score", &vaccination_score);
    context.insert("completeness", &completeness);
