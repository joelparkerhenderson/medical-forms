...
use crate::models::_entities::echocardiogram_test_requests::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.breathlessness = Set(self.breathlessness);
      item.chest_pain = Set(self.chest_pain);
      item.palpitations = Set(self.palpitations);
      item.syncope = Set(self.syncope);
      item.oedema = Set(self.oedema);
      item.nyha_class = Set(self.nyha_class.clone());
      item.ecg_findings = Set(self.ecg_findings.clone());
      item.bnp_or_nt_probnp = Set(self.bnp_or_nt_probnp);
      item.known_murmur = Set(self.known_murmur);
      item.previous_echo = Set(self.previous_echo.clone());
      item.previous_echo_date = Set(self.previous_echo_date);
      item.ejection_fraction_known = Set(self.ejection_fraction_known);
      item.on_cardiotoxic_chemotherapy = Set(self.on_cardiotoxic_chemotherapy);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/echocardiogram_test_requests/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
