...
use crate::models::_entities::lumbar_puncture_test_request_grades::{ActiveModel, Entity, Model};

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Params {
    pub deleted_at: Option<DateTimeWithTimeZone>,
    pub lumbar_puncture_test_request_id: i64,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.lumbar_puncture_test_request_id = Set(self.lumbar_puncture_test_request_id);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/lumbar_puncture_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
