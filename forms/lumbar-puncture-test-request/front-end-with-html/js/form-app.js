...
import { emptyRequest, indicationLabel } from './types.js';

// Lumbar Puncture Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'lumbar-puncture-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 6;
...
const INTENT_OPTIONS = [
  { value: 'diagnostic', label: 'Diagnostic (CSF sampling / manometry)' },
  { value: 'therapeutic', label: 'Therapeutic (e.g. CSF drainage)' },
  { value: 'other', label: 'Other' }
];

const INDICATION_OPTIONS = [
  { value: 'suspected-meningitis', label: 'Suspected meningitis' },
  { value: 'suspected-subarachnoid-haemorrhage', label: 'Suspected subarachnoid haemorrhage' },
  { value: 'suspected-multiple-sclerosis', label: 'Suspected multiple sclerosis' },
  { value: 'suspected-guillain-barre', label: 'Suspected Guillain-Barré' },
  { value: 'idiopathic-intracranial-hypertension', label: 'Idiopathic intracranial hypertension' },
  { value: 'suspected-cns-malignancy', label: 'Suspected CNS malignancy' },
  { value: 'cns-infection', label: 'CNS infection' },
  { value: 'other', label: 'Other' }
];

const CT_HEAD_OPTIONS = [
  { value: 'done-normal', label: 'Done — normal' },
  { value: 'done-abnormal', label: 'Done — abnormal' },
...
function renderStep3() {
  const card = sectionCard({
    stepNumber: 3,
    title: 'Procedure & indication',
    description: 'Procedure intent, indication, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Procedure intent', section: 'procedure', field: 'procedureIntent',
    options: INTENT_OPTIONS, required: true
  }));
  card.appendChild(selectInput({
    label: 'Primary indication', section: 'procedure', field: 'primaryIndication',
    options: INDICATION_OPTIONS, required: true
  }));
  card.appendChild(textArea({
    label: 'Specific clinical question', section: 'procedure', field: 'clinicalQuestion',
    rows: 2, required: true,
    placeholder: 'e.g. Exclude bacterial meningitis; CSF cell count, protein, glucose, culture.'
  }));
  card.appendChild(textArea({ label: 'Relevant history', section: 'procedure', field: 'relevantHistory', rows: 2 }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Raised-ICP / neuro safety',
    description: 'Raised intracranial pressure requires imaging and stabilisation before LP.'
  });
  card.appendChild(subHead('Raised-ICP risk factors'));
  card.appendChild(boolField({ label: 'Suspected raised intracranial pressure', section: 'neuroSafety', field: 'suspectedRaisedIntracranialPressure' }));
  card.appendChild(boolField({ label: 'New focal neurological signs', section: 'neuroSafety', field: 'focalNeurologicalSigns' }));
  card.appendChild(boolField({ label: 'Reduced consciousness (e.g. GCS ≤ 9)', section: 'neuroSafety', field: 'reducedConsciousness' }));
  card.appendChild(selectInput({
    label: 'CT head status', section: 'neuroSafety', field: 'ctHeadStatus',
    options: CT_HEAD_OPTIONS
  }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Bleeding / coagulation safety',
    description: 'Anticoagulation, coagulopathy, thrombocytopenia, and local infection drive the contraindication band.'
  });
  card.appendChild(boolField({ label: 'Taking an anticoagulant', section: 'bleeding', field: 'takingAnticoagulant' }));
  card.appendChild(textInput({ label: 'Anticoagulant agent', section: 'bleeding', field: 'anticoagulantAgent', placeholder: 'e.g. warfarin, apixaban' }));
  card.appendChild(boolField({ label: 'Taking an antiplatelet agent', section: 'bleeding', field: 'takingAntiplatelet' }));
  card.appendChild(textInput({ label: 'Antiplatelet agent', section: 'bleeding', field: 'antiplateletAgent', placeholder: 'e.g. aspirin, clopidogrel' }));
  card.appendChild(grid('two-col', [
  ...
  card.appendChild(boolField({ label: 'Local skin / soft-tissue infection at puncture site', section: 'bleeding', field: 'localSkinInfection' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Procedure detail & triage',
    description: 'Opening-pressure requirement, requested urgency, setting, and notes. Submit to compute the four-axis grade and flags.'
  });
  card.appendChild(boolField({ label: 'Opening-pressure measurement (manometry) required', section: 'triage', field: 'openingPressureRequired' }));
  card.appendChild(grid('two-col', [
  ...
  card.appendChild(selectInput({
    label: 'Care setting', section: 'triage', field: 'setting', options: SETTING_OPTIONS
  }));
...
const TRACKED_FIELDS = [
  ...
  // 3 Procedure
  ['procedure', 'procedureIntent'], ['procedure', 'primaryIndication'],
  ['procedure', 'clinicalQuestion'],
  // 4 Neuro safety
  ['neuroSafety', 'ctHeadStatus'],
  // 6 Triage
  ['triage', 'urgency'], ['triage', 'setting']
];

const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'procedure', 4: 'neuroSafety',
  5: 'bleeding', 6: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 4, section: 'neuroSafety', title: 'Neuro safety' },
  { step: 5, section: 'bleeding',    title: 'Bleeding safety' },
...
function updateStepListStatuses(sectionAnswered, sectionTotal) {
  ...
  for (const def of STEP_DEFINITIONS) {
    ...
    // Bleeding safety has no tracked text fields; treat as informational.
    if (t > 0 && a === t) {
...
function renderReport() {
  ...
  const contraValue = contraindicationBand
    ? `<span class="band-badge contra-${esc(contraindicationBand)}">${esc(titleCase(contraindicationBand))}</span>`
  ...
  out.innerHTML = `
    <h2>Lumbar Puncture Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Indication: ${esc(indicationLabel(state.procedure.primaryIndication) || '—')}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Safety / contraindication</span>
        <span class="axis-value">${contraValue}</span>
      </div>
      <div class="axis-card">
