...
// inline structured laboratory-medicine report. State is persisted to
...
const STORAGE_KEY = 'tumor-marker-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').TumorMarkerResult} */
...
function mergeIntoDefaults(parsed) {
  ...
  // Measured-marker fields default to null (not measured) and accept a
  // stored number; other fields must match the default's type.
...
/** @param {import('./types.js').TumorMarkerResult} state */
...
/** @type {import('./types.js').TumorMarkerResult} */
let state = loadState();
 ...
 * `null` (not measured); a non-numeric entry also stores `null` so the
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'Specimen condition, clinical history, and any known cancer site.'
  });
  ...
  card.appendChild(textInput({
    label: 'Known cancer site',
    field: 'knownCancerSite',
    placeholder: 'e.g. ovary, testis, colorectal'
  }));
  ...
  card.appendChild(conditionalAlert({
    id: 'specimen-warning',
    type: 'warning',
    heading: 'Specimen condition may reduce assay validity',
    message: 'A haemolysed, lipaemic, or insufficient specimen may make the assay non-interpretable; consider repeating on a satisfactory specimen.'
  }));
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Enter each measured serum tumour marker. Leave a field blank when the marker was not measured.'
  });

  const markerHints = {
    psa: 'Prostate cancer; informed-choice testing.',
    ca125: 'Suspected ovarian cancer (NICE CG122 ultrasound threshold 35 IU/mL).',
    ca19_9: 'Pancreatic / hepatobiliary cancer (reference < 37 U/mL).',
    carcinoembryonicAntigenCea: 'Colorectal cancer monitoring.',
    alphaFetoproteinAfp: 'Hepatocellular / germ-cell tumours; a very high value suggests a germ-cell tumour.',
    betaHcg: 'Germ-cell / trophoblastic tumours; a very high value suggests a germ-cell tumour.',
    ca15_3: 'Breast cancer monitoring.',
    lactateDehydrogenaseLdh: 'Germ-cell tumour staging; lymphoma prognosis.',
    calcitonin: 'Medullary thyroid carcinoma.',
    chromograninA: 'Neuroendocrine tumours.'
  };
  ...
  card.appendChild(conditionalAlert({
    id: 'germ-cell-alert',
    type: 'error',
    heading: 'Very high germ-cell marker',
    message: 'A very high AFP or beta-hCG suggests a germ-cell tumour and is treated as a critical result; the follow-up urgency auto-escalates to a critical alert. Ensure the result is communicated to the requester on sign-off.'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Trend',
    description: 'Comparison with the previous value drives the monitoring / response interpretation.'
  });
  ...
  card.appendChild(numberInput({
    ...
    field: 'previousValue',
    min: 0, step: 0.1,
    hint: 'Most recent previous value of the principal marker, for trend interpretation.'
  }));
  card.appendChild(selectInput({
    label: 'Trend',
    field: 'trend',
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'Narrative comparison with previous tumour-marker results…'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Findings',
    description: 'The narrative findings, markedly-elevated flag, and overall result status.'
  });
  ...
  card.appendChild(conditionalAlert({
    id: 'action-signal-alert',
    type: 'warning',
    heading: 'Action signal present',
    message: 'A markedly elevated value or a rising trend on treatment classifies the result as abnormal and escalates the follow-up to an urgent oncology review.'
  }));
...
function renderStep6() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Structured-reporting category label (e.g. a germ-cell-marker pattern or monitoring-trend category).',
    placeholder: 'e.g. germ-cell-marker-pattern'
  }));
  card.appendChild(textArea({
    label: 'Recommended follow-up',
    field: 'recommendedFollowUp', rows: 3,
    placeholder: 'Recommended follow-up: repeat interval, referral, or further investigation…'
  }));
...
function renderStep7() {
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'This report contains a critical result (very high AFP / beta-hCG, or a reported critical status). Communicate the result directly to the requester and record it below before signing.'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'specimen-warning':
      ...
      state.specimenCondition === 'lipaemic' ||
      state.specimenCondition === 'insufficient',
    'germ-cell-alert': hasGermCellCriticalMarker(state),
    'action-signal-alert': hasActionSignal(state),
...
// measured-values step count as answered once any marker is recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['specimenCondition'], ['knownCancerSite'], ['clinicalHistory']],
  3: [['psa', 'ca125', 'ca19_9', 'carcinoembryonicAntigenCea',
       'alphaFetoproteinAfp', 'betaHcg', 'ca15_3', 'lactateDehydrogenaseLdh',
       'calcitonin', 'chromograninA']],
  4: [['previousValue'], ['trend'], ['comparisonWithPrevious']],
  5: [['findingsNarrative'], ['markedlyElevated', 'overallResultStatus']],
...
/** Render the measured-marker table rows for the report (only measured). */
function measuredValueRows() {
  return MARKERS
    .filter((m) => state[m.key] !== null && state[m.key] !== undefined)
    .map((m) => `
      <tr>
        <th scope="row">${esc(m.label)}</th>
        <td>${esc(String(state[m.key]))} ${esc(m.unit)}</td>
      </tr>
    `).join('');
}
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Known cancer site', state.knownCancerSite || 'Not recorded'],
    ['Trend', trendLabel(state.trend)],
  ...
  const markerRows = measuredValueRows();
  const measuredValuesSection = markerRows
    ? `
    ...
    : `
      <h3>Measured values</h3>
      <p class="muted">No tumour-marker value recorded.</p>
    `;
  ...
  out.innerHTML = `
    <div class="report-card">
      <header class="report-header">
        <h2>Tumour Marker Test Report</h2>
