...
-- Individual UKMEC rules that fired during grading.
...
CREATE TABLE grading_fired_rule (
    ...
    severity_level VARCHAR(10) NOT NULL DEFAULT ''
        CHECK (severity_level IN ('1', '2', '3', '4', '')),
...
COMMENT ON TABLE grading_fired_rule IS
    'Individual UKMEC eligibility rules that evaluated to true during grading.';
...
COMMENT ON COLUMN grading_fired_rule.rule_id IS
    'Identifier of the rule that fired (e.g. UKMEC-VTE-001, UKMEC-MIG-001).';
COMMENT ON COLUMN grading_fired_rule.category IS
    'Category of the rule (e.g. VTE Risk, Migraine, Cardiovascular, Breast Cancer).';
COMMENT ON COLUMN grading_fired_rule.description IS
    'Human-readable description of the rule.';
COMMENT ON COLUMN grading_fired_rule.severity_level IS
    'UKMEC category contributed by this rule: 1, 2, 3, 4, or empty.';
