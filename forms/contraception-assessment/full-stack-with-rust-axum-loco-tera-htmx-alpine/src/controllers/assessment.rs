...
use crate::engine::{eligibility_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "smoking_bmi" => "smokingBmi",
        "contraceptive_preferences" => "contraceptivePreferences",
        "ukmec_eligibility" => "ukmecEligibility",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "patientName"
            ...
            | "fertilityPlans"
            | "partnerInvolvement"
            | "clinicianOverrideNotes"
            | "methodExplained"
            | "risksBenefitsDiscussed"
            | "sideEffectsDiscussed"
            | "alternativeMethodsDiscussed"
            | "stiPreventionDiscussed"
            | "emergencyContraceptionDiscussed"
...
async fn show_report(
    ...
    // Run grading engine
    let (eligibility_level, ukmec_category, fired_rules) =
        eligibility_grader::calculate_eligibility(&assessment_data);
    ...
    let risk_level = crate::engine::utils::risk_level_for_category(Some(ukmec_category));
    ...
    context.insert("ukmec_category", &ukmec_category);
