...
import { abnormalitySeverityClass, abnormalitySeverityLabel, emptyResult, followUpUrgencyClass, followUpUrgencyLabel, priorityLabel, recommendationLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel, testTypeLabel } from './types.js';

// Allergy Skin Test Result — reporting-clinician wizard
...
// inline structured allergy report. State is persisted to localStorage so a
...
const STORAGE_KEY = 'allergy-skin-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').AllergySkinResult} */
...
/** @param {import('./types.js').AllergySkinResult} state */
...
/** @type {import('./types.js').AllergySkinResult} */
let state = loadState();
...
function readOnlyReadout(opts) {
  ...
  wrapper.innerHTML = `
    ...
    <div id="${opts.id}">${opts.render()}</div>
  `;
  return wrapper;
}
...
function renderStep1() {
  const card = sectionCard({
    ...
    description: 'Who authored the report, the originating request, the performed test type, and key dates.'
  });

  card.appendChild(textInput({
    label: 'Reporting clinician',
    field: 'reportingClinician', required: true,
    placeholder: 'e.g. Dr A Allergist'
  }));
  card.appendChild(textInput({
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'intradermal-test', label: 'Intradermal test' },
      { value: 'patch-test', label: 'Patch test' },
      { value: 'specific-ige-blood', label: 'Specific-IgE blood' },
      { value: 'drug-provocation-challenge', label: 'Drug-provocation challenge' },
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The clinical history and the question the test was performed to answer.'
  });

  card.appendChild(textArea({
    label: 'Clinical history',
    field: 'clinicalHistory', rows: 4,
    placeholder: 'Clinical history, suspected triggers, and the question the test was performed to answer…'
  }));
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Pre-analytic validity controls. A skin-prick / intradermal test is only interpretable after an adequate antihistamine washout and with a valid positive histamine control.'
  });

  card.appendChild(checkboxGroup({
    label: 'Validity controls',
    items: [
      { field: 'antihistaminesWithheld', label: 'Antihistamines withheld for an adequate washout' },
      { field: 'positiveControlValid', label: 'Positive histamine control produced an adequate weal' }
    ]
  }));
  ...
  card.appendChild(conditionalAlert({
    id: 'validity-warning',
    type: 'warning',
    heading: 'Validity controls incomplete',
    message: 'Without an adequate antihistamine washout and a valid positive control, the test may be non-interpretable. Mark the test invalid on the reaction-summary step if appropriate.'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Allergens and reactions',
    description: 'The allergens tested and their measured reactions (weal sizes in mm and specific-IgE in kUA/L).'
  });

  card.appendChild(textArea({
    label: 'Allergens tested',
    field: 'allergensTested', rows: 3,
    placeholder: 'e.g. house dust mite, cat, peanut, egg, milk, amoxicillin…'
  }));
  card.appendChild(textArea({
    label: 'Weal sizes',
    field: 'whealSizes', rows: 3,
    hint: 'Measured weal sizes per allergen in millimetres, with positive (histamine) and negative (saline) controls.',
    placeholder: 'e.g. peanut 8 mm; cat 0 mm; histamine 6 mm; saline 0 mm…'
  }));
  card.appendChild(textArea({
    label: 'Specific-IgE results',
    field: 'specificIgeResults', rows: 3,
    hint: 'Specific-IgE blood (sIgE) results per allergen in kUA/L, where performed.',
    placeholder: 'e.g. peanut sIgE 14 kUA/L; Ara h 2 9 kUA/L…'
  }));
  card.appendChild(textArea({
    label: 'Sensitised allergens',
    field: 'sensitisedAllergens', rows: 2,
    placeholder: 'Allergens to which sensitisation was demonstrated…'
  }));
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'Structured reaction summary. A positive reaction demonstrates sensitisation, not necessarily clinical allergy; confirm clinical relevance against the history.'
  });

  card.appendChild(checkboxGroup({
    label: 'Structured reaction summary',
    items: [
      { field: 'positiveReactions', label: 'Positive reactions' },
      { field: 'sensitisationConfirmed', label: 'Clinically relevant sensitisation confirmed' },
      { field: 'anaphylaxisDuringTest', label: 'Anaphylaxis during test' },
      { field: 'allNegative', label: 'All allergens negative' },
      { field: 'testInvalid', label: 'Test invalid / non-interpretable' }
    ]
  }));

  card.appendChild(conditionalAlert({
    id: 'anaphylaxis-alert',
    type: 'error',
    heading: 'Critical event selected',
    message: 'Anaphylaxis during the test auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
  card.appendChild(conditionalAlert({
    id: 'invalid-alert',
    type: 'warning',
    heading: 'Invalid test selected',
    message: 'An invalid / non-interpretable test classifies the result as inconclusive.'
  }));
...
function renderStep6() {
  const card = sectionCard({
    ...
    description: 'The clinical interpretation (sensitisation versus clinically relevant allergy), the summary impression, a structured-reporting category, and recommended follow-up.'
  });

  card.appendChild(textArea({
    label: 'Interpretation',
    field: 'interpretation', rows: 4,
    placeholder: 'Interpretation of the reactions in the context of the history (sensitisation vs clinically relevant allergy)…'
  }));
  card.appendChild(textArea({
    label: 'Impression',
  ...
  card.appendChild(textInput({
    ...
    hint: 'Optional structured-reporting category label (e.g. a sensitisation pattern or component-resolved diagnostics summary).',
    placeholder: 'e.g. peanut-component sensitisation (Ara h 2 positive)'
  }));
  card.appendChild(textArea({
    label: 'Recommended follow-up',
    field: 'recommendedFollowUp', rows: 3,
    placeholder: 'Allergen avoidance advice, oral food / drug challenge, immunotherapy referral, or further testing…'
  }));
...
function renderStep7() {
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'This report contains a critical event (anaphylaxis during the test). Communicate the result directly to the referrer and record it below before signing.'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'validity-warning': !state.antihistaminesWithheld || !state.positiveControlValid,
    'anaphylaxis-alert': state.anaphylaxisDuringTest,
    'invalid-alert': state.testInvalid,
...
// answered once any structured finding or validity control is recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['testType'],
      ['reportStatus'], ['performedDate'], ['reportedDate']],
  2: [['clinicalHistory']],
  3: [['antihistaminesWithheld', 'positiveControlValid']],
  4: [['allergensTested'], ['whealSizes', 'specificIgeResults'], ['sensitisedAllergens']],
  5: [['positiveReactions', 'sensitisationConfirmed', 'anaphylaxisDuringTest',
       'allNegative', 'testInvalid']],
  6: [['interpretation'], ['impression'], ['reportingCategory'], ['recommendedFollowUp']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Sensitised allergens', state.sensitisedAllergens || 'Not recorded'],
  ...
  out.innerHTML = `
    <div class="report-card">
      <header class="report-header">
        <h2>Allergy Skin Test Report</h2>
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, title: 'Validity' },
  { step: 4, title: 'Allergens' },
  { step: 5, title: 'Reactions' },
  { step: 6, title: 'Interpretation' },
