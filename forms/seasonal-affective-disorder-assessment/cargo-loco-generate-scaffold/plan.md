...
Generate Loco (loco.rs) scaffolding for the `seasonal-affective-disorder-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name seasonal-affective-disorder-assessment --db postgres --bg async --assets serverside
   ```
