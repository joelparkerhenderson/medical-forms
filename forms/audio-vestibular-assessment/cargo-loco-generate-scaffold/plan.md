...
Generate Loco (loco.rs) scaffolding for the `audio-vestibular-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name audio-vestibular-assessment --db postgres --bg async --assets serverside
   ```
