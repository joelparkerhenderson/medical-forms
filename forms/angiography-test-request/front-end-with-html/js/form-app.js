...
const STORAGE_KEY =
  'angiography-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
const ANGIOGRAPHY_TYPE_OPTIONS = [
...
const INDICATION_OPTIONS = [
  { value: 'suspected-coronary-disease', label: 'Suspected coronary disease' },
  ...
  { value: 'suspected-pulmonary-embolism', label: 'Suspected pulmonary embolism' },
  { value: 'gi-bleeding', label: 'GI bleeding' },
  { value: 'pre-intervention-planning', label: 'Pre-intervention planning' },
...
function renderStep1() {
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'interventional-radiologist', label: 'Interventional radiologist' },
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Modality, body region, indication, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(grid('two-col', [
    selectInput({
      label: 'Angiography type', section: 'request', field: 'angiographyType',
      options: ANGIOGRAPHY_TYPE_OPTIONS, required: true
    }),
    selectInput({
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Contrast requirement, renal function, allergy, diabetes, and metformin — drives the safety axis.'
  });
  card.appendChild(grid('two-col', [
  ...
  card.appendChild(boolField({ label: 'Previous contrast-media allergy / reaction', section: 'contrast', field: 'contrastAllergy' }));
  card.appendChild(boolField({ label: 'Diabetes mellitus', section: 'contrast', field: 'diabetes' }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Bleeding & anticoagulation',
    description: 'Anticoagulation, antiplatelet, and bleeding-disorder status relevant to arterial access.'
  });
  ...
  card.appendChild(boolField({ label: 'Taking an antiplatelet', section: 'bleeding', field: 'takingAntiplatelet' }));
  card.appendChild(boolField({ label: 'Known bleeding disorder', section: 'bleeding', field: 'bleedingDisorder' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Pregnancy & radiation',
    description: 'Pregnancy status and IR(ME)R justification for any ionising-radiation examination.'
  });
  card.appendChild(selectInput({ label: 'Pregnancy status', section: 'pregnancy', field: 'pregnancyStatus', options: PREGNANCY_OPTIONS }));
  card.appendChild(textArea({
    label: 'IR(ME)R justification', section: 'pregnancy', field: 'irMeRJustification', rows: 2,
    placeholder: 'Justification for the radiation exposure under IR(ME)R 2017.'
  }));
  return card;
}
...
const TRACKED_FIELDS = [
  ...
  // 3 Request
  ['request', 'angiographyType'], ['request', 'bodyRegion'],
...
const SECTION_BY_STEP = {
  ...
  5: 'bleeding', 6: 'pregnancy', 7: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 5, section: 'bleeding',  title: 'Bleeding' },
  { step: 6, section: 'pregnancy', title: 'Pregnancy' },
...
function updateStepListStatuses(sectionAnswered, sectionTotal) {
  ...
  for (const def of STEP_DEFINITIONS) {
    ...
    // Steps with no tracked text fields (bleeding) are treated as informational.
    if (t > 0 && a === t) {
...
function renderReport() {
  ...
  const examLabel = angiographyTypeLabel(state.request.angiographyType) || '—';
  const regionLabel = bodyRegionLabel(state.request.bodyRegion);

  out.innerHTML = `
    <h2>Angiography Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Exam: ${esc(examLabel)}${regionLabel ? ` · ${esc(regionLabel)}` : ''}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Contrast / radiation safety</span>
