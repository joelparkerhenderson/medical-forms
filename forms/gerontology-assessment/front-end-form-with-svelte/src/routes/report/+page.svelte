<script lang="ts">
	...
	import { cfsScoreLabel, cfsScoreColor, bmiCategory, calculateAge } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `gerontology-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
	...
	<div class="min-h-screen bg-gray-50">
		<header class="border-b border-gray-200 bg-white shadow-sm no-print">
			<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
				<h1 class="text-lg font-bold text-gray-900">Gerontology Assessment Report</h1>
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			<!-- CFS Score Banner -->
			<div class="mb-6 rounded-xl border-2 p-6 text-center {cfsScoreColor(result.cfsScore)}">
				<div class="text-3xl font-bold">CFS {result.cfsScore}</div>
				<div class="mt-1 text-lg">{cfsScoreLabel(result.cfsScore)}</div>
			...
			{#if result.additionalFlags.length > 0}
				<div class="mb-6 rounded-xl border border-red-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-red-800">Flagged Issues for Geriatrician</h2>
					<div class="space-y-2">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">CFS Score Justification</h2>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				...
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					...
					<div>
						<span class="font-medium text-gray-600">Living Situation:</span>
						{data.demographics.livingSituation || 'N/A'}
					</div>
				</div>
			</div>
