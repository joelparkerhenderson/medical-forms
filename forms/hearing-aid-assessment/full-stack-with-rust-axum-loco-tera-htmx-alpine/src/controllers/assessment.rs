...
use crate::engine::{hearing_aid_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "current_hearing_aids" => "currentHearingAids",
        "fitting_requirements" => "fittingRequirements",
        "expectations_goals" => "expectationsGoals",
...
async fn show_report(
    ...
    // Run grading engine
    let (hearing_aid_level, hearing_aid_score, fired_rules) =
        hearing_aid_grader::calculate_hearing_aid_level(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        hearing_aid_level: hearing_aid_level.clone(),
        hearing_aid_score,
    ...
    let hearing_loss_cat = crate::engine::utils::hearing_loss_category(
        Some(std::cmp::max(
            assessment_data.audiometric_results.right_ear_pta.unwrap_or(0),
            assessment_data.audiometric_results.left_ear_pta.unwrap_or(0),
        )),
    );
    ...
    context.insert("hearing_aid_level", &hearing_aid_level);
    context.insert("hearing_aid_label", &crate::engine::utils::hearing_aid_level_label(&hearing_aid_level));
    context.insert("hearing_aid_score", &hearing_aid_score);
    context.insert("hearing_loss_category", &hearing_loss_cat);
