...
Generate Loco (loco.rs) scaffolding for the `hormone-replacement-therapy-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name hormone-replacement-therapy-assessment --db postgres --bg async --assets serverside
   ```
