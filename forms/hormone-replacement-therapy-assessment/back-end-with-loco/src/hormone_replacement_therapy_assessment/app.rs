...
impl Hooks for App {
    ...
    fn routes(_ctx: &AppContext) -> AppRoutes {
        AppRoutes::with_default_routes() // controller routes below
            ...
            .add_route(controllers::assessment_current_medications::routes())
    ...
    async fn seed(ctx: &AppContext, _base: &Path) -> Result<()> {
        db::seed::<users::ActiveModel>(
            &ctx.db,
            &format!("{}/src/hormone_replacement_therapy_assessment/fixtures/users.yaml", env!("CARGO_MANIFEST_DIR")),
        )
            .await?;
        Ok(())
    }
