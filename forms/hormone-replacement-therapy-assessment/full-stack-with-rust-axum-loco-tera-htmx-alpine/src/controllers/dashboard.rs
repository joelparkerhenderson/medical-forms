...
struct DashboardParams {
    ...
    menopausal_status: Option<String>,
}
...
async fn dashboard(
    ...
    if let Some(ref term) = params.search {
        ...
        items.retain(|row| {
            row.patient_name.to_lowercase().contains(&term)
                || row.patient_age.to_lowercase().contains(&term)
                || row.review_date.to_lowercase().contains(&term)
        });
    }
    ...
    if let Some(ref status) = params.menopausal_status {
        if !status.is_empty() {
            items.retain(|row| row.menopausal_status == *status);
        }
    }

    // Sort by review date descending
    items.sort_by(|a, b| b.review_date.cmp(&a.review_date));
    ...
    context.insert("menopausal_status", &params.menopausal_status.unwrap_or_default());
