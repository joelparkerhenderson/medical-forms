...
import { abnormalitySeverityClass, abnormalitySeverityLabel, anaesthesiaLabel, complicationLabel, emptyResult, followUpUrgencyClass, followUpUrgencyLabel, priorityLabel, procedureLabel, recommendationLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel, tumourAppearanceLabel } from './types.js';

// Cystoscopy Test Result — reporting-clinician wizard
...
// inline structured cystoscopy report. State is persisted to localStorage so
...
const STORAGE_KEY = 'cystoscopy-test-result.front-end-with-html.v1';

// Fields whose canonical unanswered value is `null` (numbers), not `''`.
// `typeof null === 'object'`, so the generic same-type rehydration check
// below would always reject them; they get a null-safe number check instead.
const NUMERIC_FIELDS = ['tumourSizeMm'];

/** @returns {import('./types.js').CystoscopyResult} */
...
function mergeIntoDefaults(parsed) {
  ...
  for (const key of Object.keys(fresh)) {
    ...
    if (NUMERIC_FIELDS.includes(key)) {
      // null-safe: accept a finite number or null; anything else stays null.
      const v = parsed[key];
...
/** @param {import('./types.js').CystoscopyResult} state */
...
/** @type {import('./types.js').CystoscopyResult} */
let state = loadState();
...
/**
 * Numeric input bound to a `number | null` field: an empty box maps to `null`
 * (never `''` or `NaN`), and a stored `null` renders as an empty box.
 */
...
function renderStep1() {
  ...
  card.appendChild(textInput({
    ...
    placeholder: 'e.g. Dr A Urologist'
  }));
  card.appendChild(textInput({
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The performed cystoscopy procedure and anaesthesia.'
  });
...
function renderStep4() {
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'bladderTumour', label: 'Bladder tumour / suspicious lesion' },
      { field: 'inflammationCystitis', label: 'Inflammation / cystitis' },
      { field: 'bladderStones', label: 'Bladder stones' },
      { field: 'urethralStricture', label: 'Urethral stricture' },
      { field: 'trabeculation', label: 'Trabeculation' },
      { field: 'prostaticEnlargement', label: 'Prostatic enlargement' },
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'A bladder tumour or suspicious lesion auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off and book urgent TURBT / MDT referral.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Tumour and complication',
    description: 'Tumour / lesion detail and any procedure complication.'
  });

  card.appendChild(numberInput({
    label: 'Tumour size (mm)',
    field: 'tumourSizeMm',
    min: 0, max: 500, step: 0.1,
    hint: 'Largest tumour / lesion size, for categorisation and surveillance.'
  }));
  card.appendChild(selectInput({
    label: 'Tumour appearance',
    field: 'tumourAppearance',
    options: [
  ...
  card.appendChild(checkboxField({
    label: 'A biopsy / sample was taken',
    field: 'biopsyTaken'
  }));
  card.appendChild(selectInput({
    label: 'Complication',
...
function renderStep6() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Structured-reporting category (e.g. an EAU NMIBC risk group or a suspected-tumour category).',
    placeholder: 'e.g. EAU NMIBC high-risk'
  }));
  card.appendChild(textArea({
    label: 'Recommended follow-up',
    field: 'recommendedFollowUp', rows: 3,
    placeholder: 'Surveillance cystoscopy interval, referral, or further management…'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'critical-finding-alert': state.bladderTumour,
...
// the checkbox-only slots count as answered once any structured finding is
// recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['procedure'], ['anaesthesia']],
  3: [['clinicalHistory']],
  4: [['findingsNarrative'],
      ['bladderTumour', 'inflammationCystitis', 'bladderStones', 'urethralStricture',
       'trabeculation', 'prostaticEnlargement', 'normalExamination']],
  5: [['tumourSizeMm', 'tumourAppearance', 'biopsyTaken'], ['complication']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Anaesthesia', anaesthesiaLabel(state.anaesthesia)],
    ...
    ['Tumour size (mm)', state.tumourSizeMm === null ? 'Not recorded' : String(state.tumourSizeMm)],
    ['Tumour appearance', tumourAppearanceLabel(state.tumourAppearance)],
