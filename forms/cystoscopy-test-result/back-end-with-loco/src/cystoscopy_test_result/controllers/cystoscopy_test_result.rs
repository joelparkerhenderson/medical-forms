...
use crate::models::_entities::cystoscopy_test_results::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.anaesthesia = Set(self.anaesthesia.clone());
      item.clinical_history = Set(self.clinical_history.clone());
      item.bladder_tumour = Set(self.bladder_tumour);
      item.inflammation_cystitis = Set(self.inflammation_cystitis);
      item.bladder_stones = Set(self.bladder_stones);
      item.urethral_stricture = Set(self.urethral_stricture);
      item.trabeculation = Set(self.trabeculation);
      item.prostatic_enlargement = Set(self.prostatic_enlargement);
      item.normal_examination = Set(self.normal_examination);
      item.tumour_size_mm = Set(self.tumour_size_mm);
      item.tumour_appearance = Set(self.tumour_appearance.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/cystoscopy_test_results/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
