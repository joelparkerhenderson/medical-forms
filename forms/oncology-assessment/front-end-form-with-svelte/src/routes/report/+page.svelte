	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `oncology-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			...
			{#if result.additionalFlags.length > 0}
				<div class="mb-6 rounded-xl border border-red-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-red-800">Flagged Issues for Oncologist</h2>
					<div class="space-y-2">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">ECOG Grade Justification</h2>
			...
			<!-- Cancer Diagnosis -->
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				<h2 class="mb-4 text-lg font-bold text-gray-900">Cancer Diagnosis</h2>
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					<div>
						<span class="font-medium text-gray-600">Cancer Type:</span>
						{cancerTypeLabel(data.cancerDiagnosis.cancerType)}
					</div>
					<div>
						<span class="font-medium text-gray-600">Primary Site:</span>
						{data.cancerDiagnosis.primarySite || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">TNM:</span>
						{formatTNM(data.cancerDiagnosis.stageT, data.cancerDiagnosis.stageN, data.cancerDiagnosis.stageM)}
					</div>
					<div>
						<span class="font-medium text-gray-600">Overall Stage:</span>
						{stageLabel(data.cancerDiagnosis.overallStage)}
					</div>
					<div>
					...
					<div>
						...
						{data.cancerDiagnosis.dateOfDiagnosis || 'N/A'}
					</div>
				</div>
			</div>
		</main>
