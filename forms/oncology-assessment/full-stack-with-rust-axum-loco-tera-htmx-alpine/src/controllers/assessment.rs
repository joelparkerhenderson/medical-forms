...
use crate::engine::{oncology_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "staging_grading" => "stagingGrading",
        "treatment_history" => "treatmentHistory",
        "current_treatment" => "currentTreatment",
        "side_effects_toxicity" => "sideEffectsToxicity",
        "performance_status" => "performanceStatus",
        "psychosocial_assessment" => "psychosocialAssessment",
        "palliative_care_needs" => "palliativeCareNeeds",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "patientName"
            ...
            | "goalsOfCareDiscussed"
            | "hospiceReferralIndicated"
            | "endOfLifePlanning"
...
async fn show_report(
    ...
    // Run grading engine
    let (oncology_level, oncology_score, fired_rules) =
        oncology_grader::calculate_oncology(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        oncology_level: oncology_level.clone(),
        oncology_score,
    ...
    let stage_cat = crate::engine::utils::stage_category(
        &assessment_data.staging_grading.overall_stage,
    );

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("oncology_level", &oncology_level);
    context.insert("oncology_label", &crate::engine::utils::oncology_level_label(&oncology_level));
    context.insert("oncology_score", &oncology_score);
    context.insert("stage_category", &stage_cat);
    context.insert("overall_stage", &assessment_data.staging_grading.overall_stage);
    context.insert("ecog_score", &assessment_data.performance_status.ecog_score);
