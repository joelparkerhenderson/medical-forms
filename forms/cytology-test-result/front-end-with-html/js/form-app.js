...
// inline structured cytology report. State is persisted to localStorage so a
...
const STORAGE_KEY = 'cytology-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').CytologyResult} */
...
function mergeIntoDefaults(parsed) {
  ...
  // and any legacy null values rehydrate to the typed defaults.
...
/** @param {import('./types.js').CytologyResult} state */
...
/** @type {import('./types.js').CytologyResult} */
let state = loadState();
...
function renderStep1() {
  ...
  card.appendChild(textInput({
    ...
    placeholder: 'e.g. Dr A Cytopathologist'
  }));
  card.appendChild(textInput({
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The examined specimen type and its adequacy for interpretation.'
  });
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'fluid-pleural-ascitic', label: 'Serous fluid (pleural / ascitic)' },
      { value: 'fine-needle-aspiration-thyroid', label: 'Thyroid FNA' },
      { value: 'fine-needle-aspiration-breast', label: 'Breast FNA' },
  ...
  card.appendChild(conditionalAlert({
    id: 'adequacy-warning',
    type: 'warning',
    heading: 'Unsatisfactory specimen',
    message: 'An unsatisfactory specimen classifies the result as inconclusive; a repeat specimen is recommended.'
  }));
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'The clinical question and any comparison with previous cytology / histology.'
  });
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'Relevant previous cytology / histology results and changes since…'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Findings',
    description: 'The cytology result category, HPV status, headline findings, and microscopic description.'
  });

  card.appendChild(textArea({
    label: 'Cytology result category',
    field: 'cytologyResultCategory', rows: 2,
    hint: 'The recognised grading category for the specimen type (e.g. negative / borderline / low-grade / high-grade dyskaryosis; Thy1–Thy5; C1–C5).',
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      ...
      { field: 'dysplasiaPresent', label: 'Dysplasia / dyskaryosis present' }
    ]
  }));
  card.appendChild(textArea({
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'Malignant cells auto-escalate the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off, with urgent colposcopy / MDT referral.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Diagnosis and category',
    description: 'The cytological diagnosis and the structured-reporting grading category.'
  });

  card.appendChild(textArea({
    label: 'Diagnosis',
    field: 'diagnosis', rows: 3,
    placeholder: 'Cytological diagnosis / interpretation…'
  }));
  card.appendChild(textInput({
    label: 'Reporting (grading) category',
    field: 'reportingCategory',
    hint: 'The structured-reporting grading label (e.g. an NHS cervical-screening dyskaryosis grade, RCPath Thy or breast C category).',
    placeholder: 'e.g. high-grade dyskaryosis (severe), Thy5, C5…'
  }));
...
function renderStep6() {
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'Repeat cytology, colposcopy, biopsy, MDT referral, or routine recall…'
  }));
...
function renderStep7() {
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'This report contains a critical finding. Communicate the result directly to the referrer, refer for urgent colposcopy / MDT review, and record it below before signing.'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'adequacy-warning': state.specimenAdequacy === 'unsatisfactory',
    'malignancy-alert': state.malignancyPresent,
...
// counts only when ticked). This lets the headline-findings checkboxes count
// as answered once either finding is recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['specimenType'], ['specimenAdequacy']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['cytologyResultCategory'], ['hpvResult'],
      ['malignancyPresent', 'dysplasiaPresent'], ['microscopicDescription']],
  5: [['diagnosis'], ['reportingCategory']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['HPV result', hpvResultLabel(state.hpvResult)],
  ...
  out.innerHTML = `
    <div class="report-card">
      ...
      ${state.diagnosis ? `<p class="muted">Diagnosis: ${esc(state.diagnosis)}</p>` : ''}
