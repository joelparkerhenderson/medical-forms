...
use crate::models::_entities::x_ray_test_request_grades::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.x_ray_test_request_id = Set(self.x_ray_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.radiation_safety_band = Set(self.radiation_safety_band.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/x_ray_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
