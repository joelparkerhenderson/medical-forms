...
use crate::models::_entities::x_ray_test_requests::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.recent_similar_xray = Set(self.recent_similar_xray);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/x_ray_test_requests/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
