...
const STORAGE_KEY = 'pet-scan-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
const SCAN_TYPE_OPTIONS = [
  { value: 'fdg-pet-ct', label: 'FDG-PET-CT ([18F]FDG)' },
...
const INDICATION_OPTIONS = [
  ...
  { value: 'cancer-restaging', label: 'Cancer restaging' },
  { value: 'treatment-response', label: 'Treatment response' },
  { value: 'suspected-recurrence', label: 'Suspected recurrence' },
  { value: 'solitary-pulmonary-nodule', label: 'Solitary pulmonary nodule' },
  { value: 'lymphoma', label: 'Lymphoma' },
  { value: 'cardiac-viability', label: 'Cardiac viability' },
  { value: 'infection-inflammation', label: 'Infection / inflammation' },
  { value: 'neurology-dementia', label: 'Neurology — dementia' },
  { value: 'other', label: 'Other' }
];
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'Patient demographics, weight, and care setting.'
  });
  card.appendChild(grid('two-col', [
  ...
  card.appendChild(grid('two-col', [
    textInput({ label: 'Weight', section: 'patient', field: 'weightKg', type: 'number', min: 0, max: 500, step: 0.1, unit: 'kg' }),
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Tracer / scan type, indication, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Requested scan type / tracer', section: 'request', field: 'scanType',
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Stage biopsy-proven NSCLC and assess for distant metastases.'
  }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Primary tumour site, relevant history, and recent treatment.'
  });
  card.appendChild(textInput({ label: 'Primary tumour site', section: 'context', field: 'primaryTumourSite', placeholder: 'e.g. right upper lobe lung' }));
  card.appendChild(textArea({ label: 'Relevant history', section: 'context', field: 'relevantHistory', rows: 2 }));
  card.appendChild(textArea({ label: 'Recent chemotherapy / radiotherapy', section: 'context', field: 'recentChemoRadiotherapy', rows: 2, placeholder: 'Agent and dates — affects timing and uptake.' }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'Diabetes, blood glucose, pregnancy, breastfeeding, and renal function drive the preparation-safety axis.'
  });
  card.appendChild(boolField({ label: 'Diabetes', section: 'preparation', field: 'diabetes' }));
  card.appendChild(grid('two-col', [
    textInput({ label: 'Blood glucose', section: 'preparation', field: 'bloodGlucoseMmolL', type: 'number', min: 0, max: 60, step: 0.1, unit: 'mmol/L' }),
    textInput({ label: 'eGFR', section: 'preparation', field: 'egfr', type: 'number', min: 0, max: 200, step: 0.1, unit: 'mL/min/1.73m²' })
  ]));
  card.appendChild(selectInput({ label: 'Pregnancy status', section: 'preparation', field: 'pregnancyStatus', options: PREGNANCY_OPTIONS }));
  card.appendChild(subHead('Safety'));
  card.appendChild(boolField({ label: 'Breastfeeding', section: 'preparation', field: 'breastfeeding' }));
  card.appendChild(boolField({ label: 'Claustrophobia', section: 'preparation', field: 'claustrophobia' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Radiation justification',
    description: 'IR(ME)R justification statement and requested urgency.'
  });
  card.appendChild(textArea({
    label: 'IR(ME)R justification statement', section: 'justification', field: 'irMeRJustification',
    rows: 3,
    placeholder: 'State why the benefit of the radiation exposure outweighs the dose.'
  }));
  card.appendChild(selectInput({ label: 'Requested urgency', section: 'justification', field: 'urgency', options: URGENCY_OPTIONS, required: true }));
  return card;
}
...
function renderStep7() {
  const card = sectionCard({
    stepNumber: 7,
    title: 'Triage & submit',
    description: 'Requested-by date and notes. Submit to compute the four-axis grade and flags.'
  });
  card.appendChild(textInput({ label: 'Requested-by date', section: 'triage', field: 'requestedByDate', type: 'date' }));
...
const TRACKED_FIELDS = [
  ...
  // 3 Request
  ['request', 'scanType'], ['request', 'primaryIndication'],
  ['request', 'clinicalQuestion'],
  // 4 Context
  ['context', 'primaryTumourSite'], ['context', 'relevantHistory'],
  // 5 Preparation
  ['preparation', 'bloodGlucoseMmolL'], ['preparation', 'pregnancyStatus'],
  ['preparation', 'egfr'],
  // 6 Justification
  ['justification', 'irMeRJustification'], ['justification', 'urgency'],
  // 7 Triage
  ['triage', 'requestedByDate']
];

const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'request', 4: 'context',
  5: 'preparation', 6: 'justification', 7: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 5, section: 'preparation',   title: 'Preparation' },
  { step: 6, section: 'justification', title: 'Justification' },
...
function renderReport() {
  ...
  const prepValue = prepSafetyBand
    ? `<span class="band-badge prep-${esc(prepSafetyBand)}">${esc(titleCase(prepSafetyBand))}</span>`
    : '<span class="muted">Not assessed</span>';

  const doseValue = radiationDoseBand
    ? `<span class="band-badge dose-${esc(radiationDoseBand)}">${esc(titleCase(radiationDoseBand))}</span>`
    : '<span class="muted">Not assessed</span>';
  ...
  out.innerHTML = `
    <h2>PET-CT Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Scan: ${esc(scanTypeLabel(state.request.scanType) || '—')}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Preparation safety &amp; radiation dose</span>
        <span class="axis-value">
          ${prepValue}
          ${doseValue}
        </span>
