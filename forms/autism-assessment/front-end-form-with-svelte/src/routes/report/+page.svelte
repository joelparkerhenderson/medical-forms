<script lang="ts">
	...
	import { aq10ScoreLabel, aq10ScoreColor, calculateAge, ageGroupLabel, referralSourceLabel } from '$lib/engine/utils';
...
{#if result}
	<div class="min-h-screen bg-gray-50">
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			<!-- AQ-10 Score Banner -->
			<div class="mb-6 rounded-xl border-2 p-6 text-center {aq10ScoreColor(result.aq10Score)}">
				<div class="text-3xl font-bold">AQ-10 {result.aq10Score}/10</div>
				<div class="mt-1 text-lg">{result.aq10Category}</div>
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">AQ-10 Score Breakdown</h2>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				...
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					...
					<div>
						<span class="font-medium text-gray-600">Age Group:</span>
						{data.demographics.ageGroup ? ageGroupLabel(data.demographics.ageGroup) : 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Referral Source:</span>
						{data.screeningPurpose.referralSource ? referralSourceLabel(data.screeningPurpose.referralSource) : 'N/A'}
					</div>
					<div class="sm:col-span-2">
						<span class="font-medium text-gray-600">Reason for Screening:</span>
						{data.screeningPurpose.reasonForScreening || 'N/A'}
					</div>
				</div>
			</div>
			...
			{#if data.currentSupport.currentTherapies.length > 0 || data.currentSupport.medications.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">Current Support</h2>
					{#if data.currentSupport.currentTherapies.length > 0}
						<h3 class="text-sm font-semibold text-gray-600">Therapies</h3>
						<ul class="list-disc space-y-1 pl-5 text-sm mb-3">
							{#each data.currentSupport.currentTherapies as therapy}
								<li>{therapy}</li>
							{/each}
						</ul>
					{/if}
					{#if data.currentSupport.medications.length > 0}
						<h3 class="text-sm font-semibold text-gray-600">Medications</h3>
						<ul class="list-disc space-y-1 pl-5 text-sm">
							{#each data.currentSupport.medications as med}
