...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "restricted_repetitive_behaviours" => "restrictedRepetitiveBehaviours",
        "sensory_processing" => "sensoryProcessing",
        "aq10_screening" => "aq10Screening",
        "daily_living_skills" => "dailyLivingSkills",
        "mental_health_comorbidities" => "mentalHealthComorbidities",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "patientName"
            ...
            | "speechDelay"
            | "motorDelay"
            | "socialPlayDifferences"
            | "earlyRepetitiveBehaviours"
            | "ageFirstConcerns"
            | "previousAssessments"
            | "developmentalNotes"
            | "communicationPreference"
            | "specialInterestDescription"
            | "sensoryOverloadFrequency"
            | "sensoryCopingStrategies"
            | "executiveFunctionDifficulties"
            | "selfHarmRisk"
            | "currentMedications"
...
/// Parse a form value, keeping string fields as strings and converting scale fields to numbers.
...
async fn show_report(
    ...
    // Run grading engine
    let (likelihood_level, aq10_score, fired_rules) =
    ...
    let support_level = crate::engine::utils::support_level_label(
        &assessment_data.support_needs.support_level_needed,
    );
    ...
    context.insert("aq10_score", &aq10_score);
    context.insert("support_level", &support_level);
