...
use crate::engine::{ophthalmology_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "intraocular_pressure" => "intraocularPressure",
        "posterior_segment" => "posteriorSegment",
        "visual_fields" => "visualFields",
        "ocular_motility" => "ocularMotility",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "examDate"
            ...
            | "measurementTime"
            | "tonometryMethod"
            | "centralCornealThicknessRight"
            | "centralCornealThicknessLeft"
            | "gonioscopyRight"
            | "gonioscopyLeft"
            | "rightCupDiscRatio"
            | "leftCupDiscRatio"
            ...
            | "rightMeanDeviation"
            | "leftMeanDeviation"
            | "rightPatternStandardDeviation"
            | "leftPatternStandardDeviation"
            | "visualFieldTestType"
            | "visualFieldNotes"
            | "coverTestDistance"
            | "coverTestNear"
            | "relativeAfferentPupilDefect"
            | "stereopsis"
            | "octPerformed"
            | "octRightFindings"
            | "octLeftFindings"
            | "fundusPhotoPerformed"
            | "ffaPerformed"
            | "ffaFindings"
            | "cornealTopographyPerformed"
            | "cornealTopographyFindings"
            | "biometryPerformed"
            | "biometryFindings"
            | "otherInvestigations"
            | "primaryDiagnosis"
...
/// Parse a form value, keeping string fields as strings and converting clinical grading fields to numbers.
...
async fn show_report(
    ...
    // Run grading engine
    let (impairment_level, impairment_score, fired_rules) =
        ophthalmology_grader::calculate_impairment(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        impairment_level: impairment_level.clone(),
        impairment_score,
    ...
    let right_iop_risk = crate::engine::utils::iop_risk(
        assessment_data.intraocular_pressure.right_iop,
    );
    let left_iop_risk = crate::engine::utils::iop_risk(
        assessment_data.intraocular_pressure.left_iop,
    );

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("impairment_level", &impairment_level);
    context.insert("impairment_label", &crate::engine::utils::impairment_level_label(&impairment_level));
    context.insert("impairment_score", &impairment_score);
    context.insert("right_iop_risk", &right_iop_risk);
    context.insert("left_iop_risk", &left_iop_risk);
