...
use crate::models::_entities::pulmonary_function_test_request_grades::{ActiveModel, Entity, Model};

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Params {
    pub deleted_at: Option<DateTimeWithTimeZone>,
    pub pulmonary_function_test_request_id: i64,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.pulmonary_function_test_request_id = Set(self.pulmonary_function_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.contraindication_band = Set(self.contraindication_band.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/pulmonary_function_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
