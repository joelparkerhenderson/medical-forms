...
// Pulmonary Function Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'pulmonary-function-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
const INDICATION_OPTIONS = [
  { value: 'suspected-asthma', label: 'Suspected asthma' },
  { value: 'suspected-copd', label: 'Suspected COPD' },
  { value: 'breathlessness', label: 'Breathlessness' },
  { value: 'chronic-cough', label: 'Chronic cough' },
  { value: 'pre-operative', label: 'Pre-operative' },
  { value: 'occupational-lung-disease', label: 'Occupational lung disease' },
  { value: 'monitoring', label: 'Monitoring' },
  { value: 'restrictive-disease', label: 'Restrictive disease' },
  { value: 'other', label: 'Other' }
];
...
function renderStep1() {
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'physiologist', label: 'Respiratory physiologist' },
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'Patient demographics and anthropometry (height and weight drive predicted values).'
  });
  card.appendChild(grid('two-col', [
  ...
  card.appendChild(grid('two-col', [
    textInput({ label: 'Height', section: 'patient', field: 'heightCm', type: 'number', min: 50, max: 250, step: 0.1, unit: 'cm' }),
    textInput({ label: 'Weight', section: 'patient', field: 'weightKg', type: 'number', min: 2, max: 350, step: 0.1, unit: 'kg' })
  ]));
  return card;
}
...
function renderStep3() {
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Confirm airflow obstruction and bronchodilator reversibility.'
  }));
  card.appendChild(textArea({ label: 'Relevant history', section: 'request', field: 'relevantHistory', rows: 2, placeholder: 'Respiratory, medical, and occupational history.' }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Symptoms',
    description: 'Respiratory symptoms relevant to the requested test.'
  });
  card.appendChild(boolField({ label: 'Breathlessness', section: 'symptoms', field: 'breathlessness' }));
  card.appendChild(boolField({ label: 'Cough', section: 'symptoms', field: 'cough' }));
  card.appendChild(boolField({ label: 'Wheeze', section: 'symptoms', field: 'wheeze' }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Background',
    description: 'Smoking status and current inhalers / respiratory medication.'
  });
  card.appendChild(selectInput({ label: 'Smoking status', section: 'background', field: 'smokingStatus', options: SMOKING_OPTIONS }));
  card.appendChild(textArea({
    label: 'Current inhalers / respiratory medication', section: 'background', field: 'currentInhalers',
    rows: 2,
    placeholder: 'Relevant to withhold instructions before reversibility testing.'
  }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Safety screen',
    description: 'Forced-expiration and infection-control contraindications downgrade the safety band and may defer the test.'
  });
  card.appendChild(subHead('Contraindications'));
  card.appendChild(boolField({ label: 'Recent MI or recent eye / thoracic / abdominal surgery', section: 'safety', field: 'recentMiOrEyeAbdominalSurgery' }));
  card.appendChild(boolField({ label: 'Haemoptysis of unknown origin', section: 'safety', field: 'haemoptysis' }));
  card.appendChild(subHead('Infection control'));
  card.appendChild(boolField({ label: 'Recent / active respiratory infection', section: 'safety', field: 'recentRespiratoryInfection' }));
  card.appendChild(boolField({ label: 'Suspected active tuberculosis', section: 'safety', field: 'suspectedActiveTuberculosis' }));
  return card;
}
...
const SECTION_BY_STEP = {
  ...
  5: 'background', 6: 'safety', 7: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, section: 'request',    title: 'Test' },
...
function updateStepListStatuses(sectionAnswered, sectionTotal) {
  ...
  for (const def of STEP_DEFINITIONS) {
    ...
    // Symptoms and safety have no tracked text fields; treat as informational.
    if (t > 0 && a === t) {
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>Pulmonary Function Test Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Test: ${esc(testTypeLabel(state.request.testType) || '—')}</p>
