...
Generate Loco (loco.rs) scaffolding for the `workplace-safety-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name workplace-safety-assessment --db postgres --bg async --assets serverside
   ```
