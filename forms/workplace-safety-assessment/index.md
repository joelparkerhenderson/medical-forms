# Workplace Safety Assessment

Workplace safety audit form aligned with UK Health and Safety Executive (HSE) standards, covering physical, chemical, biological, ergonomic, and organisational risks to identify hazards and verify control measures in healthcare settings.

## Scoring system

- **Instrument**: HSE Workplace Safety Audit Checklist
- **Range**: Compliant / Minor Findings / Major Findings / Critical Findings
- **Categories**:
  - Compliant: All controls in place, no action required
  - Minor Findings: Low-risk gaps, action within 90 days
  - Major Findings: Moderate-risk gaps, action within 30 days
  - Critical Findings: Imminent risk, immediate corrective action and RIDDOR reporting if applicable

## Steps

| #   | Step                             |
| --- | -------------------------------- |
| 1   | Demographics & Site Details      |
| 2   | PPE & Hazard Controls            |
| 3   | Chemical & Biological Hazards    |
| 4   | Electrical Safety                |
| 5   | Fire Safety & Emergency Egress   |
| 6   | Ergonomics & Manual Handling     |
| 7   | Emergency Procedures             |
| 8   | Training & Competence            |
| 9   | Incident Reporting & Near Misses |
| 10  | Sign-off & Action Plan           |

## Directory structure

```
workplace-safety-assessment/
  front-end-patient-form-with-svelte/
  front-end-clinician-dashboard-with-svelte/
```

## Technology

See [root index.md](../index.md) for technology stacks.
