<script lang="ts">
	import type { AllergyItem, AllergyReactionSeverity } from '$lib/engine/types';

	let {
		allergies = $bindable<AllergyItem[]>([])
	}: {
		allergies: AllergyItem[];
	} = $props();

	const severityOptions: { value: AllergyReactionSeverity; label: string }[] = [
	...
	function addAllergy() {
		allergies = [...allergies, { allergen: '', reactionType: '', severity: '', timing: '', alternativesTolerated: '' }];
	}
		...
		<div class="flex items-start gap-2 rounded-lg border border-gray-200 bg-gray-50 p-3">
			<div class="grid flex-1 grid-cols-1 gap-2 sm:grid-cols-3">
				...
				<input
					...
					bind:value={allergy.reactionType}
					class="rounded border border-gray-300 px-2 py-1.5 text-sm focus:border-primary focus:outline-none"
				/>
				<select
