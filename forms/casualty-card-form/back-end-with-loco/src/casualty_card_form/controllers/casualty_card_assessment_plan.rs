...
use crate::models::_entities::casualty_card_assessment_plans::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.differential_diagnoses = Set(self.differential_diagnoses.clone());
      item.clinical_impression = Set(self.clinical_impression.clone());
      item.risk_stratification = Set(self.risk_stratification.clone());
      }
}
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/casualty_card_assessment_plans/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
