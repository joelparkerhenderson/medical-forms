...
use crate::models::_entities::gen_practice_waiting_list_card_appointments::{ActiveModel, Entity, Model};

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Params {
    pub deleted_at: Option<DateTimeWithTimeZone>,
    pub gen_practice_waiting_list_card_id: i64,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.gen_practice_waiting_list_card_id = Set(self.gen_practice_waiting_list_card_id);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/gen_practice_waiting_list_card_appointments/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
