...
use crate::engine::utils::copd_gold_stage;
...
impl PatientRow {
    ...
    pub fn from_model(m: &Model) -> Option<Self> {
        ...
        let gold = copd_gold_stage(data.pulmonary_function_tests.fev1_percent_predicted);

        Some(Self {
            id: m.id.to_string(),
            patient_name: data.patient_information.patient_name,
            assessment_date: data.patient_information.assessment_date,
            primary_complaint: data.patient_information.primary_complaint,
            severity_level: result.severity_level,
            severity_score: result.severity_score,
            smoking_status: data.smoking_exposure.smoking_status,
            gold_stage: gold.to_string(),
        })
    }
}
