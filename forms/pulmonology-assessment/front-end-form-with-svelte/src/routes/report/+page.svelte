<script lang="ts">
	...
	import { goldStageLabel, goldStageColor, abcdGroupLabel, bmiCategory, calculateAge } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `pulmonology-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
	...
	<div class="min-h-screen bg-gray-50">
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			...
			<div class="mb-6 rounded-xl border-2 p-6 text-center {goldStageColor(result.goldStage)}">
				...
				<div class="mt-2 text-sm font-medium">ABCD Group: {result.abcdGroup} - {abcdGroupLabel(result.abcdGroup)}</div>
			...
			{#if result.additionalFlags.length > 0}
				<div class="mb-6 rounded-xl border border-red-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-red-800">Flagged Issues for Pulmonologist</h2>
					<div class="space-y-2">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">GOLD Stage Justification</h2>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				...
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					...
					{#if data.spirometry.fev1PercentPredicted !== null}
						<div>
							<span class="font-medium text-gray-600">FEV1 % Predicted:</span>
							{data.spirometry.fev1PercentPredicted}%
						</div>
					{/if}
					{#if data.spirometry.fev1FvcRatio !== null}
						<div>
							<span class="font-medium text-gray-600">FEV1/FVC Ratio:</span>
							{data.spirometry.fev1FvcRatio}
						</div>
					{/if}
				</div>
			</div>
