...
-- Smoking and exposures section of the pulmonology assessment.
...
CREATE TABLE assessment_smoking_exposures (
    ...
    coal_dust_exposure VARCHAR(5) NOT NULL DEFAULT ''
        CHECK (coal_dust_exposure IN ('yes', 'no', '')),
    biomass_fuel_exposure VARCHAR(5) NOT NULL DEFAULT ''
        CHECK (biomass_fuel_exposure IN ('yes', 'no', '')),
    chemical_exposure VARCHAR(5) NOT NULL DEFAULT ''
        CHECK (chemical_exposure IN ('yes', 'no', '')),
    chemical_details TEXT NOT NULL DEFAULT '',
    smoking_cessation_advice VARCHAR(5) NOT NULL DEFAULT ''
        CHECK (smoking_cessation_advice IN ('yes', 'no', '')),
    cessation_support_offered VARCHAR(5) NOT NULL DEFAULT ''
        CHECK (cessation_support_offered IN ('yes', 'no', '')),
...
COMMENT ON TABLE assessment_smoking_exposures IS
    'Smoking and exposures section: smoking history, pack-years, vaping, occupational and environmental exposures, and cessation support. One-to-one child of assessment.';
...
COMMENT ON COLUMN assessment_smoking_exposures.pack_years IS
    'Cumulative smoking exposure in pack-years (packs per day x years smoked).';
COMMENT ON COLUMN assessment_smoking_exposures.cigarettes_per_day IS
    'Number of cigarettes smoked per day (current or former).';
COMMENT ON COLUMN assessment_smoking_exposures.age_started_smoking IS
    'Age when the patient started smoking.';
COMMENT ON COLUMN assessment_smoking_exposures.age_stopped_smoking IS
    'Age when the patient stopped smoking, if former smoker.';
...
COMMENT ON COLUMN assessment_smoking_exposures.silica_exposure IS
    'Whether the patient has had silica dust exposure.';
COMMENT ON COLUMN assessment_smoking_exposures.biomass_fuel_exposure IS
    'Whether the patient has had biomass fuel exposure (wood, dung, crop residue).';
COMMENT ON COLUMN assessment_smoking_exposures.smoking_cessation_advice IS
    'Whether smoking cessation advice was given.';
