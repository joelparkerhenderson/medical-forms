...
Generate Loco (loco.rs) scaffolding for the `medical-language-speaking-assessment-for-cymraeg` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name medical-language-speaking-assessment-for-cymraeg --db postgres --bg async --assets serverside
   ```
