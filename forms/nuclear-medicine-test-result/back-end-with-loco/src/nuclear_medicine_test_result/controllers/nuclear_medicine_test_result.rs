...
use crate::models::_entities::nuclear_medicine_test_results::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.radiopharmaceutical = Set(self.radiopharmaceutical.clone());
      ...
      item.abnormal_uptake = Set(self.abnormal_uptake);
      item.metastatic_pattern = Set(self.metastatic_pattern);
      item.perfusion_defect = Set(self.perfusion_defect);
      item.photopenic_area = Set(self.photopenic_area);
      item.no_significant_abnormality = Set(self.no_significant_abnormality);
      item.incidental_finding = Set(self.incidental_finding);
      item.ejection_fraction_percent = Set(self.ejection_fraction_percent);
      item.split_function_left_percent = Set(self.split_function_left_percent);
      item.split_function_right_percent = Set(self.split_function_right_percent);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/nuclear_medicine_test_results/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
