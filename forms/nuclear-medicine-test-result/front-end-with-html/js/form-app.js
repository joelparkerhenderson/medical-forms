...
import { abnormalitySeverityClass, abnormalitySeverityLabel, emptyResult, examinationAdequacyLabel, followUpUrgencyClass, followUpUrgencyLabel, priorityLabel, recommendationLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel, scanTypeLabel } from './types.js';

// Nuclear Medicine Test Result — reporting-clinician wizard
...
// inline structured nuclear medicine report. State is persisted to
...
const STORAGE_KEY = 'nuclear-medicine-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').NuclearMedicineResult} */
...
/** @param {import('./types.js').NuclearMedicineResult} state */
...
/** @type {import('./types.js').NuclearMedicineResult} */
let state = loadState();
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The performed examination, radiopharmaceutical, injected activity, and adequacy.'
  });
  ...
  card.appendChild(textInput({
    ...
    field: 'radiopharmaceutical',
    placeholder: 'e.g. Tc-99m MDP, Tc-99m MAA, Tc-99m sestamibi, Ga-68 DOTATATE'
  }));
  card.appendChild(textInput({
    label: 'Injected activity (MBq)',
    field: 'injectedActivityMbq', type: 'number',
    min: 0, max: 100000, step: 0.1,
    hint: 'Administered activity in megabecquerels, for dose audit (DRL comparison).'
  }));
  card.appendChild(selectInput({
...
function renderStep4() {
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'abnormalUptake', label: 'Abnormal uptake' },
      { field: 'metastaticPattern', label: 'Metastatic pattern' },
      { field: 'perfusionDefect', label: 'Perfusion defect' },
      { field: 'photopenicArea', label: 'Photopenic area' },
      { field: 'noSignificantAbnormality', label: 'No significant abnormality' },
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'A high-probability PE on a V/Q lung scan, or a widespread metastatic pattern, auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Measurements',
    description: 'Key quantitative measurements for gated cardiac and renal studies.'
  });

  card.appendChild(textInput({
    label: 'Ejection fraction (%)',
    field: 'ejectionFractionPercent', type: 'number',
    min: 0, max: 100, step: 0.1,
    hint: 'Left-ventricular ejection fraction, where gated cardiac studies are performed (0–100).'
  }));
  card.appendChild(textInput({
    label: 'Split function — left kidney (%)',
    field: 'splitFunctionLeftPercent', type: 'number',
    min: 0, max: 100, step: 0.1,
    hint: 'Left-kidney split (differential) function percentage, for renal studies (0–100).'
  }));
  card.appendChild(textInput({
    label: 'Split function — right kidney (%)',
    field: 'splitFunctionRightPercent', type: 'number',
    min: 0, max: 100, step: 0.1,
    hint: 'Right-kidney split (differential) function percentage, for renal studies (0–100).'
  }));
...
function renderStep6() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Structured-reporting category, e.g. a V/Q PE probability (normal / very-low / intermediate / high-probability) or a bone-scan pattern.',
    placeholder: 'e.g. high-probability'
  }));
  card.appendChild(textArea({
...
function updateConditionalAlerts() {
  ...
  const criticalSelected =
    (state.scanType === 'vq-lung-scan' && state.perfusionDefect) ||
    state.metastaticPattern;
  const visibility = {
...
// counts only when ticked). This lets the checkbox-only findings group count
// as answered once any structured finding is recorded, and the measurements
// step count once any measurement is entered.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['scanType'], ['radiopharmaceutical'], ['injectedActivityMbq'],
      ['examinationAdequacy']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['findingsNarrative'],
      ['abnormalUptake', 'metastaticPattern', 'perfusionDefect',
       'photopenicArea', 'noSignificantAbnormality', 'incidentalFinding']],
  5: [['ejectionFractionPercent', 'splitFunctionLeftPercent',
       'splitFunctionRightPercent']],
...
/** Format a nullable numeric measurement for the report. */
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Radiopharmaceutical', state.radiopharmaceutical || 'Not recorded'],
    ['Injected activity', fmtMeasurement(state.injectedActivityMbq, ' MBq')],
    ['Examination adequacy', examinationAdequacyLabel(state.examinationAdequacy)],
    ['Ejection fraction', fmtMeasurement(state.ejectionFractionPercent, ' %')],
    ['Split function — left kidney', fmtMeasurement(state.splitFunctionLeftPercent, ' %')],
    ['Split function — right kidney', fmtMeasurement(state.splitFunctionRightPercent, ' %')],
