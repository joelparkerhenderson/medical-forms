<script lang="ts">
	...
	import { ccsClassLabel, nyhaClassLabel, riskLevelLabel, riskLevelColor, bmiCategory, calculateAge } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `cardiology-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
	...
	<div class="min-h-screen bg-gray-50">
		<header class="border-b border-gray-200 bg-white shadow-sm no-print">
			<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
				<h1 class="text-lg font-bold text-gray-900">Cardiology Assessment Report</h1>
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			...
			<div class="mb-6 rounded-xl border-2 p-6 text-center {riskLevelColor(result.overallRisk)}">
				...
				<div class="mt-2 flex justify-center gap-6 text-sm">
					{#if result.ccsClass}
						<span>{ccsClassLabel(result.ccsClass)}</span>
					{/if}
					{#if result.nyhaClass}
						<span>{nyhaClassLabel(result.nyhaClass)}</span>
					{/if}
				</div>
			...
			{#if result.additionalFlags.length > 0}
				<div class="mb-6 rounded-xl border border-red-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-red-800">Flagged Issues for Cardiologist</h2>
					<div class="space-y-2">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">Risk Assessment Justification</h2>
			...
			{#if data.allergies.allergies.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					...
					<ul class="list-disc space-y-1 pl-5 text-sm">
						{#each data.allergies.allergies as allergy}
							<li>
