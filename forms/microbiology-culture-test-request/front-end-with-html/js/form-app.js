...
const STORAGE_KEY =
  'microbiology-culture-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 5;
 ...
 * selected tests sits above the group. Bound to the boolean `tests.*` fields.
 */
function testCheckboxGroup() {
  ...
  for (const t of TEST_FIELDS) {
    ...
    label.htmlFor = cbId;
    const tag = t.primary
      ? ' <span class="test-tag test-tag-primary">most common</span>'
      : '';
    label.innerHTML = `
    ...
    input.addEventListener('change', () => {
      setBool('tests', t.field, input.checked);
      updateTestCount();
...
function updateTestCount() {
  ...
  el.textContent = n === 0
    ? 'No tests selected yet — select at least one (culture & sensitivity is the most common).'
...
const INDICATION_OPTIONS = [
  { value: 'suspected-sepsis', label: 'Suspected sepsis' },
  ...
  { value: 'sti-screen', label: 'STI screen' },
  { value: 'pyrexia-unknown-origin', label: 'Pyrexia of unknown origin' },
...
function renderStep3() {
  const card = sectionCard({
    stepNumber: 3,
    title: 'Specimen & requested tests',
    description: 'Specimen type and site, collection status, and the requested microbiology tests.'
  });
  card.appendChild(selectInput({
    label: 'Specimen type', section: 'specimen', field: 'specimenType',
    options: SPECIMEN_TYPE_OPTIONS, required: true
  }));
  card.appendChild(textInput({
    label: 'Specimen site detail', section: 'specimen', field: 'specimenSiteDetail',
    placeholder: 'e.g. left leg ulcer, midstream urine'
  }));
  card.appendChild(grid('two-col', [
    selectInput({
      label: 'Specimen collected?', section: 'specimen', field: 'specimenCollected',
      options: COLLECTED_OPTIONS
    }),
    textInput({
      label: 'Collection date / time', section: 'specimen', field: 'collectionDatetime',
      type: 'datetime-local'
    })
  ]));
  card.appendChild(subHead('Requested tests'));
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Indication and clinical details drive appropriateness and triage — the highest-value fields.'
  });
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Febrile, rigors, suspected line infection. Relevant history.'
  }));
  card.appendChild(subHead('Patient-safety factors'));
  card.appendChild(boolField({ label: 'Fever / pyrexia', section: 'clinical', field: 'fever' }));
  card.appendChild(boolField({ label: 'Currently on antibiotics', section: 'clinical', field: 'currentAntibiotics' }));
  card.appendChild(textInput({ label: 'Antibiotic name(s)', section: 'clinical', field: 'antibioticName' }));
  card.appendChild(boolField({ label: 'Relevant recent travel', section: 'clinical', field: 'recentTravel' }));
  card.appendChild(boolField({ label: 'Immunocompromised', section: 'clinical', field: 'immunocompromised' }));
  return card;
}
...
function updateProgress() {
  ...
  // The requested-tests checkbox group counts as a tracked "specimen" field.
  sectionTotal.specimen = (sectionTotal.specimen || 0) + 1;
  if (countSelectedTests(state.tests) > 0) {
    answered++;
    sectionAnswered.specimen = (sectionAnswered.specimen || 0) + 1;
  }
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, section: 'specimen',  title: 'Specimen & tests' },
  { step: 4, section: 'clinical',  title: 'Clinical context' },
  { step: 5, section: 'triage',    title: 'Triage' }
];
...
function validateForm() {
  ...
  // At least one requested test must be selected.
  if (countSelectedTests(state.tests) === 0) {
    errors.push({ id: 'tests-fieldset', message: 'Select at least one requested test' });
    const el = document.getElementById('tests-error');
    if (el) el.textContent = 'Select at least one requested test';
  } else {
    const el = document.getElementById('tests-error');
    if (el) el.textContent = '';
  }
...
function selectedTestLabels() {
  return TEST_FIELDS
    .filter((t) => state.tests[t.field] === true)
    .map((t) => t.label);
}
...
function renderReport() {
  ...
  const testList = selectedTestLabels();
  const testsValue = testList.length === 0
    ? '<span class="muted">None selected</span>'
    : testList.map((l) => `<span class="subscale-chip">${esc(l)}</span>`).join('');
  ...
  out.innerHTML = `
    <h2>Microbiology Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Specimen: ${esc(specimenTypeLabel(state.specimen.specimenType) || '—')} · Indication: ${esc(indicationLabel(state.clinical.primaryIndication) || '—')}</p>
    ...
    <div class="subscale-chips">${testsValue}</div>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Pre-analytical safety</span>
