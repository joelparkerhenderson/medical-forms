...
use crate::models::_entities::microbiology_culture_test_request_grades::{ActiveModel, Entity, Model};

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Params {
    pub deleted_at: Option<DateTimeWithTimeZone>,
    pub microbiology_culture_test_request_id: i64,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.microbiology_culture_test_request_id = Set(self.microbiology_culture_test_request_id);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/microbiology_culture_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
