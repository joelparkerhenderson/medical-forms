<script lang="ts">
	...
	import { vaGradeLabel, vaGradeColor, calculateAge, iopStatusLabel, iopStatusColor } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `ophthalmology-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			...
			{#if result.additionalFlags.length > 0}
				<div class="mb-6 rounded-xl border border-red-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-red-800">Flagged Issues for Ophthalmologist</h2>
					<div class="space-y-2">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">VA Grade Justification</h2>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				...
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					...
					<div>
						<span class="font-medium text-gray-600">Affected Eye:</span>
						{data.chiefComplaint.affectedEye}
					</div>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				<h2 class="mb-4 text-lg font-bold text-gray-900">Visual Acuity</h2>
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					<div>
						<span class="font-medium text-gray-600">Right Eye (corrected):</span>
						{data.visualAcuity.distanceVaRightCorrected || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Left Eye (corrected):</span>
						{data.visualAcuity.distanceVaLeftCorrected || 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">IOP Right:</span>
					...
					<div>
						<span class="font-medium text-gray-600">IOP Left:</span>
			...
			{#if data.currentMedications.eyeDrops.length > 0 || data.currentMedications.oralMedications.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					...
					{#if data.currentMedications.eyeDrops.length > 0}
						<h3 class="mb-2 font-medium text-gray-700">Eye Drops</h3>
						<ul class="mb-3 list-disc space-y-1 pl-5 text-sm">
							{#each data.currentMedications.eyeDrops as med}
								<li>{med.name} {med.dose} {med.frequency}</li>
							{/each}
						</ul>
					{/if}
					{#if data.currentMedications.oralMedications.length > 0}
						<h3 class="mb-2 font-medium text-gray-700">Oral Medications</h3>
						<ul class="list-disc space-y-1 pl-5 text-sm">
							{#each data.currentMedications.oralMedications as med}
								<li>{med.name} {med.dose} {med.frequency}</li>
							{/each}
						</ul>
					{/if}
			...
			{#if data.currentMedications.ophthalmicDrugAllergies.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">Ophthalmic Drug Allergies</h2>
					<ul class="list-disc space-y-1 pl-5 text-sm">
						{#each data.currentMedications.ophthalmicDrugAllergies as allergy}
							<li>
