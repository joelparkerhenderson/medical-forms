...
struct DashboardParams {
    ...
    adverse_incident: Option<String>,
}
...
async fn dashboard(
    ...
    if let Some(ref term) = params.search {
        ...
        items.retain(|row| {
            row.nhs_number.to_lowercase().contains(&term)
                ...
                || row.surgery_procedure.to_lowercase().contains(&term)
        });
    }

    // ASA grade filter
    if let Some(grade) = params.asa_grade {
        items.retain(|row| row.asa_grade == grade);
    }
    ...
    // Adverse incident flag filter
    if let Some(ref adverse) = params.adverse_incident {
        match adverse.as_str() {
            "yes" => items.retain(|row| row.previous_adverse_incident),
            "no" => items.retain(|row| !row.previous_adverse_incident),
            _ => {}
        }
    }
    ...
    context.insert("asa_grade", &params.asa_grade.map(|g| g.to_string()).unwrap_or_default());
    context.insert("allergy", &params.allergy.unwrap_or_default());
    context.insert("adverse_incident", &params.adverse_incident.unwrap_or_default());
