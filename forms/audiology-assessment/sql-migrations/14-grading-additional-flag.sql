...
-- Additional flags detected during audiology assessment grading.
...
COMMENT ON TABLE grading_additional_flag IS
    'Additional flags detected during audiology assessment grading (e.g. asymmetric hearing loss requiring MRI, sudden onset, pulsatile tinnitus, Meniere suspicion).';
...
COMMENT ON COLUMN grading_additional_flag.flag_id IS
    'Identifier of the flag (e.g. FLAG-ASYM-001, FLAG-SUD-001, FLAG-MEN-001).';
COMMENT ON COLUMN grading_additional_flag.category IS
    'Category of the flag (e.g. Asymmetric Loss, Sudden Onset, Pulsatile Tinnitus, Vestibular).';
