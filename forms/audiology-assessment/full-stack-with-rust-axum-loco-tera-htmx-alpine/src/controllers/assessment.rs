...
use crate::engine::{hearing_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "symptoms_assessment" => "symptomsAssessment",
        "otoscopic_examination" => "otoscopicExamination",
        "audiometric_results" => "audiometricResults",
        "tinnitus" => "tinnitus",
        "balance_assessment" => "balanceAssessment",
        "communication_impact" => "communicationImpact",
        "hearing_aid_assessment" => "hearingAidAssessment",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "patientName"
            ...
            | "affectedEar"
            | "familyHistory"
            | "noiseExposureHistory"
            | "noiseExposureType"
            | "hearingProtectionUse"
            | "previousHearingTest"
            | "previousHearingAid"
            | "ototoxicMedication"
            | "ototoxicMedicationName"
            | "earPain"
            | "earDischarge"
            | "earFullness"
            | "autophony"
            ...
            | "fallsFrequency"
            | "nauseaWithDizziness"
            | "communicationStrategiesUsed"
            | "additionalConcerns"
            | "currentHearingAid"
            | "hearingAidType"
            | "hearingAidAge"
            | "hearingAidHoursPerDay"
            | "hearingAidDifficulties"
            | "interestInHearingAid"
            | "hearingAidConcerns"
            | "assistiveDeviceUse"
            | "lossTypeRight"
            | "lossTypeLeft"
...
/// Parse a form value, keeping string fields as strings and converting Likert/numeric fields to numbers.
...
async fn show_report(
    ...
    // Run grading engine
    let (hearing_level, pure_tone_average, fired_rules) =
        hearing_grader::calculate_hearing_level(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        hearing_level: hearing_level.clone(),
        pure_tone_average,
    ...
    let hearing_aid_status = crate::engine::utils::hearing_aid_status(&assessment_data);

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("hearing_level", &hearing_level);
    context.insert("hearing_label", &crate::engine::utils::hearing_level_label(&hearing_level));
    context.insert("pure_tone_average", &pure_tone_average);
    context.insert("hearing_aid_status", &hearing_aid_status);
