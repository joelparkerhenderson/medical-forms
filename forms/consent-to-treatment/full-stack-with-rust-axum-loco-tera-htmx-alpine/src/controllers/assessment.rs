...
use crate::engine::{consent_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "interpreter_requirements" => "interpreterRequirements",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "patientName"
            ...
            | "noTreatmentOption"
            | "reasonForChosenTreatment"
            | "patientHasCapacity"
            | "capacityAssessedBy"
            | "capacityAssessmentDate"
            | "capacityNotes"
            | "questionsAsked"
            ...
            | "interpreterName"
            | "interpreterService"
            | "communicationAidsNeeded"
            | "communicationAidsDetails"
            | "patientSignature"
            | "patientSignatureDate"
            | "consentVoluntary"
            | "rightToWithdrawUnderstood"
            | "witnessName"
            | "witnessSignature"
...
/// Parse a form value, keeping string fields as strings and converting rating fields to numbers.
...
async fn show_report(
    ...
    // Run grading engine
    let (consent_status, completeness_score, fired_rules) =
        consent_grader::calculate_consent(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        consent_status: consent_status.clone(),
        completeness_score,
    ...
    context.insert("consent_status", &consent_status);
    context.insert("consent_label", &crate::engine::utils::consent_status_label(&consent_status));
    context.insert("completeness_score", &completeness_score);
