//! `SeaORM` Entity, @generated by sea-orm-codegen 1.1.20

use sea_orm::entity::prelude::*;
use serde::{Deserialize, Serialize};

#[derive(Clone, Debug, PartialEq, DeriveEntityModel, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
#[sea_orm(table_name = "patients")]
pub struct Model {
    pub created_at: DateTimeWithTimeZone,
    pub updated_at: DateTimeWithTimeZone,
    #[sea_orm(primary_key)]
    pub id: i64,
    pub deleted_at: Option<DateTimeWithTimeZone>,
    pub name: String,
    pub birth_date: Date,
    #[sea_orm(column_type = "Text", nullable)]
    pub email: Option<String>,
    #[sea_orm(column_type = "Text", nullable)]
    pub phone: Option<String>,
    #[sea_orm(column_type = "Text", nullable)]
    pub postal_address_as_full_text: Option<String>,
    pub country_as_iso_3166_1_alpha_2: Option<String>,
    #[sea_orm(column_type = "Text", nullable)]
    pub postcode: Option<String>,
    #[sea_orm(unique)]
    pub united_kingdom_nhs_number: Option<String>,
    #[sea_orm(column_type = "Double", nullable)]
    pub waist_as_cm: Option<f64>,
    #[sea_orm(column_type = "Double", nullable)]
    pub height_as_cm: Option<f64>,
    #[sea_orm(column_type = "Double", nullable)]
    pub weight_as_kg: Option<f64>,
    #[sea_orm(column_type = "Double", nullable)]
    pub body_mass_index: Option<f64>,
    #[sea_orm(column_type = "Double", nullable)]
    pub waist_height_ratio: Option<f64>,
    #[sea_orm(column_type = "Double", nullable)]
    pub vo2_max: Option<f64>,
}

#[derive(Copy, Clone, Debug, EnumIter, DeriveRelation)]
pub enum Relation {
    #[sea_orm(has_many = "super::bone_marrow_donation_assessments::Entity")]
    BoneMarrowDonationAssessments,
}

impl Related<super::bone_marrow_donation_assessments::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::BoneMarrowDonationAssessments.def()
    }
}
