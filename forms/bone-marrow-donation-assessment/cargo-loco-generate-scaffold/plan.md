...
Generate Loco (loco.rs) scaffolding for the `bone-marrow-donation-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name bone-marrow-donation-assessment --db postgres --bg async --assets serverside
   ```
