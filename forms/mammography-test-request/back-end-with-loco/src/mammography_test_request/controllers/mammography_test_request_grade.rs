...
use crate::models::_entities::mammography_test_request_grades::{ActiveModel, Entity, Model};

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Params {
    pub deleted_at: Option<DateTimeWithTimeZone>,
    pub mammography_test_request_id: i64,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.mammography_test_request_id = Set(self.mammography_test_request_id);
      ...
      item.priority_band = Set(self.priority_band.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/mammography_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
