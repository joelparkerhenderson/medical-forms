...
const STORAGE_KEY =
  'mammography-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
const INDICATION_OPTIONS = [
  { value: 'routine-screening', label: 'Routine screening' },
  { value: 'breast-lump', label: 'Breast lump' },
  { value: 'breast-pain', label: 'Breast pain' },
  { value: 'nipple-discharge', label: 'Nipple discharge' },
  { value: 'skin-change', label: 'Skin change' },
  { value: 'family-history', label: 'Family history' },
  { value: 'follow-up-known-cancer', label: 'Follow-up known cancer' },
  { value: 'post-treatment-surveillance', label: 'Post-treatment surveillance' },
  { value: 'recall-from-screening', label: 'Recall from screening' },
  { value: 'other', label: 'Other' }
];
...
const PREVIOUS_MAMMOGRAM_OPTIONS = [
...
const SETTING_OPTIONS = [
  ...
  { value: 'screening-unit', label: 'Screening unit' },
  { value: 'emergency', label: 'Emergency' }
];
...
function renderStep1() {
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'gp', label: 'GP' },
      { value: 'breast-surgeon', label: 'Breast surgeon' },
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'Patient demographics — date of birth drives age-based NICE NG12 triggers.'
  });
  card.appendChild(grid('two-col', [
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Exam type, indication, laterality, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Exam type', section: 'request', field: 'examType',
    options: EXAM_TYPE_OPTIONS, required: true
  }));
  card.appendChild(selectInput({
  ...
  card.appendChild(selectInput({
    label: 'Laterality', section: 'request', field: 'laterality',
    options: LATERALITY_OPTIONS
  }));
  card.appendChild(textArea({
    label: 'Specific clinical question', section: 'request', field: 'clinicalQuestion',
    rows: 2, required: true,
    placeholder: 'e.g. Characterise palpable lump in the left upper outer quadrant.'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Symptoms',
    description: 'Symptoms drive the clinical-priority axis and NICE NG12 two-week-wait triggers.'
  });
  card.appendChild(boolField({ label: 'Breast lump', section: 'symptoms', field: 'symptomLump' }));
  card.appendChild(boolField({ label: 'Breast pain', section: 'symptoms', field: 'symptomPain' }));
  card.appendChild(boolField({ label: 'Nipple discharge', section: 'symptoms', field: 'symptomNippleDischarge' }));
  card.appendChild(boolField({ label: 'Skin change (dimpling, peau d’orange, erythema)', section: 'symptoms', field: 'symptomSkinChange' }));
  card.appendChild(boolField({ label: 'New nipple inversion / retraction', section: 'symptoms', field: 'symptomNippleInversion' }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Breast history & risk',
    description: 'Previous imaging, family history, implants, pregnancy / lactation, and HRT.'
  });
  card.appendChild(grid('two-col', [
    selectInput({
      label: 'Previous mammogram', section: 'history', field: 'previousMammogram',
      options: PREVIOUS_MAMMOGRAM_OPTIONS
    }),
    textInput({ label: 'Previous mammogram date', section: 'history', field: 'previousMammogramDate', type: 'date' })
  ]));
  card.appendChild(selectInput({
    label: 'Pregnancy / lactation', section: 'history', field: 'pregnancyOrLactating',
    options: PREGNANCY_OPTIONS
  }));
  card.appendChild(subHead('Risk factors'));
  card.appendChild(boolField({ label: 'Family history of breast cancer', section: 'history', field: 'familyHistoryBreastCancer' }));
  card.appendChild(boolField({ label: 'Breast implants', section: 'history', field: 'breastImplants' }));
  card.appendChild(boolField({ label: 'Hormone replacement therapy (HRT)', section: 'history', field: 'hormoneReplacementTherapy' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Triage',
    description: 'Requested urgency, requested-by date, setting, and notes.'
  });
  card.appendChild(grid('two-col', [
  ...
  card.appendChild(grid('two-col', [
    ...
    textInput({ label: 'Imaging site', section: 'triage', field: 'siteName' })
  ]));
...
function renderStep7() {
  const card = sectionCard({
    stepNumber: 7,
    title: 'Review & submit',
    description: 'Submit to compute the four-axis grade (appropriateness, cancer-pathway urgency, completeness, priority) and safety flags.'
  });
  const note = document.createElement('p');
  note.className = 'muted';
  note.textContent =
    'Press “Submit and view report” below to run the grading engine. The vetting report appears beneath the form.';
...
const TRACKED_FIELDS = [
  ...
  // 3 Request
  ['request', 'examType'], ['request', 'primaryIndication'],
  ['request', 'laterality'], ['request', 'clinicalQuestion'],
  // 5 History
  ['history', 'previousMammogram'], ['history', 'pregnancyOrLactating'],
  // 6 Triage
...
const STEP_DEFINITIONS = [
  ...
  { step: 5, section: 'history',   title: 'History & risk' },
  { step: 6, section: 'triage',    title: 'Triage' },
  { step: 7, section: 'triage',    title: 'Review' }
];
...
function updateStepListStatuses(sectionAnswered, sectionTotal) {
  ...
  for (const def of STEP_DEFINITIONS) {
    ...
    // Symptoms and the review step have no tracked text fields; informational.
    if (t > 0 && a === t) {
...
function renderReport() {
  ...
  const twoWWBadge = twoWeekWaitEligible
    ? `<span class="band-badge twoww-yes">Two-week-wait eligible</span>`
    : `<span class="band-badge twoww-no">Not two-week-wait</span>`;
  ...
  out.innerHTML = `
    <h2>Mammography Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Exam: ${esc(examTypeLabel(state.request.examType) || '—')}${state.request.primaryIndication ? ` · ${esc(titleCase(state.request.primaryIndication))}` : ''}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        ...
        ${twoWeekWaitEligible && twoWeekWaitRationale ? `<div class="muted">${esc(twoWeekWaitRationale)}</div>` : ''}
      </div>
      <div class="axis-card">
