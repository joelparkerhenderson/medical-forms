<script lang="ts">
	...
	let testingFilter = $state('');
	let counselingFilter = $state('');
	...
	const testingOptions = [
		{ value: '', label: 'All testing statuses' },
		{ value: 'Not started', label: 'Not started' },
		{ value: 'Pending', label: 'Pending' },
		{ value: 'In progress', label: 'In progress' },
		{ value: 'Awaiting consent', label: 'Awaiting consent' },
		{ value: 'Complete', label: 'Complete' },
	];

	const counselingOptions = [
		{ value: '', label: 'All counseling statuses' },
		...
		const filter = (row: PatientRow) => {
			...
			if (term) {
				const matches =
					...
					row.referralReason.toLowerCase().includes(term);
			...
			if (testingFilter && row.testingStatus !== testingFilter) {
				return false;
			}

			// Counseling status filter
			if (counselingFilter && row.counselingStatus !== counselingFilter) {
				return false;
			}
	...
	let hasActiveFilters = $derived(
		searchTerm !== '' || riskFilter !== '' || testingFilter !== '' || counselingFilter !== ''
	);
</script>
...
<div class="min-h-screen bg-gray-50">
	...
	<header class="bg-nhs-blue text-white shadow">
		<div class="mx-auto max-w-7xl px-4 py-4 sm:px-6">
			<h1 class="text-2xl font-bold">Genetic Assessment — Clinician Dashboard</h1>
			<p class="mt-1 text-sm text-blue-100">Patient list with genetic assessment and counseling status</p>
		</div>
	</header>
			...
			<div class="flex flex-wrap items-end gap-4">
				...
				<div class="min-w-[240px] flex-1">
					...
					<input
						id="search"
						type="text"
						placeholder="NHS number, name, or referral reason..."
				...
				<!-- Testing Status -->
				<div>
					<label for="testing-filter" class="mb-1 block text-sm font-medium text-gray-700">
						Testing Status
					</label>
					<select
						id="testing-filter"
						bind:value={testingFilter}
						onchange={applyFilters}
						class="rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none"
					>
						{#each testingOptions as opt (opt.value)}
							<option value={opt.value}>{opt.label}</option>
						{/each}
					</select>
				</div>
				...
				<!-- Counseling Status -->
				<div>
					<label for="counseling-filter" class="mb-1 block text-sm font-medium text-gray-700">
						Counseling
					</label>
					<select
						id="counseling-filter"
						bind:value={counselingFilter}
						onchange={applyFilters}
						class="rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none"
					>
						{#each counselingOptions as opt (opt.value)}
							<option value={opt.value}>{opt.label}</option>
						{/each}
					</select>
				</div>
