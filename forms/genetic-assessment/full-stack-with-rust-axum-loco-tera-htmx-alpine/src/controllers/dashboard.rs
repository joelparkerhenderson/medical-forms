...
async fn dashboard(
    ...
    if let Some(ref term) = params.search {
        ...
        items.retain(|row| {
            row.patient_name.to_lowercase().contains(&term)
                || row.referral_reason.to_lowercase().contains(&term)
                || row.family_pattern.to_lowercase().contains(&term)
        });
    }
    ...
    if let Some(ref status) = params.testing_status {
        if !status.is_empty() {
            items.retain(|row| row.testing_status == *status);
        }
    }
    ...
    context.insert("testing_status", &params.testing_status.unwrap_or_default());
