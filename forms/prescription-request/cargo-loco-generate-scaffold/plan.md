...
Generate Loco (loco.rs) scaffolding for the `prescription-request` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name prescription-request --db postgres --bg async --assets serverside
   ```
