...
use crate::engine::{flagged_issues, pediatric_grader, types::AssessmentData};
...
fn section_to_json(section: &str) -> &str {
    match section {
        "patient_parent_information" => "patientParentInformation",
        "birth_neonatal_history" => "birthNeonatalHistory",
        "growth_development" => "growthDevelopment",
        "immunization_status" => "immunizationStatus",
        "feeding_nutrition" => "feedingNutrition",
        "developmental_milestones" => "developmentalMilestones",
        "behavioral_assessment" => "behavioralAssessment",
        "family_social_history" => "familySocialHistory",
...
async fn show_report(
    ...
    // Run grading engine
    let (concern_level, concern_score, fired_rules) =
        pediatric_grader::calculate_concern(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        concern_level: concern_level.clone(),
        concern_score,
    ...
    let health_category = crate::engine::utils::health_impression_category(
        assessment_data.clinical_review.overall_health_impression,
    );

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("concern_level", &concern_level);
    context.insert(
        "concern_label",
        &crate::engine::utils::concern_level_label(&concern_level),
    );
    context.insert("concern_score", &concern_score);
    context.insert("health_category", &health_category);
    context.insert(
        "health_impression",
        &assessment_data.clinical_review.overall_health_impression,
    );
