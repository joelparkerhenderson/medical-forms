...
use crate::models::_entities::blood_cross_match_test_request_grades::{ActiveModel, Entity, Model};

#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct Params {
    pub deleted_at: Option<DateTimeWithTimeZone>,
    pub blood_cross_match_test_request_id: i64,
    pub appropriateness_score: Option<i32>,
    pub appropriateness_band: String,
    pub identity_safety_band: String,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.blood_cross_match_test_request_id = Set(self.blood_cross_match_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.identity_safety_band = Set(self.identity_safety_band.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/blood_cross_match_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
