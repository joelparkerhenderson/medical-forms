...
// inline structured electrodiagnostic report. State is persisted to
...
const STORAGE_KEY = 'nerve-conduction-study-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').NerveConductionStudyResult} */
...
function mergeIntoDefaults(parsed) {
  ...
  // The type check keeps null / mistyped saved values (e.g. a numeric ''
  // that was persisted as null by an older build) from clobbering the
  // canonical defaults — rehydration is null-safe.
...
/** @param {import('./types.js').NerveConductionStudyResult} state */
...
/** @type {import('./types.js').NerveConductionStudyResult} */
let state = loadState();
...
function renderStep1() {
  ...
  card.appendChild(textInput({
    ...
    placeholder: 'e.g. Dr A Neurophysiologist'
  }));
  card.appendChild(textInput({
...
function renderStep2() {
  const card = sectionCard({
    stepNumber: 2,
    title: 'Study details',
    description: 'The performed study type, region, laterality, and adequacy.'
  });
  ...
  card.appendChild(selectInput({
    ...
    options: [
      { value: 'nerve-conduction', label: 'Nerve conduction' },
      { value: 'emg', label: 'Needle EMG' },
      { value: 'nerve-conduction-and-emg', label: 'Nerve conduction and EMG' },
      { value: 'repetitive-stimulation', label: 'Repetitive stimulation' },
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'all-limbs', label: 'All limbs' },
  ...
  card.appendChild(selectInput({
    ...
    field: 'studyAdequacy',
    hint: 'A non-diagnostic study classifies the result as inconclusive; a limited study without an impression does too.',
    options: [
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'The clinical history, the question the study was performed to answer, and comparison with previous studies.'
  });
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'Relevant previous electrodiagnostic studies and changes since…'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Findings',
    description: 'The nerve-conduction and needle-EMG narratives plus structured finding flags.'
  });

  card.appendChild(textArea({
    label: 'Nerve-conduction findings',
    field: 'nerveConductionFindings', rows: 4,
    placeholder: 'Latencies, amplitudes, conduction velocities, conduction block…'
  }));
  card.appendChild(textArea({
    label: 'EMG findings',
    field: 'emgFindings', rows: 4,
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'carpalTunnelSyndrome', label: 'Carpal tunnel syndrome' },
      { field: 'peripheralNeuropathy', label: 'Peripheral neuropathy' },
      { field: 'radiculopathy', label: 'Radiculopathy' },
      { field: 'motorNeuroneDiseaseFeatures', label: 'Motor neurone disease features' },
      { field: 'myopathy', label: 'Myopathy' },
      { field: 'neuromuscularJunctionDisorder', label: 'Neuromuscular junction disorder' },
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'Motor neurone disease / anterior-horn-cell features auto-escalate the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'The overall severity and predominant pathophysiological pattern.'
  });
  ...
  card.appendChild(selectInput({
    ...
    field: 'pattern',
    options: [
      { value: 'demyelinating', label: 'Demyelinating' },
      { value: 'axonal', label: 'Axonal' },
  ...
  card.appendChild(conditionalAlert({
    id: 'gbs-alert',
    type: 'error',
    heading: 'Severe acute neuropathy pattern',
    message: 'A severe, demyelinating peripheral neuropathy (e.g. a Guillain-Barré syndrome pattern) auto-escalates the follow-up urgency to a critical alert and warrants urgent neurology review.'
  }));
...
function renderStep6() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Optional structured-reporting category label (e.g. an AANEM CTS severity grade or polyneuropathy descriptor).',
    placeholder: 'e.g. carpal-tunnel-syndrome-mild'
  }));
  card.appendChild(textArea({
    label: 'Recommended follow-up',
    field: 'recommendedFollowUp', rows: 3,
    placeholder: 'Recommended follow-up study, referral, or management…'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'mnd-alert': state.motorNeuroneDiseaseFeatures,
    'gbs-alert': state.peripheralNeuropathy &&
      state.severity === 'severe' && state.pattern === 'demyelinating',
...
// counts only when ticked). This lets the findings step count as answered
// once a narrative or any structured finding is recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['studyType'], ['region'], ['laterality'], ['studyAdequacy']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['nerveConductionFindings', 'emgFindings'],
      ['carpalTunnelSyndrome', 'peripheralNeuropathy', 'radiculopathy',
       'motorNeuroneDiseaseFeatures', 'myopathy',
       'neuromuscularJunctionDisorder', 'normalStudy']],
  5: [['severity'], ['pattern']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Region', regionLabel(state.region)],
  ...
  out.innerHTML = `
    <div class="report-card">
      <header class="report-header">
        <h2>Nerve Conduction Study / EMG Report</h2>
      ...
      ${state.nerveConductionFindings ? `<p class="muted">Nerve conduction: ${esc(state.nerveConductionFindings)}</p>` : ''}
      ${state.emgFindings ? `<p class="muted">EMG: ${esc(state.emgFindings)}</p>` : ''}
