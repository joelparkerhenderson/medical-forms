...
use crate::engine::{semaglutide_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "weight_bmi_history" => "weightBmiHistory",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "fullName"
            ...
            | "depressionAnxiety"
            | "otherComorbidities"
            | "personalMedullaryThyroidCancer"
            | "familyMen2"
            | "pancreatitisHistory"
            | "severeGiDisease"
            | "pregnancyOrPlanning"
            ...
            | "smokingStatus"
            | "primaryGoal"
            | "glycaemicControlGoal"
            | "cardiovascularRiskReduction"
            | "mobilityImprovement"
            | "qualityOfLifeImprovement"
            | "understandsMechanism"
            | "understandsSideEffects"
            | "understandsInjectionTechnique"
            | "understandsDoseEscalation"
            | "understandsMonitoringRequirements"
            | "consentGiven"
            | "consentDate"
            | "clinicianName"
            | "baselineBloodsCompleted"
            | "hba1cBaseline"
            | "renalFunctionChecked"
            | "thyroidFunctionChecked"
            | "weightMonitoringFrequency"
            | "sideEffectMonitoringPlan"
            | "doseEscalationSchedule"
            | "recommendedStartingDose"
            | "additionalInvestigationsNeeded"
            | "referralsNeeded"
            | "clinicianNotes"
    )
}
...
async fn show_report(
    ...
    // Run grading engine
    let (eligibility_level, eligibility_score, fired_rules) =
        semaglutide_grader::calculate_eligibility(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        eligibility_level: eligibility_level.clone(),
        eligibility_score,
    ...
    let bmi_cat = crate::engine::utils::bmi_category(
        assessment_data.weight_bmi_history.current_bmi,
    );

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("eligibility_level", &eligibility_level);
    context.insert("eligibility_label", &crate::engine::utils::eligibility_level_label(&eligibility_level));
    context.insert("eligibility_score", &eligibility_score);
    context.insert("bmi_category", &bmi_cat);
    context.insert("bmi", &assessment_data.weight_bmi_history.current_bmi);
