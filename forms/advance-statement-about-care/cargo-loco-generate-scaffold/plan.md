...
Generate Loco (loco.rs) scaffolding for the `advance-statement-about-care` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name advance-statement-about-care --db postgres --bg async --assets serverside
   ```
