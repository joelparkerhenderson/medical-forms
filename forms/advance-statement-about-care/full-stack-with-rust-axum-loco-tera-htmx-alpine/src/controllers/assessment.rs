...
use crate::engine::{completeness_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "daily_living_preferences" => "dailyLivingPreferences",
        "spiritual_cultural" => "spiritualCultural",
        "nominated_persons" => "nominatedPersons",
        "end_of_life_preferences" => "endOfLifePreferences",
        "healthcare_professional_review" => "healthcareProfessionalReview",
        "signatures_verification" => "signaturesVerification",
        other => other,
    }
}
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "fullName"
            ...
            | "treatmentGoals"
            | "resuscitationWishes"
            | "artificialNutritionView"
            | "ventilationView"
            | "preferredLanguage"
            | "interpreterNeeded"
            | "communicationAids"
            | "informationSharingWishes"
            | "whoToInform"
            | "howToBreakBadNews"
            | "foodPreferences"
            | "personalCareWishes"
...
async fn show_report(
    ...
    // Run grading engine
    let (completeness_status, sections_completed, fired_rules) =
        completeness_grader::calculate_completeness(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        completeness_status: completeness_status.clone(),
        sections_completed,
        total_sections: 10,
    ...
    context.insert("completeness_status", &completeness_status);
    context.insert("completeness_label", &crate::engine::utils::completeness_status_label(&completeness_status));
    context.insert("sections_completed", &sections_completed);
    context.insert("total_sections", &10u32);
