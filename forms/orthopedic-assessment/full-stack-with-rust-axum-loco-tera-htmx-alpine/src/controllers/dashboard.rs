...
async fn dashboard(
    ...
    if let Some(ref term) = params.search {
        ...
        items.retain(|row| {
            row.assessment_date.to_lowercase().contains(&term)
                ...
                || row.primary_complaint.to_lowercase().contains(&term)
        });
    }
    ...
    // Affected joint filter
    if let Some(ref joint) = params.affected_joint {
        if !joint.is_empty() {
            items.retain(|row| row.affected_joint == *joint);
        }
    }
    ...
    context.insert("affected_joint", &params.affected_joint.unwrap_or_default());
