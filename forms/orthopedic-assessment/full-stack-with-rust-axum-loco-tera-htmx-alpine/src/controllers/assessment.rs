...
use crate::engine::{orthopedic_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "joint_examination" => "jointExamination",
        "muscle_assessment" => "muscleAssessment",
        "spinal_assessment" => "spinalAssessment",
        "imaging_investigations" => "imagingInvestigations",
        "functional_status" => "functionalStatus",
        "surgical_considerations" => "surgicalConsiderations",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "patientName"
            ...
            | "boneDensityResult"
            | "bloodTestsResult"
            | "furtherImagingNeeded"
            | "walkingDistance"
            | "assistiveDevices"
            | "surgicalCandidate"
            | "conservativeOptionsExhausted"
            | "patientConsentDiscussion"
            | "rehabilitationPlan"
...
async fn show_report(
    ...
    let (severity_level, severity_score, fired_rules) =
        orthopedic_grader::calculate_severity(&assessment_data);
