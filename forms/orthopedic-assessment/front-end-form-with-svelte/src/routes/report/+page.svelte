<script lang="ts">
	...
	import { dashScoreLabel, dashScoreColor, calculateAge, sideLabel, onsetTypeLabel } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `orthopedic-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
...
{#if result}
	<div class="min-h-screen bg-gray-50">
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			<!-- DASH Score Banner -->
			<div class="mb-6 rounded-xl border-2 p-6 text-center {result.dashScore !== null ? dashScoreColor(result.dashScore) : 'bg-gray-100 text-gray-800 border-gray-300'}">
			...
			{#if result.additionalFlags.length > 0}
				<div class="mb-6 rounded-xl border border-red-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-red-800">Flagged Issues for Orthopedic Surgeon</h2>
					<div class="space-y-2">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">DASH Score Breakdown</h2>
			...
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				...
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					...
					<div>
						<span class="font-medium text-gray-600">Dominant Hand:</span>
						{data.demographics.dominantHand || 'N/A'}
					</div>
					<div>
					...
					<div>
						<span class="font-medium text-gray-600">Affected Joint:</span>
						{data.chiefComplaint.affectedJoint} ({sideLabel(data.chiefComplaint.side)})
					</div>
					...
					<div>
						...
						{onsetTypeLabel(data.chiefComplaint.onsetType)}
					</div>
					<div>
						<span class="font-medium text-gray-600">Current Pain:</span>
						{data.painAssessment.currentPainLevel !== null ? `${data.painAssessment.currentPainLevel}/10` : 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Worst Pain:</span>
						{data.painAssessment.worstPain !== null ? `${data.painAssessment.worstPain}/10` : 'N/A'}
					</div>
				</div>
			</div>
			...
			{#if data.currentTreatment.medications.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					...
					<ul class="list-disc space-y-1 pl-5 text-sm">
						{#each data.currentTreatment.medications as med}
			...
			{#if data.currentTreatment.allergies.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					...
					<ul class="list-disc space-y-1 pl-5 text-sm">
						{#each data.currentTreatment.allergies as allergy}
							<li>
