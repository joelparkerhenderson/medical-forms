...
Generate Loco (loco.rs) scaffolding for the `orthopedic-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name orthopedic-assessment --db postgres --bg async --assets serverside
   ```
