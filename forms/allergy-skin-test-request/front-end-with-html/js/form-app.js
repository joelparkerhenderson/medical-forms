...
const STORAGE_KEY =
  'allergy-skin-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
/**
 * Render the allergen-panel checkbox group. Each panel is a Lily
 * `.checkbox-input` inside a `.checkbox-group` (role="group"). A live count of
 * selected panels sits above the group. The panels are the six
 * `allergen_*` BOOLEAN columns from the SQL source of truth.
 */
function allergenCheckboxGroup() {
  const wrapper = document.createElement('fieldset');
  wrapper.className = 'field';
  wrapper.id = 'allergen-panels-fieldset';

  const legend = document.createElement('legend');
  legend.className = 'label';
  legend.innerHTML = 'Allergen panels <span class="req" aria-hidden="true">*</span>';
  wrapper.appendChild(legend);

  const hint = document.createElement('span');
  hint.className = 'hint';
  hint.id = 'allergen-panels-count';
  ...
  for (const p of ALLERGEN_PANELS) {
    const cbId = `test-${p.field}`;
    const checked = state.test[p.field] === true ? ' checked' : '';
    const label = document.createElement('label');
    label.htmlFor = cbId;
    label.innerHTML = `
      <input class="checkbox-input" type="checkbox" id="${cbId}" name="allergenPanels" value="${esc(p.field)}"${checked}>
      <span class="panel-label">${esc(p.label)}<span class="panel-hint">${esc(p.hint)}</span></span>
    `;
    const input = label.querySelector('input');
    input.addEventListener('change', () => {
      setPanel(p.field, input.checked);
      clearFieldError('allergen-panels');
    });
  ...
  errSpan.id = 'allergen-panels-error';
  wrapper.appendChild(errSpan);
  return wrapper;
}

function updatePanelCount() {
  const el = document.getElementById('allergen-panels-count');
  if (!el) return;
  const n = countSelectedPanels(state.test);
  el.textContent = n === 0
    ? 'No allergen panels selected yet — select at least one.'
    : `${n} allergen panel${n === 1 ? '' : 's'} selected.`;
}
...
const INDICATION_OPTIONS = [
  { value: 'suspected-food-allergy', label: 'Suspected food allergy' },
  { value: 'suspected-drug-allergy', label: 'Suspected drug allergy' },
  { value: 'rhinitis-asthma', label: 'Rhinitis / asthma' },
  { value: 'anaphylaxis-investigation', label: 'Anaphylaxis investigation' },
  { value: 'venom-allergy', label: 'Venom allergy' },
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'The test modality and the allergen panels to test — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Requested test type', section: 'test', field: 'testType',
    options: TEST_TYPE_OPTIONS, required: true
  }));
  card.appendChild(allergenCheckboxGroup());
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'The indication, the specific clinical question, and the relevant history.'
  });
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Confirm or exclude IgE-mediated peanut allergy.'
  }));
  card.appendChild(textArea({
    label: 'Relevant clinical details / history', section: 'indication', field: 'clinicalDetails',
    rows: 3,
    placeholder: 'Reaction description, timing, suspected triggers, prior testing.'
  }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Validity and safety',
    description: 'Drives the validity-and-safety axis. Antihistamines invalidate skin tests; beta-blocker plus anaphylaxis history and active skin disease raise caution.'
  });
  card.appendChild(subHead('Validity and safety history'));
  card.appendChild(boolField({ label: 'History of anaphylaxis', section: 'safety', field: 'previousAnaphylaxis' }));
  card.appendChild(boolField({ label: 'Currently on antihistamines', section: 'safety', field: 'onAntihistamines' }));
  card.appendChild(boolField({ label: 'Currently on a beta-blocker', section: 'safety', field: 'onBetaBlocker' }));
  card.appendChild(boolField({ label: 'Active skin disease at test site (eczema, dermographism)', section: 'safety', field: 'currentSkinDisease' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Triage and logistics',
    description: 'Requested urgency, setting, and notes.'
  });
  card.appendChild(grid('two-col', [
...
function renderStep7() {
  const card = sectionCard({
    ...
    description: 'Submit to compute the four-axis grade and safety flags.'
  });
  const p = document.createElement('p');
  p.className = 'muted';
  p.textContent =
    'Use the "Submit and view report" button below to compute the ' +
    'appropriateness, validity-and-safety, completeness, and triage axes ' +
    'plus safety flags. Required fields must be completed first.';
  card.appendChild(p);
  return card;
}
...
const TRACKED_FIELDS = [
  ...
  ['indication', 'clinicalDetails'],
  // 6 Triage
  ['triage', 'urgency'], ['triage', 'setting']
];

const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'test', 4: 'indication',
  5: 'safety', 6: 'triage', 7: 'review'
};
...
function updateProgress() {
  ...
  // The allergen-panel selection (step 3) counts as one tracked unit.
  sectionTotal['test'] = (sectionTotal['test'] || 0) + 1;
  let panelAnswered = 0;
  if (countSelectedPanels(state.test) > 0) {
    panelAnswered = 1;
    answered++;
    sectionAnswered['test'] = (sectionAnswered['test'] || 0) + 1;
  }

  const total = TRACKED_FIELDS.length + 1; // +1 for the allergen-panel unit
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, section: 'test',       title: 'Test' },
  { step: 4, section: 'indication', title: 'Indication' },
  { step: 5, section: 'safety',     title: 'Validity & safety' },
...
function updateStepListStatuses(sectionAnswered, sectionTotal) {
  ...
  for (const def of STEP_DEFINITIONS) {
    ...
    // Validity & safety and Review have no tracked text fields; informational.
    if (t > 0 && a === t) {
...
function validateForm() {
  ...
  // At least one allergen panel must be selected.
  if (countSelectedPanels(state.test) === 0) {
    errors.push({ id: 'allergen-panels-fieldset', message: 'Select at least one allergen panel' });
    const el = document.getElementById('allergen-panels-error');
    if (el) el.textContent = 'Select at least one allergen panel';
  } else {
    const el = document.getElementById('allergen-panels-error');
    if (el) el.textContent = '';
  }
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>Allergy Test Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Test: ${esc(testTypeLabel(state.test.testType) || '—')}${panels ? ` · Panels: ${esc(panels)}` : ''}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Validity &amp; safety</span>
        <span class="axis-value"><span class="band-badge validity-${esc(validitySafetyBand)}">${esc(titleCase(validitySafetyBand))}</span></span>
      </div>
      <div class="axis-card">
