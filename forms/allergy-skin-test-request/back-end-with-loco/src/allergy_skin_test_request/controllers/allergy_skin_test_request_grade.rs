...
use crate::models::_entities::allergy_skin_test_request_grades::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.allergy_skin_test_request_id = Set(self.allergy_skin_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.validity_safety_band = Set(self.validity_safety_band.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/allergy_skin_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
