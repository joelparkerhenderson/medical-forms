<script lang="ts">
	import type { Allergy, AllergySeverity, AllergyType } from '$lib/engine/types';
	...
	const typeOptions: { value: AllergyType; label: string }[] = [
	...
	function addAllergy() {
		allergies = [...allergies, { allergen: '', allergyType: '', reaction: '', severity: '' }];
	}
	...
	{#each allergies as allergy, i}
		<div class="flex items-start gap-2 rounded-lg border border-gray-200 bg-gray-50 p-3">
			<div class="grid flex-1 grid-cols-1 gap-2 sm:grid-cols-4">
				...
				/>
				<select
					bind:value={allergy.allergyType}
					class="rounded border border-gray-300 px-2 py-1.5 text-sm focus:border-primary focus:outline-none"
				>
