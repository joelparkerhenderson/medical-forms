...
/**
 * Declarative intake risk classification rules.
 * Each rule evaluates patient data and returns true if the condition is present.
 * The risk level is determined by the highest-level rule that fires.
 * Low is the default when no rules fire (healthy patient with minimal complexity).
 */
export const intakeRules: IntakeRule[] = [
	...
	},
	{
		...
		riskLevel: 'medium',
		evaluate: (d) =>
			d.medicalHistory.chronicConditions.length >= 1 &&
			d.medicalHistory.chronicConditions.length < 3
	},
	{
		id: 'MH-003',
		category: 'Medical History',
