...
use crate::engine::{satisfaction_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "wait_time_access" => "waitTimeAccess",
        ...
        "demographics_comments" => "demographicsComments",
        other => other,
    }
}
...
async fn show_report(
    ...
    // Run grading engine
    let (satisfaction_level, satisfaction_score, fired_rules) =
        satisfaction_grader::calculate_satisfaction(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        satisfaction_level: satisfaction_level.clone(),
        satisfaction_score,
    ...
    let nps_category = crate::engine::utils::nps_category(
        assessment_data.overall_experience.likelihood_to_recommend,
    );

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("satisfaction_level", &satisfaction_level);
    context.insert("satisfaction_label", &crate::engine::utils::satisfaction_level_label(&satisfaction_level));
    context.insert("satisfaction_score", &satisfaction_score);
    context.insert("nps_category", &nps_category);
    context.insert("nps_score", &assessment_data.overall_experience.likelihood_to_recommend);
