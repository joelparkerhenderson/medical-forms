...
use crate::models::_entities::immunology_waiting_list_card_appointments::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.immunology_waiting_list_card_id = Set(self.immunology_waiting_list_card_id);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/immunology_waiting_list_card_appointments/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
