...
// inline structured cardiac stress report. State is persisted to localStorage
...
const STORAGE_KEY = 'cardiac-stress-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').CardiacStressResult} */
...
/** @param {import('./types.js').CardiacStressResult} state */
...
/** @type {import('./types.js').CardiacStressResult} */
let state = loadState();
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The performed stress-test modality, the protocol, and the clinical history.'
  });
  ...
  card.appendChild(selectInput({
    ...
    options: [
      { value: 'exercise-treadmill-ecg', label: 'Exercise treadmill ECG' },
      { value: 'stress-echo', label: 'Stress echocardiography' },
      { value: 'dobutamine-stress-echo', label: 'Dobutamine stress echo' },
      { value: 'myocardial-perfusion-spect', label: 'Myocardial perfusion SPECT' },
      { value: 'stress-cardiac-mri', label: 'Stress cardiac MRI' },
      { value: 'other', label: 'Other' }
    ]
  }));
  card.appendChild(textArea({
    label: 'Protocol',
    field: 'protocol', rows: 2,
    placeholder: 'Exercise / stress protocol (e.g. Bruce, modified Bruce, dobutamine infusion, vasodilator)…'
  }));
  card.appendChild(textArea({
...
function renderStep3() {
  const card = sectionCard({
    stepNumber: 3,
    title: 'Haemodynamic and exercise response',
    description: 'Peak heart rate, functional capacity, and blood-pressure response.'
  });
  ...
  card.appendChild(textInput({
    ...
    hint: 'Maximum heart rate achieved during the test.'
  }));
  card.appendChild(textInput({
    label: 'Percent-predicted heart rate (%)',
    field: 'percentPredictedHeartRate',
    type: 'number', min: 0, max: 200, step: 0.1, unit: '%',
    hint: 'Percentage of the age-predicted maximum heart rate (≥85% for an adequate test).'
  }));
  card.appendChild(textInput({
    label: 'Exercise duration (minutes)',
    field: 'exerciseDurationMinutes',
    type: 'number', min: 0, max: 120, step: 0.1, unit: 'min'
  }));
  card.appendChild(textInput({
    label: 'METs achieved',
    field: 'metsAchieved',
    type: 'number', min: 0, max: 30, step: 0.1, unit: 'METs',
    hint: 'Peak metabolic equivalents (functional capacity).'
  }));
  card.appendChild(textInput({
    label: 'Peak blood pressure',
    field: 'peakBloodPressure',
    placeholder: 'e.g. 180/90'
  }));
  card.appendChild(selectInput({
    label: 'Blood-pressure response',
    field: 'bloodPressureResponse',
    options: [
      { value: 'normal', label: 'Normal' },
      { value: 'hypertensive', label: 'Hypertensive' },
      { value: 'hypotensive', label: 'Hypotensive (exertional)' },
  ...
  card.appendChild(conditionalAlert({
    ...
    type: 'error',
    heading: 'Exertional hypotension selected',
    message: 'A hypotensive (exertional) blood-pressure response is a critical result. It auto-escalates the follow-up urgency to a critical alert; communicate the result to the referrer on sign-off.'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Findings',
    description: 'The structured ECG / symptom / arrhythmia findings and the overall conclusion.'
  });

  card.appendChild(checkboxGroup({
    label: 'Structured findings',
    items: [
      { field: 'ischaemicStChanges', label: 'Ischaemic ST changes induced' },
      { field: 'chestPainInduced', label: 'Chest pain induced' },
      { field: 'arrhythmiaInduced', label: 'Arrhythmia induced' },
      { field: 'terminatedEarly', label: 'Terminated early' }
    ]
  }));
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'testPositive', label: 'Positive (inducible ischaemia)' },
      { field: 'testNegative', label: 'Negative (no inducible ischaemia)' },
      { field: 'testInconclusive', label: 'Inconclusive / non-diagnostic' }
    ]
  }));

  card.appendChild(textArea({
    label: 'Reason for termination',
    field: 'reasonForTermination', rows: 3,
    placeholder: 'Target heart rate reached, fatigue, chest pain, ST changes, arrhythmia, exertional hypotension…'
  }));

  card.appendChild(conditionalAlert({
    id: 'strongly-positive-alert',
    type: 'error',
    heading: 'Strongly positive test selected',
    message: 'A positive conclusion with induced ischaemic ST changes is a critical result. It auto-escalates the follow-up urgency to a critical alert; ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'The Duke treadmill prognostic score and comparison with previous tests.'
  });

  card.appendChild(textInput({
    label: 'Duke treadmill score',
    field: 'dukeTreadmillScore',
    type: 'number', min: -25, max: 15, step: 0.1,
    hint: 'DTS = exercise minutes − (5 × ST deviation mm) − (4 × angina index). Range about −25 (highest risk) to +15 (lowest risk). ≥+5 low risk, −10 to +4 intermediate, ≤−11 high risk.'
  }));
  card.appendChild(textArea({
  ...
  card.appendChild(conditionalAlert({
    id: 'duke-high-risk-alert',
    type: 'error',
    heading: 'High-risk Duke treadmill score',
    message: `A Duke treadmill score of ${DUKE_HIGH_RISK_MAX} or lower is a high-risk band and is treated as a critical result. It auto-escalates the follow-up urgency to a critical alert.`
  }));
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Impression',
    description: 'The summary impression, risk category, and recommended follow-up.'
  });
  ...
  card.appendChild(textInput({
    ...
    hint: 'Structured risk-category label (e.g. low / intermediate / high risk).',
    placeholder: 'e.g. low risk, intermediate risk, high-risk Duke score'
  }));
  card.appendChild(textArea({
    label: 'Recommended follow-up',
    field: 'recommendedFollowUp', rows: 3,
    placeholder: 'Further imaging, coronary angiography, optimise medical therapy, referral, or routine…'
  }));
...
function renderStep7() {
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'This report contains a critical result. Communicate it directly to the referrer, make an urgent cardiology referral, and record it below before signing.'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'hypotension-alert': state.bloodPressureResponse === 'hypotensive',
    'strongly-positive-alert': state.testPositive && state.ischaemicStChanges,
    'duke-high-risk-alert':
      state.dukeTreadmillScore !== null && state.dukeTreadmillScore <= DUKE_HIGH_RISK_MAX,
...
// counts only when ticked). This lets the checkbox-only findings step count
// as answered once any structured finding or conclusion is recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['testType'], ['protocol'], ['clinicalHistory']],
  3: [['maximumHeartRateBpm'], ['percentPredictedHeartRate'],
      ['exerciseDurationMinutes'], ['metsAchieved'],
      ['peakBloodPressure'], ['bloodPressureResponse']],
  4: [['ischaemicStChanges', 'chestPainInduced', 'arrhythmiaInduced',
       'terminatedEarly', 'testPositive', 'testNegative', 'testInconclusive'],
      ['reasonForTermination']],
  5: [['dukeTreadmillScore'], ['comparisonWithPrevious']],
...
/** Format a nullable numeric measurement for the report table. */
function numberOr(value, unit) {
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Maximum heart rate', numberOr(state.maximumHeartRateBpm, 'bpm')],
    ['Percent-predicted heart rate', numberOr(state.percentPredictedHeartRate, '%')],
    ['Exercise duration', numberOr(state.exerciseDurationMinutes, 'min')],
    ['METs achieved', numberOr(state.metsAchieved, 'METs')],
    ['Peak blood pressure', state.peakBloodPressure || 'Not recorded'],
    ['Blood-pressure response', bloodPressureResponseLabel(state.bloodPressureResponse)],
    ['Duke treadmill score', numberOr(state.dukeTreadmillScore)],
  ...
  out.innerHTML = `
    <div class="report-card">
      <header class="report-header">
        <h2>Cardiac Stress Test Report</h2>
      ...
      ${state.comparisonWithPrevious ? `<p class="muted">Comparison with previous: ${esc(state.comparisonWithPrevious)}</p>` : ''}
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, title: 'Haemodynamics' },
  { step: 4, title: 'Findings' },
  { step: 5, title: 'Prognosis' },
