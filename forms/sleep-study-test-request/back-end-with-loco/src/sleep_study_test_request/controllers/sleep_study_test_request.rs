...
use crate::models::_entities::sleep_study_test_requests::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.epworth_score = Set(self.epworth_score);
      item.stop_bang_score = Set(self.stop_bang_score);
      item.body_mass_index = Set(self.body_mass_index);
      item.neck_circumference_cm = Set(self.neck_circumference_cm);
      item.witnessed_apnoeas = Set(self.witnessed_apnoeas);
      item.occupational_driver = Set(self.occupational_driver);
      item.cardiovascular_disease = Set(self.cardiovascular_disease);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/sleep_study_test_requests/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
