...
// Sleep Study Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'sleep-study-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 6;
...
const STUDY_TYPE_OPTIONS = [
  { value: 'home-sleep-apnoea-test', label: 'Home sleep apnoea test (HSAT)' },
  { value: 'polysomnography', label: 'Polysomnography (PSG)' },
  { value: 'overnight-oximetry', label: 'Overnight oximetry' },
  { value: 'multiple-sleep-latency-test', label: 'Multiple sleep latency test (MSLT)' },
  { value: 'actigraphy', label: 'Actigraphy' },
  { value: 'other', label: 'Other' }
];

const INDICATION_OPTIONS = [
  { value: 'suspected-osa', label: 'Suspected OSA' },
  { value: 'snoring', label: 'Snoring' },
  { value: 'daytime-sleepiness', label: 'Daytime sleepiness' },
  { value: 'suspected-narcolepsy', label: 'Suspected narcolepsy' },
  { value: 'insomnia', label: 'Insomnia' },
  { value: 'restless-legs', label: 'Restless legs' },
  { value: 'copd-overlap', label: 'COPD overlap' },
  { value: 'pre-bariatric', label: 'Pre-bariatric' },
  { value: 'driver-assessment', label: 'Driver assessment' },
  { value: 'other', label: 'Other' }
];
...
function renderStep1() {
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'sleep-physician', label: 'Sleep physician' },
      { value: 'gp', label: 'GP' },
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Study type, indication, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Requested study type', section: 'request', field: 'studyType',
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Confirm or exclude obstructive sleep apnoea.'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Sleep scores',
    description: 'Epworth Sleepiness Scale and STOP-BANG drive appropriateness, priority, and triage.'
  });
  card.appendChild(grid('two-col', [
    textInput({ label: 'Epworth Sleepiness Scale', section: 'scores', field: 'epworthScore', type: 'number', min: 0, max: 24, unit: '/ 24' }),
    textInput({ label: 'STOP-BANG score', section: 'scores', field: 'stopBangScore', type: 'number', min: 0, max: 8, unit: '/ 8' })
  ]));
  card.appendChild(textInput({ label: 'Neck circumference', section: 'scores', field: 'neckCircumferenceCm', type: 'number', min: 20, max: 70, step: 0.1, unit: 'cm' }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Symptoms & risk',
    description: 'Occupational driving with sleepiness, and severe sleepiness, auto-escalate the triage tier.'
  });
  card.appendChild(subHead('Symptoms and risk factors'));
  card.appendChild(boolField({ label: 'Witnessed apnoeas reported', section: 'symptoms', field: 'witnessedApnoeas' }));
  card.appendChild(boolField({ label: 'Occupational / vocational driver', section: 'symptoms', field: 'occupationalDriver' }));
  card.appendChild(boolField({ label: 'Established cardiovascular disease', section: 'symptoms', field: 'cardiovascularDisease' }));
  return card;
}
...
const TRACKED_FIELDS = [
  ...
  // 3 Request
  ['request', 'studyType'], ['request', 'primaryIndication'],
  ['request', 'clinicalQuestion'],
  // 4 Scores
  ['scores', 'epworthScore'], ['scores', 'stopBangScore'],
  // 6 Triage
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, section: 'request',   title: 'Study' },
  { step: 4, section: 'scores',    title: 'Scores' },
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>Sleep Study Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Study: ${esc(studyTypeLabel(state.request.studyType) || '—')}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Clinical priority</span>
