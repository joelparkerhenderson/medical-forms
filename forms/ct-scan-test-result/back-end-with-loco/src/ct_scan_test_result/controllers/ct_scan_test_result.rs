...
use crate::models::_entities::ct_scan_test_results::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.acute_finding = Set(self.acute_finding);
      ...
      item.radiation_dose_dlp = Set(self.radiation_dose_dlp);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/ct_scan_test_results/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
