...
// inline structured CT report. State is persisted to localStorage so a
...
const STORAGE_KEY = 'ct-scan-test-result.front-end-with-html.v1';
 ...
 *
 * @returns {import('./types.js').CtScanResult}
 */
...
/** @param {import('./types.js').CtScanResult} state */
...
/** @type {import('./types.js').CtScanResult} */
let state = loadState();
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The performed examination, contrast, technique, and adequacy.'
  });
  ...
  card.appendChild(textArea({
    label: 'Technique',
    field: 'technique', rows: 3,
    placeholder: 'Phases, scan parameters, reconstructions, protocol deviations…'
  }));
...
function renderStep4() {
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'acuteFinding', label: 'Acute finding' },
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'A haemorrhage or infarct auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Measurements',
    description: 'Key measurements and radiation dose for audit.'
  });
  ...
  card.appendChild(numberInput({
    label: 'Radiation dose — DLP (mGy.cm)',
    field: 'radiationDoseDlp',
    min: 0, max: 100000, step: 0.1,
    hint: 'Dose-length product for the examination (IR(ME)R dose audit).'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    ...
    'critical-finding-alert': state.haemorrhage || state.infarct,
...
const STEP_SLOTS = {
  ...
  2: [['bodyRegion'], ['contrastUsed'], ['examinationAdequacy'], ['technique']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['findingsNarrative'],
      ['acuteFinding', 'massOrLesion', 'haemorrhage', 'infarct', 'fracture',
       'infectionInflammation', 'obstruction', 'incidentalFinding']],
  5: [['largestLesionSizeMm'], ['radiationDoseDlp']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Radiation dose (DLP)',
      state.radiationDoseDlp === null ? 'Not recorded' : `${state.radiationDoseDlp} mGy.cm`],
