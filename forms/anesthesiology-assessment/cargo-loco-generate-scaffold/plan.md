...
Generate Loco (loco.rs) scaffolding for the `anesthesiology-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name anesthesiology-assessment --db postgres --bg async --assets serverside
   ```
