...
const STORAGE_KEY =
  'coagulation-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
function setTest(field, checked) {
  state.tests[field] = !!checked;
  saveState(state);
  updateProgress();
  updateTestCount();
}
...
/**
 * Render the nine coagulation tests as a Lily `.checkbox-group` of
 * `.checkbox-input` toggles inside a `role="group"` fieldset. A live count of
 * selected tests sits above the group; a missing-field error message sits
 * below it.
 */
function testCheckboxGroup() {
  const wrapper = document.createElement('fieldset');
  wrapper.className = 'field';
  wrapper.id = 'tests-fieldset';

  const legend = document.createElement('legend');
  legend.className = 'label';
  legend.innerHTML = 'Requested coagulation tests <span class="req" aria-hidden="true">*</span>';
  ...
  group.className = 'checkbox-group test-grid';
  group.setAttribute('role', 'group');
  group.setAttribute('aria-labelledby', wrapper.id);

  for (const t of TESTS) {
    const cbId = `tests-${t.field}`;
    const checked = state.tests[t.field] === true ? ' checked' : '';
    const label = document.createElement('label');
    label.htmlFor = cbId;
    label.innerHTML = `
      <input class="checkbox-input" type="checkbox" id="${cbId}" name="tests" value="${esc(t.field)}"${checked}>
      <span class="test-label">
        <span class="test-name">${esc(t.label)}</span>
        ${t.hint ? `<span class="test-hint">${esc(t.hint)}</span>` : ''}
      </span>
    `;
    const input = label.querySelector('input');
    input.addEventListener('change', () => {
      setTest(t.field, input.checked);
  ...
  errSpan.id = 'tests-error';
  wrapper.appendChild(errSpan);
  return wrapper;
}

function updateTestCount() {
  const el = document.getElementById('tests-count');
  if (!el) return;
  const n = countSelectedTests(state.tests);
  el.textContent = n === 0
...
const INDICATION_OPTIONS = [
  ...
  { value: 'suspected-dvt-pe', label: 'Suspected DVT / PE' },
  { value: 'pre-operative', label: 'Pre-operative' },
  { value: 'thrombophilia-investigation', label: 'Thrombophilia investigation' },
  { value: 'liver-disease', label: 'Liver disease' },
  { value: 'disseminated-intravascular-coagulation', label: 'Disseminated intravascular coagulation' },
...
const CITRATE_FILL_OPTIONS = [
  { value: 'adequate', label: 'Adequate (filled to the line)' },
  { value: 'underfilled', label: 'Under-filled' },
  { value: 'overfilled', label: 'Over-filled' }
];

const RATIO_OPTIONS = [
  { value: 'yes', label: 'Yes (9:1 confirmed)' },
  { value: 'no', label: 'No' },
...
function renderStep1() {
  const card = sectionCard({
    ...
    description: 'Identify the clinician raising this coagulation request.'
  });
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Select one or more coagulation tests. At least one is required.'
  });
  card.appendChild(testCheckboxGroup());
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Indication and details drive every grading axis — they are the highest-value fields.'
  });
  card.appendChild(selectInput({
  ...
  card.appendChild(subHead('Anticoagulant'));
  card.appendChild(boolField({ label: 'Patient is currently on an anticoagulant', section: 'clinical', field: 'onAnticoagulant' }));
  card.appendChild(textInput({ label: 'Anticoagulant agent', section: 'clinical', field: 'anticoagulantAgent', placeholder: 'e.g. warfarin, apixaban, LMWH' }));
  card.appendChild(subHead('Bleeding / thrombosis context'));
  card.appendChild(boolField({ label: 'Personal or family bleeding history', section: 'clinical', field: 'bleedingHistory' }));
  card.appendChild(boolField({ label: 'Personal or family thrombosis history', section: 'clinical', field: 'thrombosisHistory' }));
  card.appendChild(boolField({ label: 'Active major bleeding now (auto-escalates triage to STAT)', section: 'clinical', field: 'activeBleeding' }));
  card.appendChild(boolField({ label: 'Suspected disseminated intravascular coagulation (DIC)', section: 'clinical', field: 'suspectedDic' }));
  card.appendChild(boolField({ label: 'For suspected DVT / PE: 2-level Wells score is "unlikely"', section: 'clinical', field: 'wellsUnlikely' }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Specimen / pre-analytical',
    description: 'Sodium-citrate tube fill, 9:1 ratio, and analysis timing determine the pre-analytical band.'
  });
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Specimen collected', section: 'specimen', field: 'specimenCollected', options: SPECIMEN_COLLECTED_OPTIONS }),
    textInput({ label: 'Collection date / time', section: 'specimen', field: 'collectionDatetime', type: 'datetime-local' })
  ]));
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Citrate tube fill', section: 'specimen', field: 'citrateTubeFill', options: CITRATE_FILL_OPTIONS }),
    selectInput({ label: '9:1 blood-to-citrate ratio correct?', section: 'specimen', field: 'citrateRatioCorrect', options: RATIO_OPTIONS })
  ]));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Triage',
    description: 'Requested urgency, site, setting, and notes.'
  });
  card.appendChild(grid('two-col', [
  ...
  card.appendChild(grid('two-col', [
    textInput({ label: 'Site / clinic name', section: 'triage', field: 'siteName' }),
...
function renderStep7() {
  const card = sectionCard({
    stepNumber: 7,
    title: 'Review & submit',
    description: 'Submit to compute the four-axis grade (appropriateness, pre-analytical, completeness, triage) and safety flags.'
  });
  const p = document.createElement('p');
  p.className = 'muted';
  p.textContent = 'Check your answers above, then submit. The computed vetting report appears below the form.';
  card.appendChild(p);
  return card;
}
...
function updateProgress() {
  ...
  // The "tests" step counts as answered when at least one test is selected.
  sectionTotal['tests'] = 1;
  if (countSelectedTests(state.tests) > 0) {
    answered++;
    sectionAnswered['tests'] = 1;
  }
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, section: 'tests',     title: 'Tests' },
  { step: 4, section: 'clinical',  title: 'Clinical' },
  { step: 5, section: 'specimen',  title: 'Specimen' },
  { step: 6, section: 'triage',    title: 'Triage' },
  { step: 7, section: 'review',    title: 'Review' }
];
...
function validateForm() {
  ...
  // At least one coagulation test must be selected.
  if (countSelectedTests(state.tests) === 0) {
    errors.push({ id: 'tests-fieldset', message: 'Select at least one coagulation test' });
    const el = document.getElementById('tests-error');
    if (el) el.textContent = 'Select at least one coagulation test';
  } else {
    const el = document.getElementById('tests-error');
    if (el) el.textContent = '';
  }
...
function renderReport() {
  ...
  const preValue = preanalyticalBand
    ? `<span class="band-badge pre-${esc(preanalyticalBand)}">${esc(titleCase(preanalyticalBand))}</span>`
  ...
  out.innerHTML = `
    <h2>Coagulation Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Indication: ${esc(indicationLabel(state.clinical.primaryIndication) || '—')}</p>
    <p class="muted">Tests: ${testsSummary}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Pre-analytical specimen safety</span>
        <span class="axis-value">${preValue}</span>
      </div>
      <div class="axis-card">
