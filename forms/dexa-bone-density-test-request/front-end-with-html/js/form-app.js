...
import { emptyRequest, indicationLabel, scanRegionLabel } from './types.js';

// DEXA Bone Density Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'dexa-bone-density-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 6;
...
const INDICATION_OPTIONS = [
  { value: 'osteoporosis-screening', label: 'Osteoporosis screening' },
  { value: 'fragility-fracture', label: 'Fragility fracture' },
  { value: 'long-term-steroids', label: 'Long-term steroids' },
  { value: 'early-menopause', label: 'Early menopause' },
  { value: 'high-frax-risk', label: 'High FRAX risk' },
  { value: 'monitoring-treatment', label: 'Monitoring treatment' },
  { value: 'secondary-osteoporosis', label: 'Secondary osteoporosis' },
  { value: 'other', label: 'Other' }
];
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'Patient demographics and pregnancy status.'
  });
  card.appendChild(grid('two-col', [
  ...
  card.appendChild(selectInput({
    label: 'Pregnancy status', section: 'patient', field: 'pregnancyStatus',
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Scan region, indication, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Requested scan region', section: 'request', field: 'scanRegion',
    options: SCAN_REGION_OPTIONS, required: true
  }));
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Establish baseline BMD to guide whether to start treatment.'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Fracture-risk factors',
    description: 'FRAX inputs and risk factors. A recent fragility fracture or very high FRAX escalates triage.'
  });
  card.appendChild(grid('two-col', [
    textInput({ label: 'FRAX major-fracture probability', section: 'riskFactors', field: 'fraxMajorFracturePercent', type: 'number', min: 0, max: 100, step: 0.1, unit: '%' }),
    textInput({ label: 'Weight', section: 'riskFactors', field: 'weightKg', type: 'number', min: 0, max: 500, step: 0.1, unit: 'kg' })
  ]));
  card.appendChild(selectInput({
    label: 'Menopause status', section: 'riskFactors', field: 'menopauseStatus',
    options: MENOPAUSE_OPTIONS
  }));
  card.appendChild(subHead('Risk factors'));
  card.appendChild(boolField({ label: 'Previous fragility (low-trauma) fracture', section: 'riskFactors', field: 'previousFragilityFracture' }));
  card.appendChild(boolField({ label: 'Long-term oral glucocorticoid (steroid) therapy', section: 'riskFactors', field: 'longTermSteroids' }));
  card.appendChild(boolField({ label: 'Parental hip fracture', section: 'riskFactors', field: 'parentalHipFracture' }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Previous DEXA',
    description: 'Prior bone-density result and date — used to detect a possible duplicate request.'
  });
  card.appendChild(grid('two-col', [
    selectInput({
      label: 'Previous DEXA result', section: 'previousDexa', field: 'previousDexa',
      options: PREVIOUS_DEXA_OPTIONS
    }),
    textInput({ label: 'Previous DEXA date', section: 'previousDexa', field: 'previousDexaDate', type: 'date' })
  ]));
  return card;
}
...
const TRACKED_FIELDS = [
  ...
  // 3 Request
  ['request', 'scanRegion'], ['request', 'primaryIndication'],
  ['request', 'clinicalQuestion'],
  // 4 Risk factors
  ['riskFactors', 'fraxMajorFracturePercent'], ['riskFactors', 'menopauseStatus'],
  ['riskFactors', 'weightKg'],
  // 5 Previous DEXA
  ['previousDexa', 'previousDexa'],
  // 6 Triage
...
const SECTION_BY_STEP = {
  ...
  4: 'riskFactors', 5: 'previousDexa', 6: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 5, section: 'previousDexa', title: 'Previous DEXA' },
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>DEXA Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Region: ${esc(scanRegionLabel(state.request.scanRegion) || '—')} · Indication: ${esc(indicationLabel(state.request.primaryIndication) || '—')}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        ...
        <span class="axis-value">${doseValue}</span>
        ${safetyNote ? `<p class="muted">${esc(safetyNote)}</p>` : ''}
      </div>
      <div class="axis-card">
