...
use crate::models::_entities::dexa_bone_density_test_request_grades::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.dexa_bone_density_test_request_id = Set(self.dexa_bone_density_test_request_id);
      ...
      item.safety_note = Set(self.safety_note.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/dexa_bone_density_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
