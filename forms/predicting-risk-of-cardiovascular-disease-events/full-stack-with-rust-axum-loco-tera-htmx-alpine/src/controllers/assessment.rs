...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "cholesterol_lipids" => "cholesterolLipids",
...
async fn show_report(
    ...
    // Run grading engine
    let (risk_category, ten_year_risk, thirty_year_risk, fired_rules) =
    ...
    let grading_result = crate::engine::types::GradingResult {
        ...
        thirty_year_risk_percent: thirty_year_risk,
    ...
    context.insert(
        "risk_label",
        &crate::engine::utils::risk_category_label(&risk_category),
    );
    context.insert("ten_year_risk", &ten_year_risk);
    context.insert("thirty_year_risk", &thirty_year_risk);
