...
Generate Loco (loco.rs) scaffolding for the `predicting-risk-of-cardiovascular-disease-events` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name predicting-risk-of-cardiovascular-disease-events --db postgres --bg async --assets serverside
   ```
