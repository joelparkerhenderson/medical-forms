...
/// Pure function: evaluates all gynecology rules against assessment data.
/// Returns the severity level, composite score (0-100), and fired rules.
/// If fewer than 2 items are answered, returns "draft" status.
pub fn calculate_severity(data: &AssessmentData) -> (SeverityLevel, f64, Vec<FiredRule>) {
    ...
    // Determine severity level from composite score and fired rules
    ...
    let level = if has_high_concern {
        "urgent"
    } else if score >= 75.0 || has_medium_concern {
        "severe"
    } else if score >= 50.0 {
        "moderate"
    } else if score >= 25.0 {
