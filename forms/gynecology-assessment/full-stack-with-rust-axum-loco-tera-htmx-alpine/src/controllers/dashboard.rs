...
async fn dashboard(
    ...
    // Referral needed filter
    if let Some(ref referral) = params.referral_needed {
        if !referral.is_empty() {
            items.retain(|row| row.referral_needed == *referral);
        }
    }
    ...
    context.insert("referral_needed", &params.referral_needed.unwrap_or_default());
