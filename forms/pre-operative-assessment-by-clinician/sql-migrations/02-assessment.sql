...
COMMENT ON TABLE assessment IS
    'Top-level clinician-driven pre-operative assessment instance. Parent entity for the 16 wizard steps and the grading result.';
...
COMMENT ON COLUMN assessment.status IS
    'Lifecycle status: draft (being filled), submitted (clinician signed), reviewed (MDT-reviewed), urgent (escalation required).';
