...
import { abnormalitySeverityClass, abnormalitySeverityLabel, contrastUsedLabel, emptyResult, examinationAdequacyLabel, followUpUrgencyClass, followUpUrgencyLabel, priorityLabel, recommendationLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel, studyTypeLabel } from './types.js';

// Fluoroscopy Test Result — reporting-clinician wizard
...
// inline structured radiology report. State is persisted to localStorage so a
...
const STORAGE_KEY = 'fluoroscopy-test-result.front-end-with-html.v1';

// Numeric report fields: blank ('') inputs are stored as null, so rehydration
// must accept either a finite number or null for these keys.
const NULLABLE_NUMERIC_FIELDS = ['screeningTimeMinutes'];

/** @returns {import('./types.js').FluoroscopyResult} */
...
function mergeIntoDefaults(parsed) {
  ...
  for (const key of Object.keys(fresh)) {
    ...
    if (NULLABLE_NUMERIC_FIELDS.includes(key)) {
      // Null-safe rehydration: number | null only; anything else resets.
...
/** @param {import('./types.js').FluoroscopyResult} state */
...
/** @type {import('./types.js').FluoroscopyResult} */
let state = loadState();
...
function textInput(opts) {
  ...
  input.addEventListener('input', () => {
    let v = input.value;
    if (type === 'number') {
      // Empty numeric input is stored as null (never '' or NaN).
      v = v === '' ? null : Number(v);
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The performed study, contrast used, adequacy, and screening time.'
  });
  ...
  card.appendChild(selectInput({
    ...
    options: [
      { value: 'barium-swallow', label: 'Barium swallow' },
      { value: 'barium-meal', label: 'Barium meal' },
      { value: 'barium-follow-through', label: 'Barium follow-through' },
      { value: 'barium-enema', label: 'Barium enema' },
      { value: 'water-soluble-contrast-swallow', label: 'Water-soluble contrast swallow' },
      { value: 'defecating-proctogram', label: 'Defecating proctogram' },
      { value: 'hysterosalpingogram', label: 'Hysterosalpingogram' },
      { value: 'micturating-cystourethrogram', label: 'Micturating cystourethrogram' },
      { value: 'arthrogram', label: 'Arthrogram' },
      { value: 'fluoroscopy-guided-procedure', label: 'Fluoroscopy-guided procedure' },
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'water-soluble-iodinated', label: 'Water-soluble iodinated' },
  ...
  card.appendChild(textInput({
    label: 'Screening time (minutes)',
    field: 'screeningTimeMinutes',
    type: 'number', min: 0, max: 600, step: 0.1, unit: 'minutes',
    hint: 'Total fluoroscopy screening time, for IR(ME)R dose audit.'
  }));
...
function renderStep4() {
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'stricture', label: 'Stricture' },
      { field: 'reflux', label: 'Reflux' },
      { field: 'obstruction', label: 'Obstruction' },
      { field: 'perforationOrLeak', label: 'Perforation or leak' },
      { field: 'fistula', label: 'Fistula' },
      { field: 'fillingDefect', label: 'Filling defect' },
      { field: 'dysmotility', label: 'Dysmotility' },
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'A perforation / contrast leak or high-grade obstruction auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep5() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Structured-reporting category / label recorded on the report.',
    placeholder: 'e.g. benign-stricture, indeterminate'
  }));
  card.appendChild(textArea({
...
function renderStep6() {
  const card = sectionCard({
    ...
    description: 'Record direct communication of any critical / urgent result to the referrer.'
  });
...
function renderStep7() {
  const card = sectionCard({
    ...
    description: 'Live four-axis interpretation grade, interpretation notes, and sign-off.'
  });
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'This report contains a critical finding. Communicate the result directly to the referrer and record it on the critical-communication step before signing.'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'critical-finding-alert': state.perforationOrLeak || state.obstruction,
...
// counts only when ticked; a numeric field counts once non-null). This lets
// the checkbox-only structured-findings slot count as answered once any
// finding is recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['studyType'], ['contrastUsed'], ['examinationAdequacy'], ['screeningTimeMinutes']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['findingsNarrative'],
      ['stricture', 'reflux', 'obstruction', 'perforationOrLeak', 'fistula',
       'fillingDefect', 'dysmotility', 'normalStudy', 'incidentalFinding']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Contrast used', contrastUsedLabel(state.contrastUsed)],
    ['Examination adequacy', examinationAdequacyLabel(state.examinationAdequacy)],
    ['Screening time', state.screeningTimeMinutes === null
      ? 'Not recorded'
      : `${state.screeningTimeMinutes} minutes`],
