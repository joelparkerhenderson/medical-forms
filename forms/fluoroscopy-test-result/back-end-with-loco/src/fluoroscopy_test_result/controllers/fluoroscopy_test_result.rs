...
use crate::models::_entities::fluoroscopy_test_results::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.screening_time_minutes = Set(self.screening_time_minutes);
      ...
      item.stricture = Set(self.stricture);
      item.reflux = Set(self.reflux);
      item.obstruction = Set(self.obstruction);
      item.perforation_or_leak = Set(self.perforation_or_leak);
      item.fistula = Set(self.fistula);
      item.filling_defect = Set(self.filling_defect);
      item.dysmotility = Set(self.dysmotility);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/fluoroscopy_test_results/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
