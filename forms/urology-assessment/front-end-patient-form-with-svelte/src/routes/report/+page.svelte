<script lang="ts">
	...
	import { ipssScoreLabel, ipssScoreColor, calculateAge, qolLabel } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `urology-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			...
			{#if result.additionalFlags.length > 0}
				<div class="mb-6 rounded-xl border border-red-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-red-800">Flagged Issues for Urologist</h2>
					<div class="space-y-2">
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">IPSS Score Breakdown</h2>
			...
			<!-- Renal Function -->
			<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
				<h2 class="mb-4 text-lg font-bold text-gray-900">Renal Function & Lab Results</h2>
				<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
					<div>
						<span class="font-medium text-gray-600">Creatinine:</span>
						{data.renalFunction.creatinine !== null ? `${data.renalFunction.creatinine} µmol/L` : 'N/A'}
					</div>
					<div>
					...
					<div>
						<span class="font-medium text-gray-600">PSA:</span>
						{data.renalFunction.psa !== null ? `${data.renalFunction.psa} ng/mL` : 'N/A'}
					</div>
					<div>
						<span class="font-medium text-gray-600">Urinalysis:</span>
						{data.renalFunction.urinalysis || 'N/A'}
					</div>
				</div>
			</div>
			...
			{#if data.currentMedications.alphaBlockers.length > 0 || data.currentMedications.fiveAlphaReductaseInhibitors.length > 0 || data.currentMedications.anticholinergics.length > 0 || data.currentMedications.otherMedications.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					...
					{#if data.currentMedications.alphaBlockers.length > 0}
						<h3 class="text-sm font-semibold text-gray-600">Alpha Blockers</h3>
						<ul class="list-disc space-y-1 pl-5 text-sm mb-3">
							{#each data.currentMedications.alphaBlockers as med}
								<li>{med.name} {med.dose} {med.frequency}</li>
							{/each}
						</ul>
					{/if}
					{#if data.currentMedications.fiveAlphaReductaseInhibitors.length > 0}
						<h3 class="text-sm font-semibold text-gray-600">5-Alpha Reductase Inhibitors</h3>
						<ul class="list-disc space-y-1 pl-5 text-sm mb-3">
							{#each data.currentMedications.fiveAlphaReductaseInhibitors as med}
								<li>{med.name} {med.dose} {med.frequency}</li>
							{/each}
						</ul>
					{/if}
					{#if data.currentMedications.anticholinergics.length > 0}
						<h3 class="text-sm font-semibold text-gray-600">Anticholinergics</h3>
						<ul class="list-disc space-y-1 pl-5 text-sm mb-3">
							{#each data.currentMedications.anticholinergics as med}
								<li>{med.name} {med.dose} {med.frequency}</li>
							{/each}
						</ul>
					{/if}
					{#if data.currentMedications.otherMedications.length > 0}
						<h3 class="text-sm font-semibold text-gray-600">Other Medications</h3>
