...
use crate::engine::{severity_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "prostate_assessment" => "prostateAssessment",
        "bladder_assessment" => "bladderAssessment",
        "stone_disease" => "stoneDisease",
        "urological_cancers" => "urologicalCancers",
        "sexual_function" => "sexualFunction",
...
async fn show_report(
    ...
    let (severity_level, severity_score, fired_rules) =
        severity_grader::calculate_severity(&assessment_data);
    ...
    let ipss_total = crate::engine::utils::calculate_ipss_total(&assessment_data);
    let ipss_cat = crate::engine::utils::ipss_category(ipss_total);
    ...
    context.insert("ipss_category", &ipss_cat);
    context.insert("ipss_total", &ipss_total);
