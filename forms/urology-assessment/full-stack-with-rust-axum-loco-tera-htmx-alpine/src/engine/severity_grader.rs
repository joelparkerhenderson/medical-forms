...
pub fn calculate_severity(data: &AssessmentData) -> (SeverityLevel, f64, Vec<FiredRule>) {
    ...
    // Determine severity level from composite score and high-concern rules
    let has_high = fired_rules.iter().any(|r| r.concern_level == "high");
    let has_medium = fired_rules.iter().any(|r| r.concern_level == "medium");

    let level = if has_high || score > 75.0 {
        "urgent"
    } else if has_medium || score > 50.0 {
        "severe"
    } else if score > 25.0 {
        "moderate"
    } else {
