...
-- Additional flags detected during asthma assessment grading.
...
COMMENT ON TABLE grading_additional_flag IS
    'Additional flags detected during asthma assessment grading (e.g. frequent exacerbations, ICU history, poor adherence, occupational triggers).';
...
COMMENT ON COLUMN grading_additional_flag.flag_id IS
    'Identifier of the flag (e.g. FLAG-ICU-001, FLAG-ADH-001).';
COMMENT ON COLUMN grading_additional_flag.category IS
    'Category of the flag (e.g. Exacerbation Risk, Medication Adherence, Occupational).';
