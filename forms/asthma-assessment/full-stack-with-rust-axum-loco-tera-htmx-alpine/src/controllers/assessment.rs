...
use crate::engine::{asthma_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "asthma_history" => "asthmaHistory",
        "symptom_assessment" => "symptomAssessment",
        "lung_function" => "lungFunction",
        "triggers_exacerbations" => "triggersExacerbations",
        "current_medications" => "currentMedications",
        "inhaler_technique" => "inhalerTechnique",
        "comorbidities" => "comorbidities",
        "lifestyle_environment" => "lifestyleEnvironment",
        "review_management_plan" => "reviewManagementPlan",
        other => other,
    }
}
...
async fn show_report(
    ...
    // Run grading engine
    let (control_level, gina_criteria_met, fired_rules) =
        asthma_grader::calculate_control(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        control_level: control_level.clone(),
        gina_criteria_met,
    ...
    let fev1_pct = crate::engine::utils::fev1_percent_predicted(&assessment_data);
    let pf_pct = crate::engine::utils::peak_flow_percent_best(&assessment_data);

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("control_level", &control_level);
    context.insert("control_label", &crate::engine::utils::control_level_label(&control_level));
    context.insert("gina_criteria_met", &gina_criteria_met);
    context.insert("fev1_percent", &fev1_pct);
    context.insert("peak_flow_percent", &pf_pct);
