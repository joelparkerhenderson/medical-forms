...
Generate Loco (loco.rs) scaffolding for the `emergency-medical-technician-psychomotor-examination` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name emergency-medical-technician-psychomotor-examination --db postgres --bg async --assets serverside
   ```
