...
Generate Loco (loco.rs) scaffolding for the `attention-deficit-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name attention-deficit-assessment --db postgres --bg async --assets serverside
   ```
