...
struct DashboardParams {
    ...
    treatment_status: Option<String>,
}
...
async fn dashboard(
    ...
    if let Some(ref status) = params.treatment_status {
        if !status.is_empty() {
            items.retain(|row| row.treatment_status == *status);
        }
    }
    ...
    context.insert("treatment_status", &params.treatment_status.unwrap_or_default());
