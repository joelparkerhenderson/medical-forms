...
const STORAGE_KEY =
  'mri-scan-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 8;
...
const GADOLINIUM_REACTION_OPTIONS = [
  { value: 'none', label: 'None' },
...
function renderStep2() {
  ...
  card.appendChild(textInput({ label: 'Weight', section: 'patient', field: 'weightKg', type: 'number', min: 0, max: 500, step: 0.1, unit: 'kg' }));
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Body region, indication, and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
    label: 'Body region', section: 'request', field: 'bodyRegion',
    options: BODY_REGION_OPTIONS, required: true
  }));
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Exclude metastatic disease in the liver.'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Contrast & renal',
    description: 'Gadolinium contrast and renal / NSF risk; gadolinium with eGFR < 30 is contraindicated.'
  });
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Contrast required', section: 'contrast', field: 'contrastRequired', options: CONTRAST_OPTIONS }),
    textInput({ label: 'eGFR', section: 'contrast', field: 'egfr', type: 'number', min: 0, max: 200, step: 0.1, unit: 'mL/min/1.73m²' })
  ]));
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Previous gadolinium reaction', section: 'contrast', field: 'previousGadoliniumReaction', options: GADOLINIUM_REACTION_OPTIONS }),
    selectInput({ label: 'Pregnancy status', section: 'contrast', field: 'pregnancyStatus', options: PREGNANCY_OPTIONS })
  ]));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'MRI safety screen',
    description: 'Ferromagnetic and electronic implant screen. Positive items drive the MRI safety band toward needs-review or contraindicated.'
  });
  card.appendChild(subHead('Absolute / high-risk'));
  card.appendChild(boolField({ label: 'Cardiac pacemaker or ICD', section: 'safety', field: 'pacemakerOrIcd' }));
  card.appendChild(boolField({ label: 'Intracranial aneurysm clip', section: 'safety', field: 'aneurysmClip' }));
  card.appendChild(boolField({ label: 'Metallic foreign body in the eye / orbit', section: 'safety', field: 'metallicForeignBodyEye' }));
  card.appendChild(boolField({ label: 'Shrapnel or embedded metal fragments', section: 'safety', field: 'shrapnelOrMetalFragments' }));

  card.appendChild(subHead('Conditional'));
  card.appendChild(boolField({ label: 'Cochlear implant', section: 'safety', field: 'cochlearImplant' }));
  card.appendChild(boolField({ label: 'Programmable CSF shunt', section: 'safety', field: 'programmableShunt' }));
  card.appendChild(boolField({ label: 'Neurostimulator (DBS / SCS / VNS)', section: 'safety', field: 'neurostimulator' }));
  card.appendChild(boolField({ label: 'Metal implant or prosthesis', section: 'safety', field: 'metalImplantOrProsthesis' }));
  card.appendChild(boolField({ label: 'Insulin pump or infusion device', section: 'safety', field: 'insulinPump' }));

  card.appendChild(subHead('Logistical'));
  card.appendChild(boolField({ label: 'Claustrophobia', section: 'safety', field: 'claustrophobia' }));

  card.appendChild(selectInput({
    label: 'Preliminary MRI safety status', section: 'safety', field: 'mriSafetyStatus',
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Prior imaging',
    description: 'Relevant previous imaging available for comparison.'
  });
  card.appendChild(textArea({
    label: 'Relevant previous imaging', section: 'priorImaging', field: 'relevantPreviousImaging',
    rows: 3, placeholder: 'e.g. CT chest 2026-01-12 — indeterminate nodule.'
  }));
  return card;
}
...
function renderStep7() {
  const card = sectionCard({
    stepNumber: 7,
    title: 'Logistics',
    description: 'Bore / table weight fit, care setting, and site.'
  });
  card.appendChild(selectInput({
    label: 'Weight versus bore / table limit', section: 'logistics', field: 'weightVersusBoreLimit',
    options: BORE_LIMIT_OPTIONS
  }));
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Care setting', section: 'logistics', field: 'setting', options: SETTING_OPTIONS }),
    textInput({ label: 'Scanning site', section: 'logistics', field: 'siteName' })
  ]));
  return card;
}
...
const TRACKED_FIELDS = [
  ...
  // 3 Request
  ['request', 'bodyRegion'], ['request', 'primaryIndication'],
  ['request', 'clinicalQuestion'],
  // 4 Contrast
  ['contrast', 'contrastRequired'], ['contrast', 'pregnancyStatus'],
  // 5 Safety
  ['safety', 'mriSafetyStatus'],
  // 7 Logistics
  ['logistics', 'weightVersusBoreLimit'], ['logistics', 'setting'],
  // 8 Triage
  ['triage', 'urgency']
];

const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'request', 4: 'contrast',
  5: 'safety', 6: 'priorImaging', 7: 'logistics', 8: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 4, section: 'contrast',     title: 'Contrast' },
  { step: 5, section: 'safety',       title: 'MRI safety' },
  { step: 6, section: 'priorImaging', title: 'Prior imaging' },
  { step: 7, section: 'logistics',    title: 'Logistics' },
...
function updateStepListStatuses(sectionAnswered, sectionTotal) {
  ...
  for (const def of STEP_DEFINITIONS) {
    ...
    // Prior imaging has no tracked fields; treat as informational.
    if (t > 0 && a === t) {
...
function renderReport() {
  ...
  const safetyValue = mriSafetyBand
    ? `<span class="band-badge safety-${esc(mriSafetyBand)}">${esc(titleCase(mriSafetyBand))}</span>${
        contrastRenalFlag && contrastRenalFlag !== 'none'
          ? ` <span class="band-badge contrast-${esc(contrastRenalFlag)}">Gad / renal: ${esc(titleCase(contrastRenalFlag))}</span>`
          : ''
      }`
  ...
  out.innerHTML = `
    <h2>MRI Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Region: ${esc(bodyRegionLabel(state.request.bodyRegion) || '—')}${state.request.primaryIndication ? ` · ${esc(indicationLabel(state.request.primaryIndication))}` : ''}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · MRI safety</span>
