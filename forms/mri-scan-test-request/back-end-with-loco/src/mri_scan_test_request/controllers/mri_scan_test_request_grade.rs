...
use crate::models::_entities::mri_scan_test_request_grades::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.mri_scan_test_request_id = Set(self.mri_scan_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.mri_safety_band = Set(self.mri_safety_band.clone());
      item.contrast_renal_flag = Set(self.contrast_renal_flag.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/mri_scan_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
