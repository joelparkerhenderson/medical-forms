...
const STORAGE_KEY =
  'cardiac-stress-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
const INDICATION_OPTIONS = [
  { value: 'suspected-angina', label: 'Suspected angina' },
  { value: 'known-cad-assessment', label: 'Known CAD assessment' },
  { value: 'risk-stratification-post-mi', label: 'Risk stratification post-MI' },
  { value: 'pre-operative-cardiac', label: 'Pre-operative cardiac' },
  { value: 'exercise-tolerance', label: 'Exercise tolerance' },
  { value: 'arrhythmia-evaluation', label: 'Arrhythmia evaluation' },
  { value: 'valve-disease', label: 'Valve disease' },
  { value: 'other', label: 'Other' }
];

const AORTIC_STENOSIS_OPTIONS = [
  { value: 'none', label: 'None' },
  { value: 'mild', label: 'Mild' },
  { value: 'moderate', label: 'Moderate' },
  { value: 'severe', label: 'Severe (symptomatic)' },
...
function renderStep3() {
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Is there inducible ischaemia to explain exertional chest pain?'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Symptoms & exercise capacity',
    description: 'Presenting symptoms, ability to exercise, and resting ECG findings.'
  });
  card.appendChild(subHead('Presenting symptoms'));
  ...
  card.appendChild(subHead('Exercise capacity'));
  card.appendChild(boolField({ label: 'Able to exercise', section: 'symptoms', field: 'ableToExercise' }));
  card.appendChild(textArea({
    label: 'Resting ECG findings', section: 'symptoms', field: 'restingEcgFindings',
    rows: 2,
    placeholder: 'e.g. LBBB, paced rhythm, resting ST changes — may affect test selection.'
  }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Cardiac safety screen',
    description: 'Contraindications auto-escalate or block the request regardless of the other axes.'
  });
  card.appendChild(boolField({ label: 'Known coronary artery disease', section: 'safety', field: 'knownCoronaryArteryDisease' }));
  card.appendChild(subHead('Contraindication screen'));
  card.appendChild(boolField({ label: 'Recent acute coronary syndrome', section: 'safety', field: 'recentAcuteCoronarySyndrome' }));
  card.appendChild(selectInput({
    label: 'Aortic stenosis severity', section: 'safety', field: 'aorticStenosis',
    options: AORTIC_STENOSIS_OPTIONS
  }));
  card.appendChild(boolField({ label: 'Uncontrolled hypertension', section: 'safety', field: 'uncontrolledHypertension' }));
  card.appendChild(boolField({ label: 'Taking a beta-blocker', section: 'safety', field: 'betaBlocker' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Triage',
    description: 'Requested urgency and care setting. Red flags auto-escalate the triage tier.'
  });
  card.appendChild(grid('two-col', [
...
const TRACKED_FIELDS = [
  ...
  // 4 Symptoms
  ['symptoms', 'restingEcgFindings'],
  // 5 Safety
  ['safety', 'aorticStenosis'],
  // 6 Triage
...
const STEP_DEFINITIONS = [
  ...
  { step: 5, section: 'safety',    title: 'Safety' },
...
function renderReport() {
  ...
  const safetyValue = contraindicationBand
    ? `<span class="band-badge safety-${esc(contraindicationBand)}">${esc(titleCase(contraindicationBand))}</span>`
  ...
  out.innerHTML = `
    <h2>Stress Test Request Vetting Report</h2>
