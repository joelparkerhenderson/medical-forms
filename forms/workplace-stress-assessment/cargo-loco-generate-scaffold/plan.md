...
Generate Loco (loco.rs) scaffolding for the `workplace-stress-assessment` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name workplace-stress-assessment --db postgres --bg async --assets serverside
   ```
