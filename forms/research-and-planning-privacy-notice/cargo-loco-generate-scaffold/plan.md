...
Generate Loco (loco.rs) scaffolding for the `research-and-planning-privacy-notice` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name research-and-planning-privacy-notice --db postgres --bg async --assets serverside
   ```
