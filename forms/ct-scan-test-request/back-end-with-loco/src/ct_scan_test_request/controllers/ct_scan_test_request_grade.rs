...
use crate::models::_entities::ct_scan_test_request_grades::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.ct_scan_test_request_id = Set(self.ct_scan_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.contrast_safety_band = Set(self.contrast_safety_band.clone());
      item.estimated_dose_band = Set(self.estimated_dose_band.clone());
      item.renal_risk = Set(self.renal_risk);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/ct_scan_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
