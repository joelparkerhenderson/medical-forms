...
import { bodyRegionLabel, contrastLabel, emptyRequest } from './types.js';

// CT Scan Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY = 'ct-scan-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 8;
...
const BODY_REGION_OPTIONS = [
  ...
  { value: 'abdomen-pelvis', label: 'Abdomen / pelvis' },
  { value: 'spine', label: 'Spine' },
  { value: 'ct-angiogram', label: 'CT angiogram' },
...
const INDICATION_OPTIONS = [
  ...
  { value: 'renal-colic', label: 'Renal colic' },
  { value: 'infection-abscess', label: 'Infection / abscess' },
  { value: 'pre-surgical-planning', label: 'Pre-surgical planning' },
...
const CONTRAST_OPTIONS = [
  ...
  { value: 'iv-iodinated', label: 'IV iodinated' },
  { value: 'oral', label: 'Oral' },
  { value: 'both', label: 'IV + oral' },
...
function renderStep3() {
  ...
  card.appendChild(selectInput({
    label: 'Requested body region', section: 'request', field: 'bodyRegion',
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Exclude intracranial haemorrhage in suspected acute stroke.'
  }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Relevant history and any previous imaging being followed up or compared.'
  });
  card.appendChild(textArea({ label: 'Relevant history', section: 'context', field: 'relevantHistory', rows: 3 }));
  card.appendChild(textArea({ label: 'Relevant previous imaging', section: 'context', field: 'relevantPreviousImaging', rows: 2 }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Contrast & renal safety',
    description: 'Contrast requirement and renal-safety factors — drive the contrast-safety band.'
  });
  card.appendChild(selectInput({
    label: 'Contrast required', section: 'contrast', field: 'contrastRequired',
    options: CONTRAST_OPTIONS
  }));
  card.appendChild(grid('two-col', [
    textInput({ label: 'eGFR', section: 'contrast', field: 'egfr', type: 'number', min: 0, max: 200, step: 0.1, unit: 'mL/min/1.73m²' }),
    selectInput({ label: 'Previous contrast reaction', section: 'contrast', field: 'previousContrastReaction', options: REACTION_OPTIONS })
  ]));
  card.appendChild(subHead('Safety factors'));
  card.appendChild(boolField({ label: 'Known iodinated-contrast allergy', section: 'contrast', field: 'iodineContrastAllergy' }));
  card.appendChild(boolField({ label: 'Taking metformin', section: 'contrast', field: 'metformin' }));
  card.appendChild(boolField({ label: 'Diabetes', section: 'contrast', field: 'diabetes' }));
  card.appendChild(boolField({ label: 'Known renal impairment', section: 'contrast', field: 'renalImpairment' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    ...
    description: 'Pregnancy status and the IR(ME)R radiation justification for this exposure.'
  });
  card.appendChild(selectInput({
    label: 'Pregnancy status', section: 'radiation', field: 'pregnancyStatus',
    options: PREGNANCY_OPTIONS
  }));
  card.appendChild(textArea({
    label: 'IR(ME)R justification', section: 'radiation', field: 'irMeRJustification',
    rows: 3,
...
function renderStep7() {
  const card = sectionCard({
    stepNumber: 7,
    title: 'Triage & setting',
    description: 'Requested urgency, care setting, and the requested-by date.'
  });
  card.appendChild(grid('two-col', [
...
function renderStep8() {
  const card = sectionCard({
    stepNumber: 8,
    title: 'Submit',
    description: 'Add any notes, then submit to compute the four-axis grade and safety flags.'
  });
...
const TRACKED_FIELDS = [
  ...
  // 5 Contrast
  ['contrast', 'contrastRequired'],
  // 6 Radiation
  ['radiation', 'pregnancyStatus'], ['radiation', 'irMeRJustification'],
  // 7 Triage
...
const SECTION_BY_STEP = {
  ...
  5: 'contrast', 6: 'radiation', 7: 'triage', 8: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 5, section: 'contrast',  title: 'Contrast' },
  { step: 6, section: 'radiation', title: 'Radiation' },
  { step: 7, section: 'triage',    title: 'Triage' },
  { step: 8, section: 'triage',    title: 'Submit' }
];
...
function renderReport() {
  ...
  const contrastValue = contrastSafetyBand
    ? `<span class="band-badge contrast-${esc(contrastSafetyBand)}">${esc(titleCase(contrastSafetyBand))}</span>${
        renalRisk ? ' <span class="muted">renal risk</span>' : ''
      }`
    : '<span class="muted">Not assessed</span>';

  const doseValue = estimatedDoseBand
    ? `<span class="band-badge dose-${esc(estimatedDoseBand)}">${esc(titleCase(estimatedDoseBand))}</span>`
    : '<span class="muted">Not assessed</span>';
  ...
  out.innerHTML = `
    <h2>CT Scan Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Region: ${esc(bodyRegionLabel(state.request.bodyRegion) || '—')} · Contrast: ${esc(contrastLabel(state.contrast.contrastRequired) || '—')}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Radiation &amp; contrast safety</span>
        <span class="axis-value">${contrastValue} ${doseValue}</span>
      </div>
      <div class="axis-card">
