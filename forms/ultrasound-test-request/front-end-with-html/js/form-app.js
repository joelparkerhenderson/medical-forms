...
// continuous wizard: every section is rendered into the page in document
// order. The user scrolls through them; a sticky top-of-page progress summary
...
const STORAGE_KEY =
  'ultrasound-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
const BODY_REGION_OPTIONS = [
  ...
  { value: 'renal-tract', label: 'Renal tract' },
  { value: 'liver-biliary', label: 'Liver / biliary' },
  { value: 'thyroid-neck', label: 'Thyroid / neck' },
  { value: 'scrotum-testes', label: 'Scrotum / testes' },
  { value: 'breast', label: 'Breast' },
  { value: 'soft-tissue', label: 'Soft tissue' },
  { value: 'vascular-doppler', label: 'Vascular Doppler' },
  { value: 'dvt-leg', label: 'DVT (leg)' },
  { value: 'carotid', label: 'Carotid' },
  { value: 'msk-joint', label: 'MSK / joint' },
...
const INDICATION_OPTIONS = [
  ...
  { value: 'suspected-gallstones', label: 'Suspected gallstones' },
  { value: 'abnormal-lfts', label: 'Abnormal LFTs' },
  { value: 'renal-impairment', label: 'Renal impairment' },
  { value: 'haematuria', label: 'Haematuria' },
  { value: 'palpable-mass', label: 'Palpable mass' },
  { value: 'suspected-dvt', label: 'Suspected DVT' },
  { value: 'suspected-aaa', label: 'Suspected AAA' },
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Body region, laterality, and the primary clinical indication — drives the appropriateness axis.'
  });
  card.appendChild(grid('two-col', [
    selectInput({
...
function renderStep4() {
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Confirm or exclude gallstones in a patient with right-upper-quadrant pain.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Preparation',
    description: 'Flag the preparation the patient must complete — drives the suitability axis.'
  });
  card.appendChild(boolField({ label: 'Fasting required (e.g. upper-abdominal / biliary)', section: 'prep', field: 'fastingRequired' }));
  card.appendChild(boolField({ label: 'Full bladder required (e.g. pelvic / lower urinary tract)', section: 'prep', field: 'fullBladderRequired' }));
  return card;
}
...
function renderStep6() {
  ...
  card.appendChild(subHead('Safety-critical red flags'));
  card.appendChild(boolField({ label: 'Suspected DVT (deep vein thrombosis)', section: 'redFlags', field: 'suspectedDvt' }));
  card.appendChild(boolField({ label: 'Suspected testicular torsion', section: 'redFlags', field: 'suspectedTesticularTorsion' }));
  card.appendChild(boolField({ label: 'Suspected AAA (abdominal aortic aneurysm)', section: 'redFlags', field: 'suspectedAaa' }));
  return card;
}
...
const TRACKED_FIELDS = [
  ...
  // 4 Clinical detail
  ['request', 'clinicalQuestion'], ['request', 'relevantHistory'],
  // 7 Triage
...
const STEP_DEFINITIONS = [
  { step: 1, title: 'Clinician',   fields: [['clinician', 'clinicianName'], ['clinician', 'clinicianRole'], ['clinician', 'referralDate']] },
  { step: 2, title: 'Patient',     fields: [['patient', 'firstName'], ['patient', 'lastName'], ['patient', 'dateOfBirth'], ['patient', 'nhsNumber']] },
  { step: 3, title: 'Examination', fields: [['request', 'bodyRegion'], ['request', 'laterality'], ['request', 'primaryIndication']] },
  { step: 4, title: 'Clinical detail', fields: [['request', 'clinicalQuestion'], ['request', 'relevantHistory']] },
  { step: 5, title: 'Preparation', fields: [] },
  { step: 6, title: 'Red flags',   fields: [] },
  { step: 7, title: 'Triage',      fields: [['triage', 'urgency'], ['triage', 'setting']] }
];
...
function updateStepListStatuses() {
  ...
  for (const def of STEP_DEFINITIONS) {
    ...
    if (!li) continue;
    const t = def.fields.length;
    let a = 0;
    for (const [section, field] of def.fields) {
      if (isAnswered(state[section][field])) a++;
    }
    if (t > 0 && a === t) {
...
function renderReport() {
  ...
  const suitabilityValue = suitabilityBand
    ? `<span class="band-badge suit-${esc(suitabilityBand)}">${esc(titleCase(suitabilityBand))}</span>${
        prepRequirements
          ? ` <span class="muted">${esc(prepRequirements)}</span>`
          : ''
      }`
  ...
  out.innerHTML = `
    ...
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Region: ${esc(bodyRegionLabel(state.request.bodyRegion) || '—')} · Indication: ${esc(indicationLabel(state.request.primaryIndication) || '—')}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Preparation / suitability</span>
        <span class="axis-value">${suitabilityValue}</span>
      </div>
      <div class="axis-card">
