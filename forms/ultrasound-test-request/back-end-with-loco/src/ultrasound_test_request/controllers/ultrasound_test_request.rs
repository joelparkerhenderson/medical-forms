...
use crate::models::_entities::ultrasound_test_requests::{ActiveModel, Entity, Model};
...
pub struct Params {
    ...
    pub full_bladder_required: bool,
    pub urgency: String,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.full_bladder_required = Set(self.full_bladder_required);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/ultrasound_test_requests/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
