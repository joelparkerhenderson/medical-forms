...
Generate Loco (loco.rs) scaffolding for the `cardiopulmonary-resuscitation-training` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name cardiopulmonary-resuscitation-training --db postgres --bg async --assets serverside
   ```
