...
// document order. The reporting clinician (endoscopist) scrolls through them;
// a sticky top-of-page progress summary reflects how many fields have been
// answered and a live four-axis interpretation preview updates as the report
// is edited. Submission runs the pure grading engine (Axis A classification,
...
// inline structured endoscopy report. State is persisted to localStorage so a
...
const STORAGE_KEY = 'colonoscopy-test-result.front-end-with-html.v1';

/** @returns {import('./types.js').ColonoscopyResult} */
...
/** @param {import('./types.js').ColonoscopyResult} state */
...
/** @type {import('./types.js').ColonoscopyResult} */
let state = loadState();
...
function renderStep1() {
  ...
  card.appendChild(textInput({
    label: 'Reporting clinician (endoscopist)',
    field: 'reportingClinician', required: true,
    placeholder: 'e.g. Dr A Endoscopist'
  }));
  card.appendChild(textInput({
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The performed procedure, extent reached, bowel preparation, and sedation.'
  });
  ...
  card.appendChild(selectInput({
    ...
    field: 'extentReached',
    hint: 'Caecal / terminal-ileal intubation is the completeness key performance indicator.',
    options: [
      { value: 'caecum', label: 'Caecum' },
      { value: 'terminal-ileum', label: 'Terminal ileum' },
      { value: 'hepatic-flexure', label: 'Hepatic flexure' },
      { value: 'splenic-flexure', label: 'Splenic flexure' },
      { value: 'descending-colon', label: 'Descending colon' },
      { value: 'rectum-sigmoid', label: 'Rectum / sigmoid' },
      { value: 'incomplete', label: 'Incomplete' }
    ]
  }));
  card.appendChild(selectInput({
    label: 'Bowel-preparation quality',
    field: 'bowelPreparationQuality',
    options: [
  ...
  card.appendChild(checkboxField({
    label: 'Sedation / analgesia administered',
    field: 'sedationUsed'
  }));
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'The clinical question the procedure was performed to answer.'
  });
...
function renderStep4() {
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'polypsFound', label: 'Polyps found' },
      { field: 'massLesion', label: 'Mass lesion' },
      { field: 'diverticulosis', label: 'Diverticulosis' },
      { field: 'inflammationIbd', label: 'Inflammation suggestive of IBD' },
      { field: 'angiodysplasia', label: 'Angiodysplasia' },
      { field: 'bleedingSourceIdentified', label: 'Bleeding source identified' },
      { field: 'normalExamination', label: 'Normal examination' }
    ]
  }));

  card.appendChild(conditionalAlert({
    id: 'mass-lesion-alert',
    type: 'error',
    heading: 'Critical finding selected',
    message: 'A mass lesion auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off and an urgent MDT / colorectal-surgical referral is arranged.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Polyps and tissue',
    description: 'Polyp count and size, tissue handling, and any complication.'
  });
  ...
  card.appendChild(textInput({
    ...
    hint: 'Number of polyps detected, for surveillance interval determination.'
  }));
  card.appendChild(textInput({
    label: 'Largest polyp (mm)',
    field: 'largestPolypMm',
    type: 'number', min: 0, max: 200, step: 0.1, unit: 'mm',
    hint: 'Largest polyp size, for surveillance and categorisation (BSG / ACPGBI / PHE).'
  }));

  card.appendChild(checkboxGroup({
    label: 'Tissue handling',
    items: [
      { field: 'biopsyTaken', label: 'Biopsy taken' },
      { field: 'polypectomyPerformed', label: 'Polypectomy performed' }
    ]
  }));
  ...
  card.appendChild(conditionalAlert({
    id: 'perforation-alert',
    type: 'error',
    heading: 'Critical complication selected',
    message: 'A perforation auto-escalates the follow-up urgency to a critical alert. Communicate the result to the referrer and arrange an urgent surgical review on sign-off.'
  }));
...
function renderStep6() {
  ...
  card.appendChild(textInput({
    ...
    hint: 'Structured-reporting label, e.g. a BSG / ACPGBI / PHE polyp-surveillance risk band.',
    placeholder: 'e.g. high-risk surveillance'
  }));
  card.appendChild(textArea({
    label: 'Recommended follow-up',
    field: 'recommendedFollowUp', rows: 3,
    placeholder: 'Recommended surveillance interval, repeat procedure, referral, or management…'
  }));
...
function renderStep7() {
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'This report contains a critical finding. Communicate the result directly to the referrer, arrange an urgent MDT / colorectal-surgical referral, and record it below before signing.'
  }));
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    'mass-lesion-alert': state.massLesion,
    'perforation-alert': state.complication === 'perforation',
...
// answered once any structured finding or tissue-handling item is recorded.
const STEP_SLOTS = {
  1: [['reportingClinician'], ['originatingRequestReference'], ['reportStatus'],
      ['performedDate'], ['reportedDate']],
  2: [['procedure'], ['extentReached'], ['bowelPreparationQuality'], ['sedationUsed']],
  3: [['clinicalHistory']],
  4: [['findingsNarrative'],
      ['polypsFound', 'massLesion', 'diverticulosis', 'inflammationIbd',
       'angiodysplasia', 'bleedingSourceIdentified', 'normalExamination']],
  5: [['polypCount'], ['largestPolypMm'], ['biopsyTaken', 'polypectomyPerformed'],
      ['complication']],
...
function renderReport() {
  ...
  const detailRows = [
    ...
    ['Extent reached', extentReachedLabel(state.extentReached)],
    ['Bowel preparation', bowelPreparationQualityLabel(state.bowelPreparationQuality)],
    ['Sedation', state.sedationUsed ? 'Yes' : 'No'],
    ...
    ['Polyp count', state.polypCount === null ? 'Not recorded' : String(state.polypCount)],
    ['Largest polyp (mm)', state.largestPolypMm === null ? 'Not recorded' : String(state.largestPolypMm)],
    ['Biopsy taken', state.biopsyTaken ? 'Yes' : 'No'],
    ['Polypectomy performed', state.polypectomyPerformed ? 'Yes' : 'No'],
