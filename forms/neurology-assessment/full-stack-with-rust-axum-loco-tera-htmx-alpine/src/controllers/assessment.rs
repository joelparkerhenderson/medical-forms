...
use crate::engine::{neurology_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "neurological_history" => "neurologicalHistory",
        "headache_assessment" => "headacheAssessment",
        "cranial_nerve_examination" => "cranialNerveExamination",
        "motor_assessment" => "motorAssessment",
        "sensory_assessment" => "sensoryAssessment",
        "reflexes_coordination" => "reflexesCoordination",
...
fn is_string_field(field: &str) -> bool {
    matches!(
        ...
        "fullName"
            ...
            | "symptomDuration"
            | "symptomOnset"
            | "previousNeurologicalCondition"
            | "seizureHistory"
            | "strokeHistory"
            | "headInjury"
            | "familyNeurologicalHistory"
            | "headacheType"
            | "headacheFrequency"
            | "auraPresent"
            | "headacheDuration"
...
async fn show_report(
    ...
    let (severity_level, severity_score, fired_rules) =
        neurology_grader::calculate_severity(&assessment_data);
