...
async fn dashboard(
    ...
    if let Some(ref term) = params.search {
        ...
        items.retain(|row| {
            row.patient_name.to_lowercase().contains(&term)
                ...
                || row.urgency.to_lowercase().contains(&term)
        });
    }
    ...
    if let Some(ref diagnosis) = params.primary_diagnosis {
        if !diagnosis.is_empty() {
            items.retain(|row| row.primary_diagnosis == *diagnosis);
        }
    }
    ...
    context.insert("primary_diagnosis", &params.primary_diagnosis.unwrap_or_default());
