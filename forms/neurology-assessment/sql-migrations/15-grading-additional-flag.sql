...
-- Additional safety-critical flags detected during neurology assessment grading.
...
COMMENT ON TABLE grading_additional_flag IS
    'Additional safety-critical flags detected during neurology assessment grading (e.g. acute stroke indicators, thunderclap headache, status epilepticus).';
...
COMMENT ON COLUMN grading_additional_flag.flag_id IS
    'Identifier of the flag (e.g. FLAG-STROKE-001, FLAG-THUNDER-001, FLAG-STATUS-001).';
COMMENT ON COLUMN grading_additional_flag.category IS
    'Category of the flag (e.g. Acute Stroke, Headache Red Flag, Seizure Emergency, Cognitive Decline).';
