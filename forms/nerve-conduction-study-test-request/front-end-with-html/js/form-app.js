...
const STORAGE_KEY =
  'nerve-conduction-study-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
const STUDY_TYPE_OPTIONS = [
  ...
  { value: 'nerve-conduction-and-emg', label: 'Nerve conduction + EMG' },
...
const INDICATION_OPTIONS = [
  ...
  { value: 'suspected-motor-neurone-disease', label: 'Suspected motor neurone disease' },
  { value: 'myopathy', label: 'Myopathy' },
  { value: 'plexopathy', label: 'Plexopathy' },
  { value: 'suspected-myasthenia', label: 'Suspected myasthenia' },
  { value: 'nerve-injury', label: 'Nerve injury' },
  { value: 'other', label: 'Other' }
];

const DURATION_OPTIONS = [
  { value: 'less-than-6-weeks', label: 'Less than 6 weeks' },
  { value: '6-weeks-to-3-months', label: '6 weeks to 3 months' },
  { value: '3-to-12-months', label: '3 to 12 months' },
...
function renderStep3() {
  const card = sectionCard({
    ...
    description: 'Study type, anatomical region, and laterality — drives the appropriateness and procedural-risk axes.'
  });
  card.appendChild(selectInput({
    label: 'Requested study type', section: 'study', field: 'studyType',
    options: STUDY_TYPE_OPTIONS, required: true
  }));
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Anatomical region', section: 'study', field: 'region', options: REGION_OPTIONS }),
    selectInput({ label: 'Laterality', section: 'study', field: 'laterality', options: LATERALITY_OPTIONS })
  ]));
  card.appendChild(textInput({ label: 'Requested-by date', section: 'study', field: 'requestedByDate', type: 'date' }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Indication and the specific clinical question — the highest-value fields.'
  });
  card.appendChild(selectInput({
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Confirm median neuropathy at the wrist and grade severity.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Symptoms',
    description: 'Presenting symptoms and their duration. Rapidly progressive weakness expedites triage.'
  });
  card.appendChild(subHead('Reported symptoms'));
  card.appendChild(boolField({ label: 'Numbness', section: 'symptoms', field: 'symptomNumbness' }));
  card.appendChild(boolField({ label: 'Weakness', section: 'symptoms', field: 'symptomWeakness' }));
  card.appendChild(boolField({ label: 'Pain', section: 'symptoms', field: 'symptomPain' }));
  card.appendChild(boolField({ label: 'Tingling / paraesthesia', section: 'symptoms', field: 'symptomTingling' }));
  card.appendChild(selectInput({ label: 'Symptom duration', section: 'symptoms', field: 'symptomDuration', options: DURATION_OPTIONS }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Safety',
    description: 'Factors affecting needle EMG and electrical stimulation — drive the procedural-risk axis.'
  });
  card.appendChild(boolField({ label: 'Diabetes', section: 'safety', field: 'diabetes' }));
  card.appendChild(boolField({ label: 'Taking anticoagulant', section: 'safety', field: 'takingAnticoagulant' }));
  card.appendChild(boolField({ label: 'Pacemaker or ICD', section: 'safety', field: 'pacemakerOrIcd' }));
  return card;
}
...
const TRACKED_FIELDS = [
  ...
  // 3 Study
  ['study', 'studyType'], ['study', 'region'], ['study', 'laterality'],
  // 4 Request
...
const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'study', 4: 'request',
  5: 'symptoms', 6: 'safety', 7: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 3, section: 'study',     title: 'Study' },
  { step: 4, section: 'request',   title: 'Indication' },
...
function updateStepListStatuses(sectionAnswered, sectionTotal) {
  ...
  for (const def of STEP_DEFINITIONS) {
    ...
    // Safety has no tracked text fields; treat as informational.
    if (t > 0 && a === t) {
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>Nerve Conduction Study Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Study: ${esc(studyTypeLabel(state.study.studyType) || '—')}${state.study.region ? ` · ${esc(regionLabel(state.study.region))}` : ''}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Procedural risk</span>
        <span class="axis-value"><span class="band-badge risk-${esc(proceduralRiskBand)}">${esc(titleCase(proceduralRiskBand))}</span></span>
      </div>
      <div class="axis-card">
