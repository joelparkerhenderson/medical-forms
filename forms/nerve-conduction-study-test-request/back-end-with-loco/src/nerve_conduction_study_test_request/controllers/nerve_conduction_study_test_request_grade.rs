...
use crate::models::_entities::nerve_conduction_study_test_request_grades::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.nerve_conduction_study_test_request_id = Set(self.nerve_conduction_study_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.procedural_risk_band = Set(self.procedural_risk_band.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/nerve_conduction_study_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
