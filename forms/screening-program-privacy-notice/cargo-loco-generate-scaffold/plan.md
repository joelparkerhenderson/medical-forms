...
Generate Loco (loco.rs) scaffolding for the `screening-program-privacy-notice` form.
...
1. Create a new Loco project (one-time):
   ...
   ```sh
   loco new --name screening-program-privacy-notice --db postgres --bg async --assets serverside
   ```
