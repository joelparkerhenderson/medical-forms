...
const STORAGE_KEY = 'genetic-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 6;
...
const INDICATION_OPTIONS = [
  { value: 'suspected-genetic-disorder', label: 'Suspected genetic disorder' },
  { value: 'familial-cancer', label: 'Familial cancer' },
  { value: 'developmental-delay', label: 'Developmental delay' },
  { value: 'congenital-anomaly', label: 'Congenital anomaly' },
  { value: 'cardiomyopathy-arrhythmia', label: 'Cardiomyopathy / arrhythmia' },
  { value: 'neuromuscular', label: 'Neuromuscular' },
  { value: 'predictive-family-history', label: 'Predictive (family history)' },
  { value: 'carrier-screening', label: 'Carrier screening' },
  { value: 'prenatal-diagnosis', label: 'Prenatal diagnosis' },
...
const SPECIMEN_OPTIONS = [
  ...
  { value: 'prenatal', label: 'Prenatal (CVS / amniotic / cffDNA)' }
];
...
function renderStep1() {
  ...
  card.appendChild(selectInput({
    ...
    options: [
      ...
      { value: 'genetic-counsellor', label: 'Genetic counsellor' },
      { value: 'gp', label: 'GP' },
...
function renderStep2() {
  ...
  card.appendChild(textInput({ label: 'Address', section: 'patient', field: 'addressLine' }));
  return card;
}
...
function renderStep3() {
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Confirm or exclude a pathogenic BRCA1/2 variant.'
  }));
  card.appendChild(textInput({ label: 'Requested-by date', section: 'request', field: 'requestedByDate', type: 'date' }));
  return card;
}
...
function renderStep4() {
  const card = sectionCard({
    ...
    description: 'Clinical details / phenotype, suspected condition, and family history.'
  });
  card.appendChild(textArea({
    label: 'Clinical details / phenotype', section: 'clinical', field: 'clinicalDetails',
    rows: 3, required: true,
    placeholder: 'Relevant clinical findings, phenotype (HPO terms), examination findings.'
  }));
  card.appendChild(textInput({
    label: 'Suspected condition / gene of interest', section: 'clinical', field: 'suspectedCondition',
    placeholder: 'e.g. Marfan syndrome, FBN1'
  }));
  card.appendChild(textArea({
    label: 'Family history', section: 'clinical', field: 'familyHistory',
    rows: 3, required: true,
    placeholder: 'Pedigree summary, affected relatives, inheritance pattern.'
  }));
  card.appendChild(boolField({ label: 'An affected relative has already been tested (familial variant known)', section: 'clinical', field: 'affectedRelativeTested' }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    ...
    description: 'Consent and pre-test counselling are mandatory for predictive / presymptomatic testing.'
  });
  card.appendChild(subHead('Consent and pre-test counselling'));
  card.appendChild(boolField({ label: 'Informed consent obtained (Record of Discussion documented)', section: 'consent', field: 'consentObtained' }));
  card.appendChild(boolField({ label: 'Pre-test genetic counselling offered or provided', section: 'consent', field: 'geneticCounsellingOffered' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Specimen & triage',
    description: 'Specimen type, urgency, setting, and notes. Submit to compute the four-axis grade and flags.'
  });
  card.appendChild(grid('two-col', [
    selectInput({ label: 'Specimen type', section: 'triage', field: 'specimenType', options: SPECIMEN_OPTIONS }),
...
const TRACKED_FIELDS = [
  ...
  // 4 Clinical
  ['clinical', 'clinicalDetails'], ['clinical', 'familyHistory'],
  // 6 Triage
  ['triage', 'specimenType'], ['triage', 'urgency']
];

// Boolean fields count as "answered" when true.
const TRACKED_BOOLS = [
  ['consent', 'consentObtained'], ['consent', 'geneticCounsellingOffered']
];

const SECTION_BY_STEP = {
  1: 'clinician', 2: 'patient', 3: 'request', 4: 'clinical',
  5: 'consent', 6: 'triage'
};
...
function updateProgress() {
  ...
  // Consent step has two boolean fields; count them toward its section.
  for (const [section, field] of TRACKED_BOOLS) {
    sectionTotal[section] = (sectionTotal[section] || 0) + 1;
    if (state[section][field] === true) {
      answered++;
      sectionAnswered[section] = (sectionAnswered[section] || 0) + 1;
    }
  }
  const total = TRACKED_FIELDS.length + TRACKED_BOOLS.length;
...
const STEP_DEFINITIONS = [
  ...
  { step: 5, section: 'consent',   title: 'Consent' },
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>Genetic Test Request Vetting Report</h2>
    <p class="muted">Generated ${esc(new Date(timestamp).toLocaleString())} · Test: ${esc(testTypeLabel(state.request.testType) || '—')} · Indication: ${esc(indicationLabel(state.request.primaryIndication) || '—')}</p>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        <span class="axis-name">B · Consent &amp; counselling</span>
        <span class="axis-value"><span class="band-badge consent-${esc(consentCounsellingBand)}">${esc(CONSENT_LABELS[consentCounsellingBand] || titleCase(consentCounsellingBand) || 'Not assessed')}</span></span>
      </div>
      <div class="axis-card">
