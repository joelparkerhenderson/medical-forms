...
use crate::models::_entities::genetic_test_request_grades::{ActiveModel, Entity, Model};
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.genetic_test_request_id = Set(self.genetic_test_request_id);
      item.appropriateness_score = Set(self.appropriateness_score);
      item.appropriateness_band = Set(self.appropriateness_band.clone());
      item.consent_counselling_band = Set(self.consent_counselling_band.clone());
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/genetic_test_request_grades/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
