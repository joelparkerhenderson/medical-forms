...
use crate::models::_entities::genetic_test_requests::{ActiveModel, Entity, Model};
...
pub struct Params {
    ...
    pub suspected_condition: String,
    pub consent_obtained: bool,
    pub genetic_counselling_offered: bool,
    pub affected_relative_tested: bool,
...
impl Params {
    fn update(&self, item: &mut ActiveModel) {
      ...
      item.suspected_condition = Set(self.suspected_condition.clone());
      item.consent_obtained = Set(self.consent_obtained);
      item.genetic_counselling_offered = Set(self.genetic_counselling_offered);
      item.affected_relative_tested = Set(self.affected_relative_tested);
...
pub fn routes() -> Routes {
    Routes::new()
        .prefix("api/genetic_test_requests/")
        .add("/", get(list))
        .add("/", post(add))
        .add("{id}", get(get_one))
