...
use crate::engine::{respirology_grader, flagged_issues, types::AssessmentData};
use crate::models::{
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "cough_assessment" => "coughAssessment",
        "dyspnoea_assessment" => "dyspnoeaAssessment",
        "chest_examination" => "chestExamination",
        "spirometry_results" => "spirometryResults",
        "oxygen_assessment" => "oxygenAssessment",
        "respiratory_infections" => "respiratoryInfections",
        "inhaler_medications" => "inhalerMedications",
...
async fn show_report(
    ...
    // Run grading engine
    let (respiratory_level, respiratory_score, fired_rules) =
        respirology_grader::calculate_respiratory_level(&assessment_data);
    ...
    let grading_result = crate::engine::types::GradingResult {
        respiratory_level: respiratory_level.clone(),
        respiratory_score,
    ...
    let smoking_category = crate::engine::utils::smoking_category(
        &assessment_data.patient_information.pack_years,
    );

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("respiratory_level", &respiratory_level);
    context.insert("respiratory_label", &crate::engine::utils::respiratory_level_label(&respiratory_level));
    context.insert("respiratory_score", &respiratory_score);
    context.insert("smoking_category", &smoking_category);
