<script lang="ts">
	...
	import { mrcGradeLabel, mrcGradeColor, bmiCategory, calculateAge } from '$lib/engine/utils';
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `respirology-assessment-${data.demographics.lastName}-${new Date().toISOString().slice(0, 10)}.pdf`;
				a.click();
...
{#if result}
	<div class="min-h-screen bg-gray-50">
		...
		<main class="mx-auto max-w-4xl px-4 py-6">
			<!-- MRC Grade Banner -->
			<div class="mb-6 rounded-xl border-2 p-6 text-center {mrcGradeColor(result.mrcGrade)}">
				<div class="text-3xl font-bold">MRC Grade {result.mrcGrade}</div>
				<div class="mt-1 text-lg">{mrcGradeLabel(result.mrcGrade)}</div>
			...
			{#if result.firedRules.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">MRC Grade Justification</h2>
			...
			<!-- Inhalers -->
			{#if data.currentMedications.inhalers.length > 0}
				<div class="mb-6 rounded-xl border border-gray-200 bg-white p-6">
					<h2 class="mb-4 text-lg font-bold text-gray-900">Inhalers</h2>
					<ul class="list-disc space-y-1 pl-5 text-sm">
						{#each data.currentMedications.inhalers as med}
