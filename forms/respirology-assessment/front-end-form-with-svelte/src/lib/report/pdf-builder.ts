...
import { mrcGradeLabel, bmiCategory, calculateAge } from '$lib/engine/utils';
...
export function buildPdfDocument(
	...
	return {
		...
		header: {
			text: 'RESPIROLOGY ASSESSMENT REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		content: [
			// Title & MRC Grade
			{
				text: `MRC Dyspnoea Grade: ${result.mrcGrade}`,
				fontSize: 24,
				bold: true,
				alignment: 'center',
				margin: [0, 0, 0, 4]
			...
			{
				text: mrcGradeLabel(result.mrcGrade),
				fontSize: 14,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 20]
			...
			...(data.chiefComplaint.primarySymptom
				? [
						...
						{
							text: data.chiefComplaint.primarySymptom,
							margin: [0, 0, 0, 16] as [number, number, number, number]
						}
					]
				: []),
			...
			...(result.firedRules.length > 0
				? [
						sectionHeader('MRC Grade Justification'),
						{
							table: {
								headerRows: 1,
								widths: [60, 80, '*', 40],
								body: [
									...
									...result.firedRules.map((r) => [
										...
										{ text: `MRC ${r.grade}`, fontSize: 9, bold: true }
									])
								]
							},
							layout: 'lightHorizontalLines',
			...
			// Inhalers
			...(data.currentMedications.inhalers.length > 0
				? [
						sectionHeader('Inhalers'),
						{
							ul: data.currentMedications.inhalers.map(
