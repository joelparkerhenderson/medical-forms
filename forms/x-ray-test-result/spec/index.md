# X-Ray Test Result — specification
...
`F-UNEXPECTED-FINDING-001` checked only fracture, bony lesion, and foreign body — missing dislocation, consolidation, pneumothorax, pleural effusion, free air, and unstable fracture, including the exact conditions driving `hasCriticalFinding` itself. Fixed by widening the condition to `hasAnyAbnormalFinding(r)`, in both `js/flags.js` and `src/lib/engine/flagged-issues.ts`. Fixed 2026-09-06; previously verified and documented (not silently patched) in `examples/personas.json`.
...
- `bin/test-form x-ray-test-result` exits cleanly.
...
- `bin/lily-html-refactor --check x-ray-test-result` reports no drift.
- LocalStorage keys preserve draft state across reloads:
  - `x-ray-test-result.front-end-with-html.v1` (HTML)
  - `x-ray-test-result.front-end-with-svelte.v1` (SvelteKit)
