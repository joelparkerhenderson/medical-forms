...
const STORAGE_KEY = 'x-ray-test-result.front-end-with-html.v1';

/**
 * Load saved state, null-safely rehydrating over a fresh empty result. Every
 * field in this form is a string or boolean, so a same-typeof merge keeps any
 * newly-added fields at their defaults; numeric fields (none yet) would round-
 * trip as `'' → null` and be accepted when saved as `null`.
 * @returns {import('./types.js').XRayResult}
 */
...
function mergeIntoDefaults(parsed) {
  ...
  for (const key of Object.keys(fresh)) {
    if (!parsed) continue;
    if (fresh[key] === null) {
      // Null-safe numeric rehydration: accept a finite number or keep null.
...
/** @param {import('./types.js').XRayResult} state */
...
/** @type {import('./types.js').XRayResult} */
let state = loadState();
...
function setField(field, value) {
  // Convention: numeric inputs store '' as null (no numeric fields today,
  // but keep the invariant future-proof).
  state[field] = value;
  saveState(state);
  updateProgress();
  updateConditionalAlerts();
...
function renderStep2() {
  const card = sectionCard({
    ...
    description: 'The performed examination, laterality, projections, and adequacy.'
  });
  ...
  card.appendChild(textArea({
    ...
    field: 'projections', rows: 3,
    placeholder: 'Views acquired (e.g. PA and lateral, AP, oblique) and any technique notes…'
  }));
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'A limited or non-diagnostic examination reduces diagnostic confidence: a non-diagnostic study classifies the result as inconclusive. Consider repeating or supplementing the examination.'
  }));
...
function renderStep4() {
  ...
  card.appendChild(checkboxGroup({
    ...
    items: [
      { field: 'fracture', label: 'Fracture' },
      { field: 'unstableFracture', label: 'Unstable fracture' },
      { field: 'dislocation', label: 'Dislocation' },
      { field: 'consolidation', label: 'Consolidation' },
      { field: 'pneumothorax', label: 'Pneumothorax' },
      { field: 'pleuralEffusion', label: 'Pleural effusion' },
      { field: 'foreignBody', label: 'Foreign body' },
      { field: 'freeAir', label: 'Free intraperitoneal air' },
      { field: 'bonyLesion', label: 'Bony lesion' },
  ...
  card.appendChild(conditionalAlert({
    ...
    message: 'A pneumothorax, free intraperitoneal air, or an unstable fracture auto-escalates the follow-up urgency to a critical alert. Ensure the result is communicated to the referrer on sign-off.'
  }));
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Impression',
    description: 'The summary impression and structured reporting category.'
  });
  ...
  card.appendChild(textInput({
    ...
    hint: 'Structured-reporting label, e.g. normal / abnormal-acute / abnormal-chronic.',
    placeholder: 'e.g. abnormal-acute'
  }));
...
function renderStep6() {
  const card = sectionCard({
    stepNumber: 6,
    title: 'Follow-up',
    description: 'The recommended follow-up imaging, referral, or management.'
  });

  card.appendChild(textArea({
    label: 'Recommended follow-up',
    field: 'recommendedFollowUp', rows: 4,
...
function updateConditionalAlerts() {
  ...
  const visibility = {
    ...
    'critical-finding-alert':
      state.pneumothorax || state.freeAir || state.unstableFracture,
...
// counts only when ticked). This lets the checkbox-only findings slot count as
...
const STEP_SLOTS = {
  ...
  2: [['bodyRegion'], ['laterality'], ['examinationAdequacy'], ['projections']],
  3: [['clinicalHistory'], ['comparisonWithPrevious']],
  4: [['findingsNarrative'],
      ['fracture', 'unstableFracture', 'dislocation', 'consolidation',
       'pneumothorax', 'pleuralEffusion', 'foreignBody', 'freeAir',
       'bonyLesion', 'incidentalFinding']],
  5: [['impression'], ['reportingCategory']],
  6: [['recommendedFollowUp']],
