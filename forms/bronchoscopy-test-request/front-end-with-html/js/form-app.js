...
// Bronchoscopy Test Request — clinician referral wizard (vanilla JS).
//
...
const STORAGE_KEY =
  'bronchoscopy-test-request.front-end-form-with-html.v1';
const TOTAL_STEPS = 7;
...
// Wrap a node in a host element carrying a conditional-display expression.
function conditional(expr, node) {
...
const PROCEDURE_OPTIONS = [
  ...
  { value: 'ebus', label: 'EBUS (endobronchial ultrasound)' },
...
const INDICATION_OPTIONS = [
  { value: 'suspected-lung-cancer', label: 'Suspected lung cancer' },
  { value: 'haemoptysis', label: 'Haemoptysis' },
  { value: 'persistent-cough', label: 'Persistent cough' },
  { value: 'lung-mass-on-imaging', label: 'Lung mass on imaging' },
  { value: 'mediastinal-lymphadenopathy', label: 'Mediastinal lymphadenopathy' },
  { value: 'infection-sampling', label: 'Infection sampling' },
  { value: 'foreign-body', label: 'Foreign body' },
  { value: 'stridor', label: 'Stridor' },
  { value: 'other', label: 'Other' }
];

const HAEMOPTYSIS_SEVERITY_OPTIONS = [
  { value: 'mild', label: 'Mild (streaking)' },
  { value: 'moderate', label: 'Moderate' },
  { value: 'massive', label: 'Massive (life-threatening)' }
];
...
function renderStep1() {
  ...
  card.appendChild(selectInput({
    ...
    options: [
      { value: 'respiratory-physician', label: 'Respiratory physician' },
      { value: 'thoracic-surgeon', label: 'Thoracic surgeon' },
      { value: 'gp', label: 'GP' },
      { value: 'oncologist', label: 'Oncologist' },
      { value: 'bronchoscopist', label: 'Bronchoscopist' },
      { value: 'other', label: 'Other' }
    ]
  }));
...
function renderStep3() {
  ...
  card.appendChild(textArea({
    ...
    placeholder: 'e.g. Obtain tissue diagnosis of the right upper-lobe mass.'
  }));
...
function renderStep4() {
  const card = sectionCard({
    stepNumber: 4,
    title: 'Symptoms & imaging',
    description: 'Respiratory symptoms and supporting imaging. Massive haemoptysis auto-escalates the urgency tier.'
  });
  card.appendChild(boolField({ label: 'Haemoptysis (coughing up blood)', section: 'symptoms', field: 'symptomHaemoptysis' }));
  card.appendChild(conditional('symptoms.symptomHaemoptysis=true', selectInput({
    label: 'Haemoptysis severity', section: 'symptoms', field: 'haemoptysisSeverity',
    options: HAEMOPTYSIS_SEVERITY_OPTIONS
  })));
  card.appendChild(boolField({ label: 'Persistent cough', section: 'symptoms', field: 'symptomCough' }));
  card.appendChild(boolField({ label: 'Breathlessness (dyspnoea)', section: 'symptoms', field: 'symptomBreathlessness' }));
  card.appendChild(boolField({ label: 'Unexplained weight loss', section: 'symptoms', field: 'symptomWeightLoss' }));
  card.appendChild(textArea({
    label: 'Imaging findings', section: 'symptoms', field: 'imagingFindings', rows: 2,
    placeholder: 'e.g. CT: 3 cm right upper-lobe mass with hilar lymphadenopathy.'
  }));
  return card;
}
...
function renderStep5() {
  const card = sectionCard({
    stepNumber: 5,
    title: 'Bleeding risk',
    description: 'Anticoagulation, antiplatelet therapy, and platelet count — drive the pre-procedure risk axis.'
  });
  card.appendChild(boolField({ label: 'Taking an anticoagulant', section: 'bleeding', field: 'takingAnticoagulant' }));
  card.appendChild(conditional('bleeding.takingAnticoagulant=true', textInput({
    label: 'Anticoagulant agent', section: 'bleeding', field: 'anticoagulantAgent',
    placeholder: 'e.g. warfarin, apixaban'
  })));
  card.appendChild(boolField({ label: 'Taking an antiplatelet agent', section: 'bleeding', field: 'takingAntiplatelet' }));
  card.appendChild(conditional('bleeding.takingAntiplatelet=true', textInput({
    label: 'Antiplatelet agent', section: 'bleeding', field: 'antiplateletAgent',
    placeholder: 'e.g. clopidogrel, aspirin'
  })));
  card.appendChild(textInput({ label: 'Platelet count', section: 'bleeding', field: 'plateletCount', type: 'number', min: 0, max: 1000, step: 1, unit: '×10⁹/L' }));
  return card;
}
...
function renderStep6() {
  const card = sectionCard({
    ...
    description: 'Oxygen dependence, ASA grade, and planned sedation.'
  });
  card.appendChild(boolField({ label: 'Oxygen-dependent (hypoxia)', section: 'procedural', field: 'oxygenDependent' }));
  card.appendChild(grid('two-col', [
    selectInput({ label: 'ASA grade', section: 'procedural', field: 'asaGrade', options: ASA_OPTIONS }),
    selectInput({ label: 'Planned sedation', section: 'procedural', field: 'sedation', options: SEDATION_OPTIONS })
  ]));
  card.appendChild(subHead('Red flags'));
  card.appendChild(boolField({ label: 'Haemodynamically unstable', section: 'procedural', field: 'haemodynamicallyUnstable' }));
  return card;
}
...
const TRACKED_FIELDS = [
  ...
  // 4 Symptoms
  ['symptoms', 'imagingFindings'],
  // 6 Procedural
  ['procedural', 'asaGrade'], ['procedural', 'sedation'],
  // 7 Triage
...
const SECTION_BY_STEP = {
  ...
  5: 'bleeding', 6: 'procedural', 7: 'triage'
};
...
const STEP_DEFINITIONS = [
  ...
  { step: 6, section: 'procedural', title: 'Procedural risk' },
...
function updateStepListStatuses(sectionAnswered, sectionTotal) {
  ...
  for (const def of STEP_DEFINITIONS) {
    ...
    // Bleeding-risk has no tracked text fields; treat as informational.
    if (t > 0 && a === t) {
...
function renderReport() {
  ...
  out.innerHTML = `
    <h2>Bronchoscopy Request Vetting Report</h2>
    ...
    <div class="axis-grid">
      ...
      <div class="axis-card">
        ...
        <span class="axis-value"><span class="band-badge tier-${esc(triageTier)}">${esc(titleCase(triageTier))}</span>${twoWeekWaitBadge}</span>
      </div>
      <div class="axis-card">
